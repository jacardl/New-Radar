...
- 启动：`node C:\Users\jacar\.qclaw\workspace\skills\web-access\scripts\cdp-proxy.mjs`
- 启动后持续运行，不建议主动停止（重启需 Chrome 重新授权）
- 强制停止：在任务管理器中结束 node 进程
...
```bash
curl -s -X POST "http://localhost:3456/setFiles?target=ID" -d "{\"selector\":\"input[type=file]\",\"files\":[\"C:/path/to/file.png\"]}"
```
...
```bash
curl -s "http://localhost:3456/screenshot?target=ID&file=C:/tmp/shot.png"
```
