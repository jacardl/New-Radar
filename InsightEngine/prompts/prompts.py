...
**内容密度要求**：
- 每100字尽量包含具体数据点或用户引用，但**前提是搜索结果中必须有这些数据**
- 每个分析点都要有真实数据或实例支撑
- 避免空洞的理论分析，重点关注实证发现
...
5. **具体扩充要求**：
   ...
   - **绝对防幻觉底线**：绝对不要为了满足“具体扩充要求”的密度或字数而编造数据或评论。宁可字数不足，也绝不编造虚假信息！
...
## 参考资料与民意来源
### 核心用户观点清单
- [话题/帖子标题](真实存在的URL) —— 平台/发布者
- [用户评论摘要](真实存在的URL) —— 平台/发布者
...
2. **民意声音突出与强制溯源**：
   - **绝对强制溯源**：报告中的每一句事实陈述、每一个数据（点赞数/比例）、每一个引语，都必须在文中紧跟其原始出处（如：[B站评论]、[微博用户XXX]）。
