...
class CrawledData(Base):
    """
    Unified table for all crawled data across platforms.
    Used by the analysis engines for semantic search.
    """
    __tablename__ = 'crawled_data'

    id = Column(Integer, primary_key=True)
    platform = Column(String(32), index=True, nullable=False)  # xhs, dy, wb, bili, ks, tieba, zhihu
    content_type = Column(String(16), nullable=False)  # content, comment
    content = Column(Text, nullable=False)  # Combined searchable text (title + body/desc)
    source_url = Column(Text)  # Original post/comment URL
    source_keyword = Column(Text)  # Search keyword that found this content
    create_time = Column(BigInteger, index=True)  # Post/comment creation timestamp
    ip_location = Column(String(128))  # IP location if available
    user_id = Column(String(128))  # Platform-specific user ID
    nickname = Column(String(256))  # User nickname
    # Engagement metrics
    liked_count = Column(String(32))  # Stored as string for consistency
    collected_count = Column(String(32))
    comment_count = Column(String(32))
    share_count = Column(String(32))
    # Embedding vector stored as JSON text
    # The backend's vector search uses: ORDER BY embedding <-> '[...]'::vector
    embedding = Column(Text)
    # Raw data JSON for platform-specific fields
    ...
    __table_args__ = (
        Index('idx_crawled_data_platform_content_type', 'platform', 'content_type'),
        Index('idx_crawled_data_source_keyword', 'source_keyword'),
    )
