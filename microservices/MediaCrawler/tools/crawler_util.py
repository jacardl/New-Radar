...
def show_qrcode(qr_code) -> None:  # type: ignore
    ...
    try:
        import requests
        if "," in qr_code:
            b64_data = qr_code.split(",")[1]
        else:
            b64_data = qr_code
        
        # 将二维码图片通过 POST 请求发送到 Flask 服务器
        requests.post(
            "http://127.0.0.1:5000/api/v1/internal/qrcode",
            json={"image": b64_data},
            timeout=2
        )
    except Exception as e:
        utils.logger.error(f"Failed to send QR code to dashboard: {e}")
