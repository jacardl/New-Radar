...
class ZhihuStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[ZhihuStoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_zhihu_content(content_item: ZhihuContent):
    ...
    await store_crawled_data(local_db_item, "zhihu", "content")
...
async def update_zhihu_content_comment(comment_item: ZhihuComment):
    ...
    await store_crawled_data(local_db_item, "zhihu", "comment")
