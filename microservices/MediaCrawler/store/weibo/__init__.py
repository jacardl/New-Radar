...
class WeibostoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[WeibostoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_weibo_note(note_item: Dict):
    ...
    await store_crawled_data(save_content_item, "wb", "content")
...
async def update_weibo_note_comment(note_id: str, comment_item: Dict):
    ...
    await store_crawled_data(save_comment_item, "wb", "comment")
