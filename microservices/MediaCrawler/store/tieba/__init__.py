...
class TieBaStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[TieBaStoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_tieba_note(note_item: TiebaNote):
    ...
    await store_crawled_data(save_note_item, "tieba", "content")
...
async def update_tieba_note_comment(note_id: str, comment_item: TiebaComment):
    ...
    await store_crawled_data(save_comment_item, "tieba", "comment")
