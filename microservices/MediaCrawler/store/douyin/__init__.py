...
class DouyinStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[DouyinStoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    await store_crawled_data(save_content_item, "dy", "content")
...
async def update_dy_aweme_comment(aweme_id: str, comment_item: Dict):
    ...
    await store_crawled_data(save_comment_item, "dy", "comment")
