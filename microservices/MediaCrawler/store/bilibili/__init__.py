...
class BiliStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[BiliStoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_bilibili_video(video_item: Dict):
    ...
    await store_crawled_data(save_content_item, "bili", "content")
...
async def update_bilibili_video_comment(video_id: str, comment_item: Dict):
    ...
    await store_crawled_data(save_comment_item, "bili", "comment")
