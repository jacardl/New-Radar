...
from store.crawled_data_store import store_crawled_data
...
class KuaishouStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError("[KuaishouStoreFactory.create_store] Invalid save option: only 'postgres' is supported.")
        return store_class()
...
async def update_kuaishou_video(video_item: Dict):
    ...
    await store_crawled_data(save_content_item, "ks", "content")
...
async def update_ks_video_comment(video_id: str, comment_item: Dict):
    ...
    await store_crawled_data(save_comment_item, "ks", "comment")


async def save_creator(creator: Dict):
    """
    Save Kuaishou creator information to local database.
    Args:
        creator: Creator info dict with 'ownerCount' and 'profile' keys.
    """
