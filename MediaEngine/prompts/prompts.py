...
4. **具体内容要求**：
   - **文本引用**：必须引用真实存在于搜索结果中的文字内容，严禁自己编造
   - **图片描述**：详细描述相关图片的内容、风格、传达的信息（基于搜索结果或图片元数据）
   - **数据提取**：准确提取和分析各种数据信息
   - **趋势识别**：基于多源信息识别发展趋势和模式

5. **信息密度标准**：
   - 尽量包含来自不同信息源的具体信息点，但**前提是搜索结果中必须有这些信息**
   - 充分利用搜索结果的多样性和丰富性
   - 避免信息冗余，确保每个信息点都有价值
   - 实现文字、图像、数据的有机结合
   - **绝对防幻觉底线**：绝对不要为了达到密度要求而编造数据、评论或视觉内容描述。如果搜索结果较少，宁可缩短篇幅，也绝不凭空捏造。
...
## 参考资料与引用来源
### 核心视觉与文本资产清单
- [视频/图文标题](真实存在的URL) —— 平台/发布者
- [笔记/帖子标题](真实存在的URL) —— 平台/发布者
...
**质量控制标准：**
- **信息覆盖度**：充分利用搜索结果中真实包含的各类信息
- **分析立体度**：从多个维度和角度进行综合分析
- **融合深度**：实现不同信息类型的深度融合
- **创新价值**：提供传统单一媒体分析无法实现的洞察
- **防幻觉底线**：绝对禁止自行生成表格数据、假图片链接或不存在的评论。如果确实没有数据，请明确表示数据不足。
