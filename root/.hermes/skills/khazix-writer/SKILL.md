...
- **长文（zhili-publish 用）**：4000-8000字，完整 Evolver 六段式或自定义深度结构，要有项目截图、数据、案例支撑
- **短评（zhilicomments-publish 用）**：500-800字，轻量三段式（事件+观点+金句收尾），一句话独立成段，大量短句断裂
- 段落要短，很多时候一句话就是一段
- 重要观点前后留白，让它"呼吸"
...
> ⚠️ **字数不达标是最高优先级的失败模式**。如果用户明确要求写长文，产出必须在4000字以上。宁可多写不可少写。低于4000字的长文等于是没有完成用户的任务。
...
**L1-0 字数检查（最高优先级）**
- 用户要求长文时，产出必须在4000字以上
- 字数不足4千字 = 任务未完成，宁可超长不可短少
...
khazix-writer 产出**纯文本内容**，发布环节由 zhili-publish 负责。交接时需满足以下要求：

**1. 内容格式**
- 纯文本，带自然段落分隔（空行）
- 无 HTML 标签，无 Markdown 格式标记
- 保留 `**粗体**` 表示重点，发布时由 zhili-publish 转为 `<strong style="color:#e63946;">`

**2. 章节标注**
- 用 `【一、章节名】` 标注大章节（如「【一、漏洞是什么】」）
- 正文段落连续写，不需要 `<p>` 标签

**3. 发布时的 HTML 格式规范（由 zhili-publish 执行）**
- 行高 1.6：`line-height:1.6`
- 两侧间距 8px：`padding:0 8px`
- 大标题 18px：`font-size:18px;font-weight:bold`
- 正文字号 16px：`font-size:16px`
- 全左对齐：`text-align:left`
- 关键词红色高亮：`<strong style="color:#e63946;">`
- 重要语句高亮标注格式：文章中用 `【重点】` 包裹的句子转为红色高亮
