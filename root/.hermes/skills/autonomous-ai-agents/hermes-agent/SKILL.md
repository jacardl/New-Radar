...
- **Feishu adapter fails to load**: The Feishu adapter requires `lark-oapi`. If the gateway logs `lark-oapi not installed or FEISHU_APP_ID/SECRET not set`, install it: `pip install lark-oapi`. Then `hermes gateway restart`.
