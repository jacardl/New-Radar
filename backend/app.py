...
# 注册OpenAI兼容API Blueprint
if OPENAI_COMPAT_AVAILABLE:
    app.register_blueprint(openai_bp, url_prefix='/v1')
    logger.info("OpenAI兼容API已注册 (POST /v1/chat/completions, GET /v1/models)")
else:
    logger.info("OpenAI兼容API不可用")
...
@app.route('/')
def landing_page():
    """Open WebUI 是唯一前端，此处仅做 API 健康检查入口."""
    return jsonify({
        'name': 'New Radar API',
        'version': '2.0.0',
