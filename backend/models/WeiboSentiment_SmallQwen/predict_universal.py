...
"""
Qwen3微博情感分析统一预测接口
支持0.6B�?B�?B三种规格的Embedding和LoRA模型
"""
...
class Qwen3UniversalPredictor:
    ...
    def load_model(self, model_type: str, model_size: str) -> None:
        ...
        if not os.path.exists(model_path):
            print(f"训练好的模型文件不存�? {model_path}")
            print(f"请先训练 {model_type.upper()}-{model_size} 模型，或检查模型路径配�?)
            return
        ...
        try:
            ...
            print(f"这可能是因为基础模型下载失败或训练好的模型文件损�?)
    ...
    def load_specific_models(self, model_configs: List[Tuple[str, str]]) -> None:
        """加载指定的模型配�?
        Args:
            model_configs: [(model_type, model_size), ...] 的列�?
        """
        print("加载指定的Qwen3模型...")
    ...
    def _select_and_load_model(self):
        ...
        print("请选择要使用的模型:")
        print("\n方法选择:")
        print("  1. Embedding + 分类�?(推理快速，显存占用�?")
        print("  2. LoRA微调 (效果更好，显存占用较�?")
        ...
        method_name = "Embedding + 分类�? if method_choice == '1' else "LoRA微调"
        
        print(f"\n已选择: {method_name}")
        print("\n模型大小选择:")
        print("  1. 0.6B - 轻量级，推理快�?)
        ...
        while True:
            try:
                ...
                for model_name, (pred, conf) in sorted_results:
                    ...
                    print(f"   {model_name:20}: {sentiment} (置信�? {conf:.4f})")
...
def main():
    ...
    if args.text:
        if args.ensemble and len(predictor.models) > 1:
            ...
            for model_name, (pred, conf) in results.items():
                sentiment = "正面" if pred == 1 else "负面"
                print(f"{model_name}: {sentiment} (置信�? {conf:.4f})")
    else:
        # 进入交互式模�?
