...
class Qwen3EmbeddingUniversal(BaseQwenModel):
    ...
    def train(self, train_data: List[Tuple[str, int]], **kwargs) -> None:
        """训练模型"""
        print(f"开始训�?Qwen3-Embedding-{self.model_size} 模型...")
        ...
        print(f"超参�? batch_size={batch_size}, lr={learning_rate}, epochs={num_epochs}")
    ...
    def load_model(self, model_path: str) -> None:
        ...
        if model_data['model_size'] != self.model_size:
            raise ValueError(f"模型大小不匹�? 期望{self.model_size}, 实际{model_data['model_size']}")
