"""Hindsight memory plugin é¥?MemoryProvider interface.
...
Config via environment variables:
  HINDSIGHT_API_KEY   é¥?API key for Hindsight Cloud
  HINDSIGHT_BANK_ID   é¥?memory bank identifier (default: hermes)
  HINDSIGHT_BUDGET    é¥?recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   é¥?API endpoint
  HINDSIGHT_MODE      é¥?cloud or local (default: cloud)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        """Custom setup wizard é¥?installs only the deps needed for the selected mode."""
        import getpass
        import subprocess
        import shutil
        import sys
        ...
        if not uv_path:
            print("  é¿?uv not found é¥?install it: curl -LsSf https://astral.sh/uv/install.sh | sh")
            ...
            try:
                ...
                print(f"  é?Dependencies up to date")
            except Exception as e:
                print(f"  é¿?Install failed: {e}")
        ...
        print(f"\n  é?Hindsight memory configured ({mode} mode)")
        if env_writes:
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            pass  # packaging not available or other issue é¥?proceed anyway
        ...
        if self._mode == "local_embedded":
            def _start_daemon():
                ...
                try:
                    ...
                    # Map openai_compatible/openrouter é«?openai for the daemon (OpenAI wire format)
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # and harmless é¥?the daemon keeps running independently.
                    try:
                        self._client.close()
                    except RuntimeError:
                        pass
