...
Config via environment variables (profile-scoped via each profile's .env):
  ...
  OPENVIKING_ACCOUNT   â€?Tenant account (default: root)
  OPENVIKING_USER      â€?Tenant user (default: default)
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        # Wait for any pending sync to finish first â€?do this before the
        ...
        def _write():
            try:
                ...
                client.post(f"/api/v1/sessions/{self._session_id}/messages", {
                    "role": "user",
                    "parts": [
                        {"type": "text", "text": f"[Memory note â€?{target}] {content}"},
                    ],
                })
            except Exception as e:
    ...
    def _tool_remember(self, args: dict) -> str:
        ...
        if category:
            text = f"[Remember â€?{category}] {content}"
