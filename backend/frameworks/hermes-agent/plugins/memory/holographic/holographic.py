...
each concept is a vector of angles in [0, 2Ï€). The algebraic operations are:

  bind   â€?circular convolution (phase addition)  â€?associates two concepts
  unbind â€?circular correlation (phase subtraction) â€?retrieves a bound value
  bundle â€?superposition (circular mean)           â€?merges multiple concepts
...
References:
  Plate (1995) â€?Holographic Reduced Representations
  Gayler (2004) â€?Vector Symbolic Architectures answer Jackendoff's challenges
"""
...
def encode_atom(word: str, dim: int = 1024) -> "np.ndarray":
    ...
    - Scale to [0, 2Ï€): phases = values * (2Ï€ / 65536)
...
def unbind(memory: "np.ndarray", key: "np.ndarray") -> "np.ndarray":
    ...
    unbind(bind(a, b), a) â‰?b  (up to superposition noise)
    """
    _require_numpy()
...
def encode_fact(content: str, entities: list[str], dim: int = 1024) -> "np.ndarray":
    ...
    This enables algebraic extraction:
        unbind(fact, bind(entity, ROLE_ENTITY)) â‰?content_vector
    """
    _require_numpy()
...
def phases_to_bytes(phases: "np.ndarray") -> bytes:
    """Serialize phase vector to bytes. float64 tobytes â€?8 KB at dim=1024."""
    _require_numpy()
