...
# config.yaml is authoritative for terminal settings ÃÂ¢?overrides .env.
...
if _config_path.exists():
    try:
        ...
        # Top-level simple values (fallback only ÃÂ¢?don't override .env)
        for _key, _val in _cfg.items():
            if isinstance(_val, (str, int, float, bool)) and _key not in os.environ:
                os.environ[_key] = str(_val)
        # Terminal config is nested ÃÂ¢?bridge to TERMINAL_* env vars.
        ...
        # and auxiliary_client.py ÃÂ¢?no env var bridging needed.
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            ...
            # Bridge agent.gateway_timeout ÃÂ¢?HERMES_AGENT_TIMEOUT env var.
        ...
        # Timezone: bridge config.yaml ÃÂ¢?HERMES_TIMEZONE env var.
...
# Validate config structure early ÃÂ¢?log warnings so gateway operators see problems
try:
...
def _platform_config_key(platform: "Platform") -> str:
    """Map a Platform enum to its config.yaml key (LOCALÃÂ¢?cli", rest->enum value)."""
...
def _resolve_gateway_model(config: dict | None = None) -> str:
    """Read model from config.yaml ÃÂ¢?single source of truth.
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    ...
    Tries in order:
    1. ``shutil.which("hermes")`` ÃÂ¢?standard PATH lookup
    2. ``sys.executable -m hermes_cli.main`` ÃÂ¢?fallback when Hermes is running
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # system prompt (including memory) every turn ÃÂ¢?breaking prefix cache
    ...
    def _flush_memories_for_session(
        ...
        Synchronous worker ÃÂ¢?meant to be called via run_in_executor from
        an async context so it doesn't block the event loop.
        """
        # Skip cron sessions ÃÂ¢?they run headless with no meaningful user
        ...
        try:
            ...
            tmp_agent = AIAgent(
                **runtime_kwargs,
                ...
                max_iterations=8,
                quiet_mode=True,
                skip_memory=True,  # Flush agent ÃÂ¢?no memory provider
                enabled_toolsets=["memory", "skills"],
                session_id=old_session_id,
            )
            # Fully silence the flush agent ÃÂ¢?quiet_mode only suppresses init
            # messages; tool call output still leaks to the terminal through
            # _safe_print ÃÂ¢?_print_fn.  Set a no-op to prevent that.
            ...
            try:
                ...
                pass  # Non-fatal ÃÂ¢?flush still works, just without the guard
            ...
            flush_prompt = (
                ...
                "3. If nothing is worth saving, that's fine ÃÂ¢?just skip.\n\n"
            )

            if _current_memory:
                flush_prompt += (
                    "IMPORTANT ÃÂ¢?here is the current live state of memory. Other "
    ...
    def _resolve_session_agent_runtime(
        ...
        *,
        ...
        if override:
            ...
            # Override exists but has no api_key ÃÂ¢?fall through to env-based
    ...
    def _load_background_notifications_mode() -> str:
        ...
        Modes:
          - ``all``    ÃÂ¢?push running-output updates *and* the final message (default)
          - ``result`` ÃÂ¢?only the final completion message (regardless of exit code)
          - ``error``  ÃÂ¢?only the final message when exit code is non-zero
          - ``off``    ÃÂ¢?no watcher messages at all
        """
    ...
    async def _handle_active_session_busy_message(self, event: MessageEvent, session_key: str) -> bool:
        ...
        if self._queue_during_drain_enabled():
            ...
            message = f"ÃÂ¢?Gateway {self._status_action_gerund()} ÃÂ¢?queued for the next turn after it comes back."
        else:
            message = f"ÃÂ¢?Gateway is {self._status_action_gerund()} and is not accepting another turn right now."
    ...
    async def start(self) -> bool:
        ...
        #
        # SKIP suspension after a clean (graceful) shutdown ÃÂ¢?the previous
        # process already drained active agents, so sessions aren't stuck.
        # This prevents unwanted auto-resets after `hermes update`,
        # `hermes gateway restart`, or `/restart`.
        _clean_marker = _hermes_home / ".clean_shutdown"
        if _clean_marker.exists():
            logger.info("Previous gateway exited cleanly ÃÂ¢?skipping session suspension")
            try:
        ...
        for platform, platform_config in self.config.platforms.items():
            ...
            try:
                ...
                if success:
                    ...
                    logger.info("ÃÂ¢?%s connected", platform.value)
                else:
                    logger.warning("ÃÂ¢?%s failed to connect", platform.value)
                    if adapter.has_fatal_error:
                        ...
                        # No fatal error info means likely a transient issue ÃÂ¢?queue for retry
                        self._failed_platforms[platform] = {
                            "config": platform_config,
                            "attempts": 1,
                            "next_retry": time.monotonic() + 30,
                ...
                logger.error("ÃÂ¢?%s error: %s", platform.value, e)
                self._update_platform_runtime_status(
                    platform.value,
                    platform_state="retrying",
                    error_code=None,
                ...
                # Unexpected exceptions are typically transient ÃÂ¢?queue for retry
                self._failed_platforms[platform] = {
                    "config": platform_config,
                    "attempts": 1,
                    "next_retry": time.monotonic() + 30,
    ...
    async def _session_expiry_watcher(self, interval: int = 300):
        ...
        """
        await asyncio.sleep(60)  # initial delay ÃÂ¢?let the gateway fully start
        ...
        while self._running:
            try:
                ...
                if _expired_entries:
                    ...
                    # Keys look like "agent:main:telegram:dm:12345" ÃÂ¢?platform is field [2].
    ...
    async def _platform_reconnect_watcher(self) -> None:
        ...
        Uses exponential backoff: 30s ÃÂ¢?60s ÃÂ¢?120s ÃÂ¢?240s ÃÂ¢?300s (cap).
        ...
        await asyncio.sleep(10)  # initial delay ÃÂ¢?let startup finish
        while self._running:
            if not self._failed_platforms:
                # Nothing to reconnect ÃÂ¢?sleep and check again
                for _ in range(30):
                    if not self._running:
                        return
                    await asyncio.sleep(1)
            ...
            for platform in list(self._failed_platforms.keys()):
                ...
                try:
                    ...
                    if success:
                        ...
                        logger.info("ÃÂ¢?%s reconnected successfully", platform.value)
        ...
        async def _stop_impl() -> None:
            ...
            for platform, adapter in list(self.adapters.items()):
                try:
                    ...
                    logger.debug("ÃÂ¢?%s background-task cancel error: %s", platform.value, e)
                try:
                    await adapter.disconnect()
                    logger.info("ÃÂ¢?%s disconnected", platform.value)
                except Exception as e:
                    logger.error("ÃÂ¢?%s disconnect error: %s", platform.value, e)
            ...
            # after unexpected exits ÃÂ¢?graceful shutdowns already drain
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        # the adapter itself ÃÂ¢?no user allowlist applies.
        ...
        # WhatsApp: resolve phoneÃÂ¢LID aliases from bridge session mapping files
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if getattr(event, "internal", False):
            ...
            # authorized ÃÂ¢?drop silently instead of triggering the pairing
        ...
        if _update_prompts.get(_quick_key):
            ...
            if response_text:
                ...
                try:
                    ...
                    return f"ÃÂ¢?Failed to send response to update process: {e}"
                _update_prompts.pop(_quick_key, None)
                label = response_text if len(response_text) <= 20 else response_text[:20] + "ÃÂ¢?
                return f"ÃÂ¢?Sent `{label}` to the update process."
        ...
        if _quick_key in self._running_agents and _stale_ts:
            ...
            # Never evict the pending sentinel ÃÂ¢?it was just placed moments
            ...
            # extreme (10x timeout or 2h, whichever is larger ÃÂ¢?catches
        ...
        if _quick_key in self._running_agents:
            ...
            # is truly hung ÃÂ¢?the executor thread is blocked and never checks
            ...
            if _cmd_def_inner and _cmd_def_inner.name == "stop":
                ...
                if _quick_key in self._running_agents:
                    del self._running_agents[_quick_key]
                logger.info("STOP for session %s ÃÂ¢?agent interrupted, session lock released", _quick_key[:20])
                return "ÃÂ¢?Stopped. You can continue this session."

            # /reset and /new must bypass the running-agent guard so they
            # actually dispatch as commands instead of being queued as user
            # text (which would be fed back to the agent with the same
            # broken history ÃÂ¢?#2170).  Interrupt the agent first, then
            ...
            # /queue <prompt> ÃÂ¢?queue without interrupting
            ...
            if _cmd_def_inner and _cmd_def_inner.name == "model":
                return "Agent is running ÃÂ¢?wait or /stop first, then switch models."

            # /approve and /deny must bypass the running-agent interrupt path.
            # The agent thread is blocked on a threading.Event inside
            # tools/approval.py ÃÂ¢?sending an interrupt won't unblock it.
            ...
            # /background must bypass the running-agent guard ÃÂ¢?it starts a
            # parallel task and must never interrupt the active conversation.
            if _cmd_def_inner and _cmd_def_inner.name == "background":
                return await self._handle_background_command(event)

            if event.message_type == MessageType.PHOTO:
                logger.debug("PRIORITY photo follow-up for session %s ÃÂ¢?queueing without interrupt", _quick_key[:20])
                adapter = self.adapters.get(source.platform)
                if adapter:
            ...
            if running_agent is _AGENT_PENDING_SENTINEL:
                ...
                if event.get_command() == "stop":
                    ...
                    if _quick_key in self._running_agents:
                        del self._running_agents[_quick_key]
                    logger.info("HARD STOP (pending) for session %s ÃÂ¢?sentinel cleared", _quick_key[:20])
                    return "ÃÂ¢?Force-stopped. The agent was still starting ÃÂ¢?session unlocked."
            ...
            if self._draining:
                ...
                return (
                    f"ÃÂ¢?Gateway {self._status_action_gerund()} ÃÂ¢?queued for the next turn after it comes back."
                    if self._queue_during_drain_enabled()
                    else f"ÃÂ¢?Gateway is {self._status_action_gerund()} and is not accepting another turn right now."
                )
        ...
        # in hermes_cli/commands.py ÃÂ¢?no hardcoded set to maintain here.
        ...
        if self._draining:
            return f"ÃÂ¢?Gateway is {self._status_action_gerund()} and is not accepting new work right now."
        ...
        if command:
            try:
                ...
                if cmd_key is not None:
                    ...
                    # Not an active skill ÃÂ¢?check if it's a known-but-disabled or
                    ...
                    if command.replace("_", "-") not in GATEWAY_KNOWN_COMMANDS:
                        logger.warning(
                            "Unrecognized slash command /%s from %s ÃÂ¢?"
        ...
        # No bare text matching ÃÂ¢?"yes" in normal conversation must not trigger
        ...
        # same session ÃÂ¢?corrupting the transcript.
            ...
            if audio_paths:
                ...
                if any(marker in message_text for marker in _stt_fail_markers):
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                "ÃÂ°ÃÂ¤ I received your voice message but can't transcribe it ÃÂ¢?"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if getattr(session_entry, 'was_auto_reset', False):
            ...
            try:
                ...
                # or crashed mid-operation) ÃÂ¢?skip the policy check.
                ...
                if should_notify:
                    ...
                    if adapter:
                        ...
                        notice = (
                            f"ÃÂ¢?Session automatically reset ({reason_text}). "
        ...
        # Only inject on NEW sessions ÃÂ¢?ongoing conversations already have the
        ...
        # compress proactively ÃÂ¢?before the agent even starts.  (#628)
        #
        ...
        #    means hygiene fires a bit early ÃÂ¢?safe and harmless.
        # -----------------------------------------------------------------
        if history and len(history) >= 4:
            ...
            # sessions that grew too large between turns ÃÂ¢?it fires pre-agent
            ...
            try:
                ...
                if _hyg_cfg_path.exists():
                    ...
                    # Read compression settings ÃÂ¢?only use enabled flag.
            ...
            if _hyg_compression_enabled:
                ...
                if _stored_tokens > 0:
                    ...
                    # sessions, but that just means hygiene fires a bit early ÃÂ¢?which
                    ...
                    # 85% * 1.4 = 119% of context ÃÂ¢?which exceeds the model's limit
                ...
                if _needs_compress:
                    logger.info(
                        "Session hygiene: %s messages, ~%s tokens (%s) ÃÂ¢?auto-compressing "
                    ...
                    try:
                        ...
                        if _hyg_runtime.get("api_key"):
                            ...
                            if len(_hyg_msgs) >= 4:
                                ...
                                # Reset stored token count ÃÂ¢?transcript was rewritten
                                ...
                                logger.info(
                                    "Session hygiene: compressed %s ÃÂ¢?%s msgs, "
                                    "~%s ÃÂ¢?~%s tokens",
        ...
        if not history and source.platform and source.platform != Platform.LOCAL and source.platform != Platform.WEBHOOK:
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        source.chat_id,
                        f"ÃÂ°ÃÂ¬ No home channel is set for {platform_name.title()}. "
        ...
        # -----------------------------------------------------------------
        # Voice channel awareness ÃÂ¢?inject current voice channel state
        ...
        try:
            ...
            if not response and agent_result.get("failed"):
                ...
                if _is_ctx_fail:
                    response = (
                        "ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Session too large for the model's context window.\n"
            ...
            # same failure on the next attempt ÃÂ¢?an infinite loop. (#1630)
            agent_failed_early = (
            ...
            if agent_failed_early:
                pass  # Skip all transcript writes ÃÂ¢?don't grow a broken session
            elif not history:
            ...
            # sends raw text chunks that include MEDIA: tags ÃÂ¢?the normal
            # post-processing in _process_message_background is skipped
            # when already_sent is True, so media files would never be
            # delivered without this.
            #
            # Never skip when the agent failed ÃÂ¢?the error message is new
            ...
            if status_code == 401:
                ...
                # for the API to process ÃÂ¢?treat it the same way.
                if _hist_len > 50:
                    return (
                        "ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Session too large for the model's context window.\n"
                        "Use /compact to compress the conversation, or "
    ...
    def _format_session_info(self) -> str:
        ...
        if config_context_length is not None:
            ...
            ctx_source = "default ÃÂ¢?set model.context_length in config to override"
        else:
            ctx_source = "detected"
        ...
        lines = [
            f"ÃÂ¢?Model: `{model}`",
            f"ÃÂ¢?Provider: {provider or 'openrouter'}",
            f"ÃÂ¢?Context: {ctx_display} tokens ({ctx_source})",
        ]

        # Show endpoint for local/custom setups
        if base_url and ("localhost" in base_url or "127.0.0.1" in base_url or "0.0.0.0" in base_url):
            lines.append(f"ÃÂ¢?Endpoint: {base_url}")
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> str:
        ...
        if new_entry:
            header = "ÃÂ¢?Session reset! Starting fresh."
        else:
            # No existing session, just create one
            new_entry = self.session_store.get_or_create_session(source, force_new=True)
            header = "ÃÂ¢?New session started!"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        """Handle /profile ÃÂ¢?show active profile name and home directory."""
        ...
        if profile_name:
            lines = [
                f"ÃÂ°ÃÂ¤ **Profile:** `{profile_name}`",
                f"ÃÂ° **Home:** `{display}`",
            ]
        else:
            lines = [
                "ÃÂ°ÃÂ¤ **Profile:** default",
                f"ÃÂ° **Home:** `{display}`",
            ]
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        lines.extend([
            ...
            f"**Agent Running:** {'Yes ÃÂ¢? if is_running else 'No'}",
            "",
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> str:
        ...
        if agent is _AGENT_PENDING_SENTINEL:
            ...
            logger.info("STOP (pending) for session %s ÃÂ¢?sentinel cleared", session_key[:20])
            return "ÃÂ¢?Stopped. The agent hadn't started yet ÃÂ¢?you can continue this session."
        if agent:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        if self._restart_requested or self._draining:
            ...
            if count:
                return f"ÃÂ¢?Draining {count} active agent(s) before restart..."
            return "ÃÂ¢?Gateway restart already in progress..."
        ...
        if active_agents:
            return f"ÃÂ¢?Draining {active_agents} active agent(s) before restart..."
        return "ÃÂ¢?Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `hermes gateway restart`."
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            skill_cmds = get_skill_commands()
            if skill_cmds:
                lines.append(f"\nÃÂ¢?**Skill Commands** ({len(skill_cmds)} active):")
                # Show first 10, then point to /commands for the rest
                sorted_cmds = sorted(skill_cmds)
                for cmd in sorted_cmds[:10]:
                    lines.append(f"`{cmd}` ÃÂ¢?{skill_cmds[cmd]['description']}")
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            f"ÃÂ° **Commands** ({len(entries)} total, page {page}/{total_pages})",
            "",
            *page_entries,
        ]
        if total_pages > 1:
            nav_parts = []
            if page > 1:
                nav_parts.append(f"`/commands {page - 1}` ÃÂ¢?prev")
            if page < total_pages:
                nav_parts.append(f"next ÃÂ¢?`/commands {page + 1}`")
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        Supports:
          /model                              ÃÂ¢?interactive picker (Telegram/Discord) or text list
        ...
        if not model_input and not explicit_provider:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        lines.append("_(session only ÃÂ¢?use `/model <name> --global` to persist)_")
                        return "\n".join(lines)
                    ...
                    if result.success:
                        return None  # Picker sent ÃÂ¢?adapter handles the response
            ...
            lines.append("`/model <name>` ÃÂ¢?switch model")
            lines.append("`/model <name> --provider <slug>` ÃÂ¢?switch provider")
            lines.append("`/model <name> --global` ÃÂ¢?persist")
            return "\n".join(lines)
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            f"ÃÂ° **Current provider:** {current_label} (`{current_provider}`)",
            "",
            "**Available providers:**",
        ]
        ...
        for p in providers:
            marker = " ÃÂ¢?active" if p["id"] == current_provider else ""
            auth = "ÃÂ¢? if p["authenticated"] else "ÃÂ¢?
            aliases = f"  _(also: {', '.join(p['aliases'])})_" if p["aliases"] else ""
            lines.append(f"{auth} `{p['id']}` ÃÂ¢?{p['label']}{aliases}{marker}")
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not args:
            lines = ["ÃÂ°ÃÂ­ **Available Personalities**\n"]
            lines.append("ÃÂ¢?`none` ÃÂ¢?(no personality overlay)")
            for name, prompt in personalities.items():
                if isinstance(prompt, dict):
                    preview = prompt.get("description") or prompt.get("system_prompt", "")[:50]
                else:
                    preview = prompt[:50] + "..." if len(prompt) > 50 else prompt
                lines.append(f"ÃÂ¢?`{name}` ÃÂ¢?{preview}")
        ...
        if args in ("none", "default", "neutral"):
            try:
                ...
                return f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Failed to save personality change: {e}"
            self._ephemeral_system_prompt = ""
            return "ÃÂ°ÃÂ­ Personality cleared ÃÂ¢?using base agent behavior.\n_(takes effect on next message)_"
            ...
            return f"ÃÂ°ÃÂ­ Personality set to **{args}**\n_(takes effect on next message)_"
    ...
    async def _handle_retry_command(self, event: MessageEvent) -> str:
        ...
        # Reset stored token count ÃÂ¢?transcript was truncated
        session_entry.last_prompt_tokens = 0
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        return f"ÃÂ¢ÃÂ©ÃÂ¯ÃÂ¸ Undid {removed_count} message(s).\nRemoved: \"{preview}\""
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        return (
            f"ÃÂ¢?Home channel set to **{chat_name}** (ID: {chat_id}).\n"
    ...
    async def _handle_voice_command(self, event: MessageEvent) -> str:
        ...
        if args in ("on", "enable"):
            ...
            # Toggle: off ÃÂ¢?on, on/all ÃÂ¢?off
    ...
    async def _handle_voice_channel_input(
        ...
        # Build source ÃÂ¢?reuse the linked text channel's metadata when available
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            # The TTS tool may convert to .ogg ÃÂ¢?use file_path from result.
    ...
    async def _deliver_media_from_response(
        ...
        text itself is already delivered ÃÂ¢?this only handles file attachments
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        """Handle /rollback command ÃÂ¢?list or restore filesystem checkpoints."""
        ...
        if result["success"]:
            return (
                f"ÃÂ¢?Restored to checkpoint {result['restored_to']}: {result['reason']}\n"
                f"A pre-rollback snapshot was saved automatically."
            )
        return f"ÃÂ¢?{result['error']}"
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        if not prompt:
            return (
                ...
                "You can keep chatting ÃÂ¢?the result will appear here when done."
            )
        ...
        return f'ÃÂ° Background task started: "{preview}"\nTask ID: {task_id}\nYou can keep chatting ÃÂ¢?results will appear when done.'
    ...
    async def _run_background_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    f"ÃÂ¢?Background task {task_id} failed: no provider credentials configured.",
            ...
            if response:
                ...
                preview = prompt[:60] + ("..." if len(prompt) > 60 else "")
                header = f'ÃÂ¢?Background task complete\nPrompt: "{preview}"\n\n'
                ...
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f'ÃÂ¢?Background task complete\nPrompt: "{preview}"\n\n(No response generated)',
                    metadata=_thread_metadata,
                )
            ...
            try:
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f"ÃÂ¢?Background task {task_id} failed: {e}",
                    metadata=_thread_metadata,
                )
            except Exception:
                pass
    ...
    async def _handle_btw_command(self, event: MessageEvent) -> str:
        """Handle /btw <question> ÃÂ¢?ephemeral side question in the same chat."""
    ...
    async def _run_btw_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    "ÃÂ¢?/btw failed: no provider credentials configured.",
                    metadata=_thread_meta,
                )
                return
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        """Handle /reasoning command ÃÂ¢?manage reasoning effort and display toggle.
        ...
        if not args:
            ...
            display_state = "on ÃÂ¢? if self._show_reasoning else "off"
            return (
                "ÃÂ°ÃÂ§ÃÂ  **Reasoning Settings**\n\n"
                f"**Effort:** `{level}`\n"
        ...
        if args in ("show", "on"):
            ...
            return (
                "ÃÂ°ÃÂ§ÃÂ  ÃÂ¢?Reasoning display: **ON**\n"
        ...
        if args in ("hide", "off"):
            ...
            return f"ÃÂ°ÃÂ§ÃÂ  ÃÂ¢?Reasoning display: **OFF** for **{platform_key}**"
        ...
        if effort == "none":
            ...
            return (
                f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Unknown argument: `{effort}`\n\n"
        ...
        if _save_config_key("agent.reasoning_effort", effort):
            return f"ÃÂ°ÃÂ§ÃÂ  ÃÂ¢?Reasoning effort set to `{effort}` (saved to config)\n_(takes effect on next message)_"
        else:
            return f"ÃÂ°ÃÂ§ÃÂ  ÃÂ¢?Reasoning effort set to `{effort}` (this session only)"

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast ÃÂ¢?mirror the CLI Priority Processing toggle in gateway chats."""
        import yaml
        ...
        if not model_supports_fast_mode(model):
            return "ÃÂ¢?/fast is only available for OpenAI models that support Priority Processing."
        ...
        if args in {"fast", "on"}:
            ...
            return (
                f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Unknown argument: `{args}`\n\n"
                "**Valid options:** normal, fast, status"
            )

        if _save_config_key("agent.service_tier", saved_value):
            return f"ÃÂ¢?ÃÂ¢?Priority Processing: **{label}** (saved to config)\n_(takes effect on next message)_"
        return f"ÃÂ¢?ÃÂ¢?Priority Processing: **{label}** (this session only)"

    async def _handle_yolo_command(self, event: MessageEvent) -> str:
        """Handle /yolo ÃÂ¢?toggle dangerous command approval bypass for this session only."""
        ...
        if current:
            ...
            return "ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ YOLO mode **OFF** for this session ÃÂ¢?dangerous commands will require approval."
        else:
            enable_session_yolo(session_key)
            return "ÃÂ¢?YOLO mode **ON** for this session ÃÂ¢?all commands auto-approved. Use with caution."

    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        """Handle /verbose command ÃÂ¢?cycle tool progress display mode.

        Gated by ``display.tool_progress_command`` in config.yaml (default off).
        When enabled, cycles the tool progress mode through off ÃÂ¢?new ÃÂ¢?all ÃÂ¢?
        verbose ÃÂ¢?off for the *current platform*.  The setting is saved to
        ...
        descriptions = {
            "off": "ÃÂ¢ÃÂ¯ÃÂ¸ Tool progress: **OFF** ÃÂ¢?no tool activity shown.",
            "new": "ÃÂ¢ÃÂ¯ÃÂ¸ Tool progress: **NEW** ÃÂ¢?shown when tool changes (preview length: `display.tool_preview_length`, default 40).",
            "all": "ÃÂ¢ÃÂ¯ÃÂ¸ Tool progress: **ALL** ÃÂ¢?every tool call shown (preview length: `display.tool_preview_length`, default 40).",
            "verbose": "ÃÂ¢ÃÂ¯ÃÂ¸ Tool progress: **VERBOSE** ÃÂ¢?every tool call with full arguments.",
        }
        ...
        try:
            ...
            return (
                ...
                f"_(saved for **{platform_key}** ÃÂ¢?takes effect on next message)_"
            )
        except Exception as e:
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Reset stored token count ÃÂ¢?transcript changed, old value is stale
            self.session_store.update_session(
            ...
            lines = [f"ÃÂ°ÃÂ¯ÃÂ¸?{summary['headline']}"]
            if focus_topic:
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        """Handle /title command ÃÂ¢?set or show the current session's title."""
        source = event.source
        session_entry = self.session_store.get_or_create_session(source)
        session_id = session_entry.session_id
        ...
        if existing_title is None:
            # Session doesn't exist in DB yet ÃÂ¢?create it
            try:
        ...
        if title_arg:
            ...
            try:
                ...
                return f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ {e}"
            if not sanitized:
                return "ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Title is empty after cleanup. Please use printable characters."
            # Set the title
            try:
                if self._session_db.set_session_title(session_id, sanitized):
                    return f"ÃÂ¢ÃÂ¯ÃÂ¸ Session title set: **{sanitized}**"
                else:
            ...
            if title:
                return f"ÃÂ° Session: `{session_id}`\nTitle: **{title}**"
            else:
                return f"ÃÂ° Session: `{session_id}`\nNo title set. Usage: `/title My Session Name`"

    async def _handle_resume_command(self, event: MessageEvent) -> str:
        """Handle /resume command ÃÂ¢?switch to a previously-named session."""
        if not self._session_db:
            return "Session database not available."
        ...
        if not name:
            ...
            try:
                ...
                lines = ["ÃÂ° **Named Sessions**\n"]
                for s in titled[:10]:
                    title = s["title"]
                    preview = s.get("preview", "")[:40]
                    preview_part = f" ÃÂ¢?_{preview}_" if preview else ""
                    lines.append(f"ÃÂ¢?**{title}**{preview_part}")
        ...
        if current_entry.session_id == target_id:
            return f"ÃÂ° Already on session **{name}**."
        ...
        return f"ÃÂ¢?Resumed session **{title}**{msg_part}. Conversation restored."
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        if not history:
            return "No conversation to branch ÃÂ¢?send a message first."
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if rl_state and rl_state.has_data:
                ...
                lines.append(f"ÃÂ¢ÃÂ±ÃÂ¯ÃÂ¸ **Rate Limits:** {format_rate_limit_compact(rl_state)}")
                lines.append("")

            # Session token usage ÃÂ¢?detailed breakdown matching CLI
            input_tokens = getattr(agent, "session_input_tokens", 0) or 0
            output_tokens = getattr(agent, "session_output_tokens", 0) or 0
            cache_read = getattr(agent, "session_cache_read_tokens", 0) or 0
            cache_write = getattr(agent, "session_cache_write_tokens", 0) or 0

            lines.append("ÃÂ° **Session Token Usage**")
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            lines = ["ÃÂ° **MCP Servers Reloaded**\n"]
            if reconnected:
                lines.append(f"ÃÂ¢ÃÂ»ÃÂ¯ÃÂ¸ Reconnected: {', '.join(sorted(reconnected))}")
            if added:
                lines.append(f"ÃÂ¢?Added: {', '.join(sorted(added))}")
            if removed:
                lines.append(f"ÃÂ¢?Removed: {', '.join(sorted(removed))}")
            if not connected_servers:
                lines.append("No MCP servers connected.")
            else:
                lines.append(f"\nÃÂ°ÃÂ§ {len(new_tools)} tool(s) available from {len(connected_servers)} server(s)")
            ...
            return f"ÃÂ¢?MCP reload failed: {e}"

    # ------------------------------------------------------------------
    # /approve & /deny ÃÂ¢?explicit dangerous-command approval
    # ------------------------------------------------------------------

    _APPROVAL_TIMEOUT_SECONDS = 300  # 5 minutes

    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        """Handle /approve command ÃÂ¢?unblock waiting agent thread(s).
        ...
        resumes and the terminal_tool executes the command inline ÃÂ¢?the same
        ...
        Usage:
            /approve              ÃÂ¢?approve oldest pending command once
            /approve all          ÃÂ¢?approve ALL pending commands at once
            /approve session      ÃÂ¢?approve oldest + remember for session
            /approve all session  ÃÂ¢?approve all + remember for session
            /approve always       ÃÂ¢?approve oldest + remember permanently
            /approve all always   ÃÂ¢?approve all + remember permanently
        """
        source = event.source
        session_key = self._session_key_for_source(source)
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                ...
                return "ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Approval expired (agent is no longer waiting). Ask the agent to try again."
        ...
        # Resume typing indicator ÃÂ¢?agent is about to continue processing.
        _adapter = self.adapters.get(source.platform)
        if _adapter:
        ...
        return f"ÃÂ¢?Command{'s' if count > 1 else ''} approved{scope_msg}{count_msg}. The agent is resuming..."

    async def _handle_deny_command(self, event: MessageEvent) -> str:
        """Handle /deny command ÃÂ¢?reject pending dangerous command(s).
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                self._pending_approvals.pop(session_key)
                return "ÃÂ¢?Command denied (approval was stale)."
        ...
        # Resume typing indicator ÃÂ¢?agent continues (with BLOCKED result).
        _adapter = self.adapters.get(source.platform)
        if _adapter:
            _adapter.resume_typing_for_chat(source.chat_id)
        ...
        return f"ÃÂ¢?Command{'s' if count > 1 else ''} denied{count_msg}."
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        """Handle /debug ÃÂ¢?upload debug report + logs and return paste URLs."""
        import asyncio
        ...
        def _collect_and_upload():
            ...
            try:
                ...
                return f"ÃÂ¢?Failed to upload debug report: {exc}"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command ÃÂ¢?update Hermes Agent to the latest version.
        ...
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "ÃÂ¢?/update is only available from messaging platforms. Run `hermes update` from the terminal."

        if is_managed():
            return f"ÃÂ¢?{format_managed_message('update Hermes Agent')}"

        project_root = Path(__file__).parent.parent.resolve()
        git_dir = project_root / '.git'

        if not git_dir.exists():
            return "ÃÂ¢?Not a git repository ÃÂ¢?cannot update."

        hermes_cmd = _resolve_hermes_bin()
        if not hermes_cmd:
            return (
                "ÃÂ¢?Could not locate the `hermes` command. "
        ...
        try:
            ...
            exit_code_path.unlink(missing_ok=True)
            return f"ÃÂ¢?Failed to start update: {e}"

        self._schedule_update_notification_watch()
        return "ÃÂ¢?Starting Hermes updateÃÂ¢?I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "ÃÂ¢?Hermes update finished.")
                    else:
                        await adapter.send(chat_id, "ÃÂ¢?Hermes update failed (exit code {}).".format(exit_code))
            ...
            # Check for prompts ÃÂ¢?only forward if we haven't already sent
            ...
            if (prompt_path.exists() and session_key
                ...
                try:
                    ...
                    if prompt_text:
                        ...
                        # .update_response to continue ÃÂ¢?it doesn't re-check
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "ÃÂ¢?Hermes update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "ÃÂ¢? + output[-3500:]
                    if exit_code == 0:
                        msg = f"ÃÂ¢?Hermes update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"ÃÂ¢?Hermes update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "ÃÂ¢?Hermes update finished successfully."
                    else:
                        msg = "ÃÂ¢?Hermes update failed. Check the gateway logs or run `hermes update` manually for details."
    ...
    async def _send_restart_notification(self) -> None:
        ...
        try:
            ...
            await adapter.send(
                chat_id,
                "ÃÂ¢?Gateway restarted successfully. Your session continues.",
                metadata=metadata,
            )
            logger.info(
    ...
    async def _enrich_message_with_transcription(
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        _no_stt_note = (
                            ...
                            "to it right now ÃÂ¢?no STT provider is configured. "
        ...
        if enriched_parts:
            ...
            # when we successfully transcribed the audio ÃÂ¢?it's redundant.
    ...
    async def _inject_watch_notification(self, synth_text: str, original_event) -> None:
        ...
        try:
            ...
            logger.info("Watch pattern notification ÃÂ¢?injecting for %s", platform_name)
    ...
    async def _run_process_watcher(self, watcher: dict) -> None:
        ...
        Notification mode (from ``display.background_process_notifications``):
          - ``all``    ÃÂ¢?running-output updates + final message
          - ``result`` ÃÂ¢?final completion message only
          - ``error``  ÃÂ¢?final message only when exit code != 0
          - ``off``    ÃÂ¢?no messages at all
        """
        from tools.process_registry import process_registry
                ...
                if agent_notify and not _pr_check.is_completion_consumed(session_id):
                    ...
                    if adapter and chat_id:
                        try:
                            ...
                            logger.info(
                                "Process %s finished ÃÂ¢?injecting agent notification for session %s",
    ...
    def _agent_config_signature(
        ...
        reused ÃÂ¢?preserving the frozen system prompt and tool schemas for
        prompt cache hits.
        """
        ...
        blob = _j.dumps(
            [
                ...
                # reasoning_config excluded ÃÂ¢?it's set per-message on the
    ...
    async def _run_agent(
        ...
        # Per-platform display settings ÃÂ¢?resolve via display_config module
        ...
        # Tool progress mode ÃÂ¢?resolved per-platform with env var fallback
        ...
        def progress_callback(event_type: str, tool_name: str = None, preview: str = None, args: dict = None, **kwargs):
            ...
            emoji = get_tool_emoji(tool_name, default="ÃÂ¢ÃÂ¯ÃÂ¸")
            ...
            if progress_mode == "verbose":
                if args:
                    ...
                    # in verbose mode ÃÂ¢?the user explicitly asked for full
            ...
            # compact ÃÂ¢?unlike CLI spinners, these persist as permanent messages).
            if preview:
                from agent.display import get_tool_preview_max_len
                _pl = get_tool_preview_max_len()
            ...
            # code (same boilerplate imports ÃÂ¢?identical previews).
        ...
        async def send_progress_messages():
            ...
            # editing (e.g. iMessage/BlueBubbles) ÃÂ¢?each progress update
                    ...
                    if can_edit and progress_msg_id is not None:
                        ...
                        if not result.success:
                            ...
                            if "flood" in _err or "retry after" in _err:
                                # Flood control hit ÃÂ¢?disable further edits,
        ...
        # Bridge sync step_callback ÃÂ¢?async hooks.emit for agent:step events
        ...
        # Bridge sync status_callback ÃÂ¢?async adapter.send for context pressure
        ...
        def run_sync():
            ...
            # `_resolve_turn_agent_config(message, ÃÂ¢?`.
            nonlocal message
            ...
            try:
                ...
                return {
                    "final_response": f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ Provider authentication failed: {exc}",
                    "messages": [],
                    "api_calls": 0,
                    "tools": [],
                }
            ...
            # None = no per-platform override ÃÂ¢?follow global config
            _streaming_enabled = (
            ...
            if _want_stream_deltas or _want_interim_consumer:
                try:
                    ...
                    if _adapter:
                        ...
                        # (e.g. QQ, WeChat) should skip streaming entirely ÃÂ¢?
            ...
            # Check agent cache ÃÂ¢?reuse the AIAgent from the previous message
            ...
            if agent is None:
                # Config changed or first message ÃÂ¢?create fresh agent
                agent = AIAgent(
                    model=turn_route["model"],
                    **turn_route["runtime"],
                    max_iterations=max_iterations,
            ...
            # Per-message state ÃÂ¢?callbacks and reasoning config change every
            ...
            # Background review delivery ÃÂ¢?send "ÃÂ°ÃÂ¾ Memory updated" etc. to user
            ...
            def _approval_notify_sync(approval_data: dict) -> None:
                ...
                # assistant_threads_setStatus disables the compose box ÃÂ¢?the
                ...
                # Check the *class* for the method, not the instance ÃÂ¢?avoids
                ...
                msg = (
                    f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ **Dangerous command requires approval:**\n"
                    f"```\n{cmd_preview}\n```\n"
                    f"Reason: {desc}\n\n"
            ...
            if not final_response:
                error_msg = f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ {result['error']}" if result.get("error") else "(No response generated)"
                return {
            ...
            if agent and session_key and hasattr(agent, 'session_id') and agent.session_id != session_id:
                ...
                logger.info(
                    "Session split detected: %s ÃÂ¢?%s (compression)",
            ...
            # user/assistant pair ÃÂ¢?losing the compressed summary and tail.
        ...
        # Start stream consumer task ÃÂ¢?polls for consumer creation since it
        ...
        # This is the PRIMARY interrupt path for regular text messages ÃÂ¢?
        ...
        async def monitor_for_interrupt():
            ...
            while True:
                ...
                try:
                    ...
                    # Must use session_key (build_session_key output) ÃÂ¢?NOT
                    # source.chat_id ÃÂ¢?because the adapter stores interrupt events
                    ...
                    if hasattr(_adapter, 'has_pending_interrupt') and _adapter.has_pending_interrupt(session_key):
                        ...
                        if agent:
                            ...
                            # is lost ÃÂ¢?neither the interrupt path nor the dequeue
                            # path finds it.
            ...
            while True:
                ...
                if _agent_ref and hasattr(_agent_ref, "get_activity_summary"):
                    try:
                        ...
                        _status_detail = " ÃÂ¢?" + ", ".join(_parts)
                    except Exception:
                        pass
                try:
                    await _notify_adapter.send(
                        source.chat_id,
                        f"ÃÂ¢?Still working... ({_elapsed_mins} min elapsed{_status_detail})",
                        metadata=_status_thread_metadata,
                    )
        ...
        try:
            ...
            if _agent_timeout is None:
                # Unlimited ÃÂ¢?still poll periodically for backup interrupt
                ...
                while True:
                    ...
                    if done:
                        response = _executor_task.result()
                        break
                    # Agent still running ÃÂ¢?check inactivity.
                    _agent_ref = agent_holder[0]
                    _idle_secs = 0.0
                    if _agent_ref and hasattr(_agent_ref, "get_activity_summary"):
                        try:
                        ...
                        if _warn_adapter:
                            ...
                            try:
                                await _warn_adapter.send(
                                    source.chat_id,
                                    f"ÃÂ¢ÃÂ ÃÂ¯ÃÂ¸ No activity for {_elapsed_warn} min. "
            ...
            if _inactivity_timeout:
                ...
                _diag_lines = [
                    f"ÃÂ¢ÃÂ±ÃÂ¯ÃÂ¸ Agent inactive for {_timeout_mins} min ÃÂ¢?no tool calls "
                    f"or API responses."
                ]
                if _cur_tool:
                    _diag_lines.append(
            ...
            # Skip eviction when the run failed ÃÂ¢?evicting a failed agent
            # forces MCP reinit on the next message for no benefit (the
            # same error will recur).  This was the root cause of #7130:
            # a bad model ID triggered fallback ÃÂ¢?eviction ÃÂ¢?recreation ÃÂ¢?
            # MCP reinit ÃÂ¢?same 400 ÃÂ¢?loop, burning 91% CPU for hours.
            _agent = agent_holder[0]
            ...
            if _agent is not None and hasattr(_agent, 'model') and not _run_failed:
                ...
                if _agent.model != _cfg_model and not self._is_intentional_model_switch(session_key, _agent.model):
                    # Fallback activated on a successful run ÃÂ¢?evict cached
            ...
            # "/new"), discard it ÃÂ¢?commands should never be passed to the agent
                ...
                if _pending_cmd_word:
                    try:
                        ...
                        if _rc_pending(_pending_cmd_word):
                            logger.info(
                                "Discarding command '/%s' from pending queue ÃÂ¢?"
            ...
            if pending_event or pending:
                ...
                if _interrupt_depth >= self._MAX_INTERRUPT_DEPTH:
                    logger.warning(
                        "Interrupt recursion depth %d reached for session %s ÃÂ¢?"
                ...
                if not was_interrupted:
                    # Queued message after normal completion ÃÂ¢?deliver the first
                ...
                # else: interrupted ÃÂ¢?discard the interrupted response ("Operation
        ...
        # BUT: never suppress delivery when the agent failed ÃÂ¢?the error
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    IMAGE_CACHE_EVERY = 60   # ticks ÃÂ¢?once per hour at default 60s interval
    CHANNEL_DIR_EVERY = 5    # ticks ÃÂ¢?every 5 minutes
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            for _ in range(20):
                ...
                # Still alive after 10s ÃÂ¢?force kill
                logger.warning(
            ...
            print(
                f"\nÃÂ¢?Gateway already running (PID {existing_pid}).\n"
    ...
    # Centralized logging ÃÂ¢?agent.log (INFO+), errors.log (WARNING+),
    # and gateway.log (INFO+, gateway-component records only).
    # Idempotent, so repeated calls from AIAgent.__init__ won't duplicate.
    from hermes_logging import setup_logging
    setup_logging(hermes_home=_hermes_home, mode="gateway")

    # Optional stderr handler ÃÂ¢?level driven by -v/-q flags on the CLI.
