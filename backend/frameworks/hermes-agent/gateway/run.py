...
if _config_path.exists():
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            ...
            # Bridge agent.gateway_timeout â†?HERMES_AGENT_TIMEOUT env var.
        ...
        # Timezone: bridge config.yaml â†?HERMES_TIMEZONE env var.
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    ...
    Tries in order:
    1. ``shutil.which("hermes")`` â€?standard PATH lookup
    2. ``sys.executable -m hermes_cli.main`` â€?fallback when Hermes is running
...
class GatewayRunner:
    ...
    def _flush_memories_for_session(
        ...
        Synchronous worker â€?meant to be called via run_in_executor from
        an async context so it doesn't block the event loop.
        """
        # Skip cron sessions â€?they run headless with no meaningful user
        ...
        try:
            ...
            tmp_agent = AIAgent(
                **runtime_kwargs,
                ...
                max_iterations=8,
                quiet_mode=True,
                skip_memory=True,  # Flush agent â€?no memory provider
                enabled_toolsets=["memory", "skills"],
                session_id=old_session_id,
            )
            # Fully silence the flush agent â€?quiet_mode only suppresses init
            # messages; tool call output still leaks to the terminal through
            # _safe_print â†?_print_fn.  Set a no-op to prevent that.
            ...
            try:
                ...
                pass  # Non-fatal â€?flush still works, just without the guard
            ...
            flush_prompt = (
                ...
                "3. If nothing is worth saving, that's fine â€?just skip.\n\n"
            )

            if _current_memory:
                flush_prompt += (
                    "IMPORTANT â€?here is the current live state of memory. Other "
        ...
        *,
        ...
        async def _stop_impl() -> None:
            ...
            for platform, adapter in list(self.adapters.items()):
                ...
                try:
                    await adapter.disconnect()
                    logger.info("âœ?%s disconnected", platform.value)
                except Exception as e:
                    logger.error("âœ?%s disconnect error: %s", platform.value, e)
            ...
            # after unexpected exits â€?graceful shutdowns already drain
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if getattr(event, "internal", False):
            ...
            # authorized â€?drop silently instead of triggering the pairing
        ...
        if _quick_key in self._running_agents:
            ...
            if _cmd_def_inner and _cmd_def_inner.name == "stop":
                ...
                if _quick_key in self._running_agents:
                    del self._running_agents[_quick_key]
                logger.info("STOP for session %s â€?agent interrupted, session lock released", _quick_key[:20])
                return "âš?Stopped. You can continue this session."
            ...
            if event.message_type == MessageType.PHOTO:
                logger.debug("PRIORITY photo follow-up for session %s â€?queueing without interrupt", _quick_key[:20])
                adapter = self.adapters.get(source.platform)
                if adapter:
            ...
            if running_agent is _AGENT_PENDING_SENTINEL:
                ...
                if event.get_command() == "stop":
                    ...
                    if _quick_key in self._running_agents:
                        del self._running_agents[_quick_key]
                    logger.info("HARD STOP (pending) for session %s â€?sentinel cleared", _quick_key[:20])
                    return "âš?Force-stopped. The agent was still starting â€?session unlocked."
        ...
        # in hermes_cli/commands.py â€?no hardcoded set to maintain here.
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        try:
            ...
            # same failure on the next attempt â€?an infinite loop. (#1630)
            agent_failed_early = (
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> str:
        ...
        if new_entry:
            header = "âœ?Session reset! Starting fresh."
        else:
            # No existing session, just create one
            new_entry = self.session_store.get_or_create_session(source, force_new=True)
            header = "âœ?New session started!"
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> str:
        ...
        if agent is _AGENT_PENDING_SENTINEL:
            ...
            logger.info("STOP (pending) for session %s â€?sentinel cleared", session_key[:20])
            return "âš?Stopped. The agent hadn't started yet â€?you can continue this session."
        if agent:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        if self._restart_requested or self._draining:
            ...
            if count:
                return f"â?Draining {count} active agent(s) before restart..."
            return "â?Gateway restart already in progress..."
        ...
        if active_agents:
            return f"â?Draining {active_agents} active agent(s) before restart..."
        return "â™?Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `hermes gateway restart`."
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            skill_cmds = get_skill_commands()
            if skill_cmds:
                lines.append(f"\nâš?**Skill Commands** ({len(skill_cmds)} active):")
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            if skill_cmds:
                entries.append("")
                entries.append("âš?**Skill Commands**:")
        ...
        lines = [
            f"ðŸ“š **Commands** ({len(entries)} total, page {page}/{total_pages})",
            "",
            *page_entries,
        ]
        if total_pages > 1:
            nav_parts = []
            if page > 1:
                nav_parts.append(f"`/commands {page - 1}` â†?prev")
            if page < total_pages:
                nav_parts.append(f"next â†?`/commands {page + 1}`")
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if not model_input and not explicit_provider:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        lines.append("_(session only â€?use `/model <name> --global` to persist)_")
                        return "\n".join(lines)
            ...
            lines.append("`/model <name>` â€?switch model")
            lines.append("`/model <name> --provider <slug>` â€?switch provider")
            lines.append("`/model <name> --global` â€?persist")
            return "\n".join(lines)
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            f"ðŸ”Œ **Current provider:** {current_label} (`{current_provider}`)",
            "",
            "**Available providers:**",
        ]
        ...
        for p in providers:
            marker = " â†?active" if p["id"] == current_provider else ""
            auth = "âœ? if p["authenticated"] else "â?
            aliases = f"  _(also: {', '.join(p['aliases'])})_" if p["aliases"] else ""
            lines.append(f"{auth} `{p['id']}` â€?{p['label']}{aliases}{marker}")
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not args:
            lines = ["ðŸŽ­ **Available Personalities**\n"]
            lines.append("â€?`none` â€?(no personality overlay)")
            for name, prompt in personalities.items():
                if isinstance(prompt, dict):
                    preview = prompt.get("description") or prompt.get("system_prompt", "")[:50]
                else:
                    preview = prompt[:50] + "..." if len(prompt) > 50 else prompt
                lines.append(f"â€?`{name}` â€?{preview}")
        ...
        if args in ("none", "default", "neutral"):
            try:
                ...
                return f"âš ï¸ Failed to save personality change: {e}"
            self._ephemeral_system_prompt = ""
            return "ðŸŽ­ Personality cleared â€?using base agent behavior.\n_(takes effect on next message)_"
            ...
            return f"ðŸŽ­ Personality set to **{args}**\n_(takes effect on next message)_"
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        return (
            f"âœ?Home channel set to **{chat_name}** (ID: {chat_id}).\n"
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            # The TTS tool may convert to .ogg â€?use file_path from result.
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        if not prompt:
            return (
                ...
                "You can keep chatting â€?the result will appear here when done."
            )
        ...
        return f'ðŸ”„ Background task started: "{preview}"\nTask ID: {task_id}\nYou can keep chatting â€?results will appear when done.'
    ...
    async def _run_background_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    f"â?Background task {task_id} failed: no provider credentials configured.",
            ...
            try:
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f"â?Background task {task_id} failed: {e}",
                    metadata=_thread_metadata,
                )
            except Exception:
                pass
    ...
    async def _handle_btw_command(self, event: MessageEvent) -> str:
        """Handle /btw <question> â€?ephemeral side question in the same chat."""
    ...
    async def _run_btw_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    "â?/btw failed: no provider credentials configured.",
                    metadata=_thread_meta,
                )
                return
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        if not args:
            ...
            display_state = "on âœ? if self._show_reasoning else "off"
            return (
                "ðŸ§  **Reasoning Settings**\n\n"
                f"**Effort:** `{level}`\n"
        ...
        if args in ("hide", "off"):
            ...
            return f"ðŸ§  âœ?Reasoning display: **OFF** for **{platform_key}**"
        ...
        if effort == "none":
            ...
            return (
                f"âš ï¸ Unknown argument: `{effort}`\n\n"
        ...
        if _save_config_key("agent.reasoning_effort", effort):
            return f"ðŸ§  âœ?Reasoning effort set to `{effort}` (saved to config)\n_(takes effect on next message)_"
        else:
            return f"ðŸ§  âœ?Reasoning effort set to `{effort}` (this session only)"
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        if not model_supports_fast_mode(model):
            return "âš?/fast is only available for OpenAI models that support Priority Processing."
        ...
        if args in {"fast", "on"}:
            ...
            return (
                f"âš ï¸ Unknown argument: `{args}`\n\n"
                "**Valid options:** normal, fast, status"
            )

        if _save_config_key("agent.service_tier", saved_value):
            return f"âš?âœ?Priority Processing: **{label}** (saved to config)\n_(takes effect on next message)_"
        return f"âš?âœ?Priority Processing: **{label}** (this session only)"
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> str:
        ...
        if current:
            ...
            return "âš ï¸ YOLO mode **OFF** for this session â€?dangerous commands will require approval."
        else:
            enable_session_yolo(session_key)
            return "âš?YOLO mode **ON** for this session â€?all commands auto-approved. Use with caution."
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        descriptions = {
            "off": "âš™ï¸ Tool progress: **OFF** â€?no tool activity shown.",
            "new": "âš™ï¸ Tool progress: **NEW** â€?shown when tool changes (preview length: `display.tool_preview_length`, default 40).",
            "all": "âš™ï¸ Tool progress: **ALL** â€?every tool call shown (preview length: `display.tool_preview_length`, default 40).",
            "verbose": "âš™ï¸ Tool progress: **VERBOSE** â€?every tool call with full arguments.",
        }
        ...
        try:
            ...
            return (
                ...
                f"_(saved for **{platform_key}** â€?takes effect on next message)_"
            )
        except Exception as e:
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        ...
        if title_arg:
            ...
            try:
                ...
                return f"âš ï¸ {e}"
            if not sanitized:
                return "âš ï¸ Title is empty after cleanup. Please use printable characters."
            # Set the title
            try:
                if self._session_db.set_session_title(session_id, sanitized):
                    return f"âœï¸ Session title set: **{sanitized}**"
                else:
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        """Handle /resume command â€?switch to a previously-named session."""
        if not self._session_db:
            return "Session database not available."
        ...
        if not name:
            ...
            try:
                ...
                lines = ["ðŸ“‹ **Named Sessions**\n"]
                for s in titled[:10]:
                    title = s["title"]
                    preview = s.get("preview", "")[:40]
                    preview_part = f" â€?_{preview}_" if preview else ""
                    lines.append(f"â€?**{title}**{preview_part}")
        ...
        if current_entry.session_id == target_id:
            return f"ðŸ“Œ Already on session **{name}**."
        ...
        return f"â†?Resumed session **{title}**{msg_part}. Conversation restored."
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        if not history:
            return "No conversation to branch â€?send a message first."
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if rl_state and rl_state.has_data:
                ...
                lines.append(f"â±ï¸ **Rate Limits:** {format_rate_limit_compact(rl_state)}")
                lines.append("")
            ...
            lines.append("ðŸ“Š **Session Token Usage**")
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            lines = ["ðŸ”„ **MCP Servers Reloaded**\n"]
            if reconnected:
                lines.append(f"â™»ï¸ Reconnected: {', '.join(sorted(reconnected))}")
            if added:
                lines.append(f"âž?Added: {', '.join(sorted(added))}")
            if removed:
                lines.append(f"âž?Removed: {', '.join(sorted(removed))}")
            if not connected_servers:
                lines.append("No MCP servers connected.")
            else:
                lines.append(f"\nðŸ”§ {len(new_tools)} tool(s) available from {len(connected_servers)} server(s)")
            ...
            return f"â?MCP reload failed: {e}"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                ...
                return "âš ï¸ Approval expired (agent is no longer waiting). Ask the agent to try again."
        ...
        return f"âœ?Command{'s' if count > 1 else ''} approved{scope_msg}{count_msg}. The agent is resuming..."
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                self._pending_approvals.pop(session_key)
                return "â?Command denied (approval was stale)."
        ...
        return f"â?Command{'s' if count > 1 else ''} denied{count_msg}."
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        """Handle /debug â€?upload debug report + logs and return paste URLs."""
        import asyncio
        ...
        def _collect_and_upload():
            ...
            try:
                ...
                return f"âœ?Failed to upload debug report: {exc}"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "âœ?/update is only available from messaging platforms. Run `hermes update` from the terminal."
        ...
        if not git_dir.exists():
            return "âœ?Not a git repository â€?cannot update."

        hermes_cmd = _resolve_hermes_bin()
        if not hermes_cmd:
            return (
                "âœ?Could not locate the `hermes` command. "
        ...
        try:
            ...
            exit_code_path.unlink(missing_ok=True)
            return f"âœ?Failed to start update: {e}"

        self._schedule_update_notification_watch()
        return "âš?Starting Hermes updateâ€?I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "âœ?Hermes update finished.")
                    else:
                        await adapter.send(chat_id, "â?Hermes update failed (exit code {}).".format(exit_code))
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "â?Hermes update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
        ...
        try:
            ...
            if adapter and chat_id:
                ...
                if output:
                    ...
                    if exit_code == 0:
                        msg = f"âœ?Hermes update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"â?Hermes update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "âœ?Hermes update finished successfully."
                    else:
                        msg = "â?Hermes update failed. Check the gateway logs or run `hermes update` manually for details."
    ...
    async def _inject_watch_notification(self, synth_text: str, original_event) -> None:
        ...
        try:
            ...
            logger.info("Watch pattern notification â€?injecting for %s", platform_name)
                ...
                if agent_notify and not _pr_check.is_completion_consumed(session_id):
                    ...
                    if adapter and chat_id:
                        try:
                            ...
                            logger.info(
                                "Process %s finished â€?injecting agent notification for session %s",
    ...
    async def _run_agent(
                    ...
                    if can_edit and progress_msg_id is not None:
                        ...
                        if not result.success:
                            ...
                            if "flood" in _err or "retry after" in _err:
                                # Flood control hit â€?disable further edits,
        ...
        def run_sync():
            ...
            if not final_response:
                error_msg = f"âš ï¸ {result['error']}" if result.get("error") else "(No response generated)"
                return {
            ...
            # user/assistant pair â€?losing the compressed summary and tail.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            print(
                f"\nâ?Gateway already running (PID {existing_pid}).\n"
