...
def _hash_chat_id(value: str) -> str:
    ...
    ``telegram:12345`` é«?``telegram:<hash>``
    ``12345``          é«?``<hash>``
    """
    colon = value.find(":")
    if colon > 0:
...
from .config import (
    Platform,
    GatewayConfig,
    SessionResetPolicy,  # noqa: F401 é¥?re-exported via gateway/__init__.py
    HomeChannel,
)
...
def build_session_context_prompt(
    ...
    *,
    ...
    # in the system prompt é¥?it changes per-turn and would bust the prompt
    ...
    if _is_shared_thread:
        lines.append(
            "**Session type:** Multi-user thread é¥?messages are prefixed "
    ...
    if context.source.platform == Platform.SLACK:
        ...
        lines.append(
            ...
            "You do NOT have access to Slack-specific APIs é¥?you cannot search "
        ...
        lines.append(
            ...
            "You do NOT have access to Discord-specific APIs é¥?you cannot search "
    ...
    for p in context.connected_platforms:
        if p != Platform.LOCAL:
            platforms_list.append(f"{p.value}: Connected é?)
    ...
    # Origin delivery
    if context.source.platform == Platform.LOCAL:
        lines.append("- `\"origin\"` é«?Local output (saved to files)")
    else:
        _origin_label = context.source.chat_name or (
            _hash_chat_id(context.source.chat_id) if redact_pii else context.source.chat_id
        )
        lines.append(f"- `\"origin\"` é«?Back to this chat ({_origin_label})")
    
    # Local always available
    lines.append("- `\"local\"` é«?Save to local files only (~/.hermes/cron/output/)")
    
    # Platform home channels
    for platform, home in context.home_channels.items():
        lines.append(f"- `\"{platform.value}\"` é«?Home channel ({home.name})")
...
def build_session_key(
    ...
    Group/channel rules:
      ...
      - thread_id differentiates threads within that parent chat.  When
        ...
        participants é¥?user_id is NOT appended, so every user in the thread
...
class SessionStore:
    ...
    def _is_session_expired(self, entry: SessionEntry) -> bool:
        ...
        Works from the entry alone é¥?no SessionSource needed.
    ...
    def has_any_sessions(self) -> bool:
        ...
        stay at 1 for single-platform users é¥?which is the bug this fixes.
    ...
    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        # full JSONL history é¥?the model sees a context of 1-4 messages instead
        # of hundreds.  Using the longer source prevents this silent truncation.
        if len(jsonl_messages) > len(db_messages):
            if db_messages:
                logger.debug(
                    "Session %s: JSONL has %d messages vs SQLite %d é¥?"
