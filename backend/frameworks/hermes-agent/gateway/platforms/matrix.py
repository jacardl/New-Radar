...
_PENDING_EVENT_TTL = 300  # seconds â€?stop retrying after 5 min
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Mention/thread gating â€?parsed once from env vars.
    ...
    async def _verify_device_keys_on_server(self, client: Any, olm: Any) -> bool:
        ...
        try:
            ...
            logger.error(
                "Matrix: cannot verify device keys on server: %s â€?refusing E2EE", exc,
            )
            return False
        ...
        if server_ed25519 != local_ed25519:
            if olm.account.shared:
                # Restored account from DB but server has different keys â€?corrupted state.
                logger.error(
            ...
            try:
                ...
                # permitted â€?that's fine, share_keys will try to overwrite.
                pass
            try:
                await olm.share_keys()
            except Exception as exc:
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                # (fresh crypto.db, share_keys re-upload) â€?otherwise the
            ...
            try:
                ...
                if _sync_msg and isinstance(_sync_msg, str):
                    ...
                    if "m_unknown_token" in _lower or "unknown_token" in _lower:
                        logger.error("Matrix: permanent auth error from sync: %s â€?stopping", _sync_msg)
                        return
                ...
                if "401" in err_str or "403" in err_str or "unauthorized" in err_str or "forbidden" in err_str:
                    logger.error("Matrix: permanent auth error: %s â€?stopping sync", exc)
                    return
    ...
    async def _on_encrypted_event(self, event: Any) -> None:
        ...
        logger.warning(
            "Matrix: could not decrypt event %s in %s â€?buffering for retry",
            event_id, room_id,
        )
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        rules â€?everything the Matrix HTML spec allows.
        """
        try:
