...
Shares credentials with the optional telephony skill é¥?same env vars:
  - TWILIO_ACCOUNT_SID
  - TWILIO_AUTH_TOKEN
...
Gateway-specific env vars:
  ...
  - SMS_WEBHOOK_URL      (public URL for Twilio signature validation é¥?required)
  - SMS_INSECURE_NO_SIGNATURE  (true to disable signature validation é¥?dev only)
...
class SmsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        if not self._from_number:
            logger.error("[sms] TWILIO_PHONE_NUMBER not set é¥?cannot send replies")
            return False
        ...
        if insecure_no_sig and not self._webhook_url:
            logger.warning(
                "[sms] SMS_INSECURE_NO_SIGNATURE=true é¥?Twilio signature validation "
    ...
    def _port_variant_url(url: str) -> str | None:
        ...
        # Non-standard port é¥?no variant
        return None
    ...
    async def _handle_webhook(self, request) -> "aiohttp.web.Response":
        ...
        # Return empty TwiML é¥?we send replies via the REST API, not inline TwiML
        return web.Response(
            text='<?xml version="1.0" encoding="UTF-8"?><Response></Response>',
            content_type="application/xml",
        )
