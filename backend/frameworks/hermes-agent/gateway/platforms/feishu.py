...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._chat_locks: Dict[str, asyncio.Lock] = {}  # chat_id â†?lock (per-chat serial processing)
    ...
    async def _add_ack_reaction(self, message_id: str) -> Optional[str]:
        ...
        if not self._client or not message_id:
            return None
        try:
            from lark_oapi.api.im.v1 import (  # lazy import â€?keeps optional dep optional
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        # URL verification challenge â€?respond before other checks so that Feishu's
    ...
    async def _resolve_sender_name_from_api(self, sender_id: Optional[str]) -> Optional[str]:
        ...
        ID-type detection mirrors openclaw: ou_ â†?open_id, on_ â†?union_id, else user_id.
    ...
    def _should_accept_group_message(self, message: Any, sender_id: Any, chat_id: str = "") -> bool:
        ...
        if not self._allow_group_message(sender_id, chat_id):
            return False
        # @_all is Feishu's @everyone placeholder â€?always route to the bot.
        raw_content = getattr(message, "content", "") or ""
