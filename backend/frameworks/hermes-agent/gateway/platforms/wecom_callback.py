...
class WecomCallbackAdapter(BasePlatformAdapter):
    ...
    def _resolve_app_for_chat(self, chat_id: str) -> Dict[str, Any]:
        ...
        if not app_name and ":" not in chat_id:
            # Legacy bare user_id â€?try to find a unique match.
    ...
    async def _handle_verify(self, request: web.Request) -> web.Response:
        """GET endpoint â€?WeCom URL verification handshake."""
    ...
    async def _handle_callback(self, request: web.Request) -> web.Response:
        """POST endpoint â€?receive an encrypted message callback."""
        msg_signature = request.query.get("msg_signature", "")
        timestamp = request.query.get("timestamp", "")
        nonce = request.query.get("nonce", "")
        ...
        for app in self._apps:
            try:
                ...
                # Immediately acknowledge â€?the agent's reply will arrive
