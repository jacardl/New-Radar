...
# Any other free model is hidden â€?prevents promotional/temporary free models
...
def filter_nous_free_models(
    ...
    Rules:
      â€?Paid models that are NOT in the allowlist â†?keep (normal case).
      â€?Free models that are NOT in the allowlist â†?drop.
      â€?Allowlist models that ARE free â†?keep.
      â€?Allowlist models that are NOT free â†?drop.
    """
    if not pricing:
        return model_ids  # no pricing data â€?can't filter, show everything
...
def is_nous_free_tier(account_info: dict[str, Any]) -> bool:
    ...
    the field is missing or unparseable (assumes paid â€?don't block users).
    """
...
def check_nous_free_tier() -> bool:
    ...
    Returns False (assume paid) on any error â€?never blocks paying users.
    """
...
#   tui_desc    â€?longer description for the `hermes model` interactive picker
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("anthropic",      "Anthropic",                "Anthropic (Claude models â€?API key or Claude Code)"),
    ProviderEntry("openai-codex",   "OpenAI Codex",             "OpenAI Codex"),
    ProviderEntry("xiaomi",         "Xiaomi MiMo",              "Xiaomi MiMo (MiMo-V2 models â€?pro, omni, flash)"),
    ProviderEntry("qwen-oauth",     "Qwen OAuth (Portal)",      "Qwen OAuth (reuses local Qwen CLI login)"),
    ProviderEntry("copilot",        "GitHub Copilot",           "GitHub Copilot (uses GITHUB_TOKEN or gh auth token)"),
    ProviderEntry("copilot-acp",    "GitHub Copilot ACP",       "GitHub Copilot ACP (spawns `copilot --acp --stdio`)"),
    ProviderEntry("huggingface",    "Hugging Face",             "Hugging Face Inference Providers (20+ open models)"),
    ProviderEntry("gemini",         "Google AI Studio",         "Google AI Studio (Gemini models â€?OpenAI-compatible endpoint)"),
    ProviderEntry("deepseek",       "DeepSeek",                 "DeepSeek (DeepSeek-V3, R1, coder â€?direct API)"),
    ProviderEntry("xai",            "xAI",                      "xAI (Grok models â€?direct API)"),
    ...
    ProviderEntry("arcee",          "Arcee AI",                 "Arcee AI (Trinity models â€?direct API)"),
...
def format_model_pricing_table(
    ...
    *models* is ``[(model_id, description), ...]``.
    ...
    for mid, inp, out, cache, is_cur in rows:
        marker = "  â†?current" if is_cur else ""
        if has_cache:
...
def detect_provider_for_model(
    ...
    2. Direct provider without credentials â†?remap to OpenRouter slug
    ...
    if direct_match:
        ...
        # No direct creds â€?try to find this model on OpenRouter instead
        or_slug = _find_openrouter_slug(name)
        if or_slug:
            return ("openrouter", or_slug)
        # Still return the direct provider â€?credential resolution will
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    ...
    if raw in _PRIORITY_PROCESSING_MODELS:
        return True
    # Anthropic fast mode â€?strip date suffixes (e.g. claude-opus-4-6-20260401)
