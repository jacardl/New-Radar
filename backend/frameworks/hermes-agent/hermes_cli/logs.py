"""``hermes logs`` â€?view and filter Hermes log files.
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hermes runs â€?try 'hermes chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet â€?run 'hermes chat' to generate logs)")
