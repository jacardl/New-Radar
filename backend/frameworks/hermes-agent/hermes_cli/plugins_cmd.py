"""``hermes plugins`` CLI subcommand â€?install, update, remove, and list plugins.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        if value:
            ...
            console.print(f"  [green]âœ“[/green] Saved to {display_hermes_home()}/.env")
        else:
...
def cmd_install(identifier: str, force: bool = False) -> None:
    ...
    # Copy .example files to their real names (e.g. config.yaml.example â†?config.yaml)
...
def cmd_enable(name: str) -> None:
    ...
    console.print(f"[green]âœ“[/green] Plugin [bold]{name}[/bold] enabled. Takes effect on next session.")
