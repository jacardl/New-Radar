...
def clarify_callback(cli, question, choices):
    ...
    if hasattr(cli, "_app") and cli._app:
        cli._app.invalidate()
    cprint(f"\n{_DIM}(clarify timed out after {timeout}s ‚Ä?agent will decide){_RST}")
    return (
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    if not getattr(cli, "_app", None):
        ...
        if not value:
            cprint(f"\n{_DIM}  ‚è?Secret entry cancelled{_RST}")
            return {
                "success": True,
                "reason": "cancelled",
                "stored_as": var_name,
        ...
        cprint(f"\n{_DIM}  ‚ú?Stored secret in {_dhh}/.env as {var_name}{_RST}")
        return {
            **stored,
            "skipped": False,
    ...
    if hasattr(cli, "_app") and cli._app:
        cli._app.invalidate()
    cprint(f"\n{_DIM}  ‚è?Timeout ‚Ä?secret capture cancelled{_RST}")
    return {
        "success": True,
        "reason": "timeout",
        "stored_as": var_name,
...
def approval_callback(cli, command: str, description: str) -> str:
    ...
    with lock:
        ...
        if hasattr(cli, "_app") and cli._app:
            cli._app.invalidate()
        cprint(f"\n{_DIM}  ‚è?Timeout ‚Ä?denying command{_RST}")
        return "deny"
