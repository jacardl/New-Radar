...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        print(f"  â?uv not found â?cannot install dependencies")
    ...
    try:
        ...
        print(f"  â?Installed {', '.join(missing)}")
    except subprocess.CalledProcessError as e:
        print(f"  â?Failed to install {', '.join(missing)}")
        stderr = (e.stderr or b"").decode()[:200]
        if stderr:
            print(f"    {stderr}")
        print(f"  Run manually: uv pip install --python {sys.executable} {' '.join(missing)}")
    except Exception as e:
        print(f"  â?Install failed: {e}")
        print(f"  Run manually: uv pip install --python {sys.executable} {' '.join(missing)}")
    ...
    for dep in ext_deps:
        ...
        if check_cmd:
            try:
                ...
                if install_cmd:
                    print(f"\n  â?'{dep_name}' not found. Install with:")
                    print(f"    {install_cmd}")
        ...
        if found:
            ...
            for pname, _, p in providers:
                if pname == provider_name:
                    if p.is_available():
                        print(f"  Status:    available â?)
                    else:
                        print(f"  Status:    not available â?)
