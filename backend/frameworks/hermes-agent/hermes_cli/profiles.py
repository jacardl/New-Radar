...
The "default" profile is ``~/.hermes`` itself é¥?backward compatible,
...
_PROFILE_DIRS = [
    ...
    # ssh, gh, npm é¥? so credentials don't bleed between profiles.  In Docker
...
# Memory files are part of the agent's curated identity é¥?just as important
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "profiles",             # other profiles é¥?never recursive-export
...
def create_wrapper_script(name: str) -> Optional[Path]:
    ...
    try:
        ...
        print(f"é¿?Could not create {wrapper_dir}: {e}")
        return None
    ...
    try:
        ...
        print(f"é¿?Could not create wrapper at {wrapper_path}: {e}")
        return None
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' é¥?it is the built-in profile (~/.hermes)."
        )
    ...
    if not soul_path.exists():
        try:
            ...
            pass  # best-effort é¥?don't fail profile creation over this
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        ...
        if not quiet:
            print(f"é¿?Skill seeding returned exit code {result.returncode}")
            if result.stderr.strip():
                print(f"  {result.stderr.strip()[:200]}")
        return None
    except subprocess.TimeoutExpired:
        if not quiet:
            print("é¿?Skill seeding timed out (60s)")
        return None
    except Exception as e:
        if not quiet:
            print(f"é¿?Skill seeding failed: {e}")
        return None
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    for item in items:
        print(f"  é¥?{item}")
    if gw_running:
        print(f"  é¿?Gateway is running é¥?it will be stopped.")
    ...
    if has_wrapper:
        if remove_wrapper_script(name):
            print(f"é?Removed {wrapper_path}")

    # 4. Remove profile directory
    try:
        shutil.rmtree(profile_dir)
        print(f"é?Removed {profile_dir}")
    except Exception as e:
        print(f"é¿?Could not remove {profile_dir}: {e}")
    ...
    try:
        ...
        if active == name:
            ...
            print("é?Active profile reset to default")
    except Exception:
        pass
    ...
    try:
        ...
        if _platform.system() == "Linux":
            ...
            if svc_file.exists():
                ...
                print(f"é?Service {svc_name} removed")
            ...
            if plist_path.exists():
                ...
                print(f"é?Launchd service removed")
    except Exception as e:
        print(f"é¿?Service cleanup: {e}")
    finally:
    ...
    try:
        ...
        for _ in range(20):
            ...
            try:
                ...
                print(f"é?Gateway stopped (PID {pid})")
                return
        # Force kill
        try:
            ...
            pass
        print(f"é?Gateway force-stopped (PID {pid})")
    except (ProcessLookupError, PermissionError):
        print("é?Gateway already stopped")
    except Exception as e:
        print(f"é¿?Could not stop gateway: {e}")
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.hermes itself é¥?its parent is ~/ and its
    ...
    # Named profiles é¥?stage a filtered copy to exclude credentials
    with tempfile.TemporaryDirectory() as tmpdir:
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.hermes itself é¥?disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' é¥?that is the built-in root profile (~/.hermes). "
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    if new_name == "default":
        raise ValueError("Cannot rename to 'default' é¥?it is reserved.")
    ...
    print(f"é?Renamed {old_dir.name} é«?{new_dir.name}")
    ...
    if not collision:
        ...
        print(f"é?Alias updated: {new_name}")
    else:
        print(f"é¿?Cannot create alias '{new_name}' é¥?{collision}")

    # 4. Update active_profile if it pointed to old name
    try:
        if get_active_profile() == old_name:
            set_active_profile(new_name)
            print(f"é?Active profile updated: {new_name}")
    except Exception:
        pass
