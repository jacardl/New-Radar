...
True on success.  No external Python dependencies ÃÂ¢?uses only OS-level
CLI tools that ship with the platform (or are commonly installed).

Platform support:
  macOS   ÃÂ¢?osascript (always available), pngpaste (if installed)
  Windows ÃÂ¢?PowerShell via .NET System.Windows.Forms.Clipboard
  WSL2    ÃÂ¢?powershell.exe via .NET System.Windows.Forms.Clipboard
  Linux   ÃÂ¢?wl-paste (Wayland), xclip (X11)
"""
...
def has_clipboard_image() -> bool:
    ...
    Lighter than save_clipboard_image ÃÂ¢?doesn't extract or write anything.
    """
    if sys.platform == "darwin":
...
def _macos_has_image() -> bool:
    ...
    try:
        ...
        return "ÃÃÂ«class PNGfÃÃÂ»" in info.stdout or "ÃÃÂ«class TIFFÃÃÂ»" in info.stdout
    except Exception:
        return False


def _macos_pngpaste(dest: Path) -> bool:
    """Use pngpaste (brew install pngpaste) ÃÂ¢?fastest, cleanest."""
    try:
        r = subprocess.run(
            ["pngpaste", str(dest)],
...
def _macos_osascript(dest: Path) -> bool:
    ...
    script = (
        'try\n'
        '  set imgData to the clipboard as ÃÃÂ«class PNGfÃÃÂ»\n'
...
# .NET System.Windows.Forms.Clipboard ÃÂ¢?used by both native Windows (powershell)
...
def _windows_save(dest: Path) -> bool:
    """Extract clipboard image on native Windows via PowerShell ÃÂ¢?base64 PNG."""
    ps = _get_ps_exe()
    if ps is None:
        logger.debug("No PowerShell found ÃÂ¢?Windows clipboard image paste unavailable")
        return False
    try:
        r = subprocess.run(
...
def _linux_save(dest: Path) -> bool:
    """Try clipboard backends in priority order: WSL ÃÂ¢?Wayland ÃÂ¢?X11."""
    if _is_wsl():
        if _wsl_save(dest):
            return True
        # Fall through ÃÂ¢?WSLg might have wl-paste or xclip working
...
def _wsl_has_image() -> bool:
    ...
    try:
        ...
        logger.debug("powershell.exe not found ÃÂ¢?WSL clipboard unavailable")
    except Exception as e:
...
def _wsl_save(dest: Path) -> bool:
    """Extract clipboard image via powershell.exe ÃÂ¢?base64 ÃÂ¢?decode to PNG."""
    try:
        r = subprocess.run(
            ["powershell.exe", "-NoProfile", "-NonInteractive", "-Command",
             _PS_EXTRACT_IMAGE],
...
def _wayland_has_image() -> bool:
    ...
    try:
        ...
        logger.debug("wl-paste not installed ÃÂ¢?Wayland clipboard unavailable")
    except Exception:
        pass
    return False
...
def _convert_to_png(path: Path) -> bool:
    ...
    try:
        ...
        if r.returncode == 0 and path.exists() and path.stat().st_size > 0:
            ...
            # Convert failed ÃÂ¢?restore the original file
            tmp.rename(path)
    except FileNotFoundError:
        logger.debug("ImageMagick not installed ÃÂ¢?cannot convert BMP to PNG")
    ...
    # Can't convert ÃÂ¢?BMP is still usable as-is for most APIs
...
def _xclip_save(dest: Path) -> bool:
    ...
    try:
        ...
        logger.debug("xclip not installed ÃÂ¢?X11 clipboard image paste unavailable")
        return False
    except Exception:
        return False
