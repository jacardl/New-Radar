...
# ---------------------------------------------------------------------------
# Tip corpus é¥?one-liners covering slash commands, CLI flags, config,
# keybindings, tools, gateway, skills, profiles, and workflow tricks.
# ---------------------------------------------------------------------------

TIPS = [
    # --- Slash Commands ---
    "/btw <question> asks a quick side question without tools or history é¥?great for clarifications.",
    ...
    "/rollback lists filesystem checkpoints é¥?restore files the agent modified to any prior state.",
    ...
    "/title \"my project\" names your session é¥?resume it later with /resume or hermes -c.",
    ...
    "/retry resends your last message é¥?useful when the agent's response wasn't quite right.",
    "/verbose cycles tool progress display: off é«?new é«?all é«?verbose.",
    ...
    "/model lets you switch models mid-session é¥?try /model sonnet or /model gpt-5.",
    "/model --global changes your default model permanently.",
    "/personality pirate sets a fun personality é¥?14 built-in options from kawaii to shakespeare.",
    "/skin changes the CLI theme é¥?try ares, mono, slate, poseidon, or charizard.",
    ...
    "/cron manages scheduled tasks é¥?set up recurring prompts with delivery to any platform.",
    ...
    "Typing @ triggers filesystem path completion é¥?navigate to any file interactively.",
    ...
    "Ctrl+Z suspends Hermes to the background é¥?run fg in your shell to resume.",
    ...
    "hermes -w creates an isolated git worktree é¥?perfect for parallel agent workflows.",
    ...
    "hermes dump outputs a compact setup summary é¥?great for bug reports.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hermes tools programmatically é¥?results stay out of context.",
    "delegate_task spawns up to 3 concurrent sub-agents with isolated contexts for parallel work.",
    "web_extract works on PDF URLs é¥?pass any PDF link and it converts to markdown.",
    "search_files is ripgrep-backed and faster than grep é¥?use it instead of terminal grep.",
    ...
    "read_file auto-deduplicates é¥?re-reading an unchanged file returns a lightweight stub.",
    "browser_vision takes a screenshot and analyzes it with AI é¥?works for CAPTCHAs and visual content.",
    ...
    "text_to_speech converts text to audio é¥?plays as voice bubbles on Telegram.",
    ...
    "Profile names become shell commands é¥?'hermes profile create coder' creates the 'coder' command.",
    ...
    # --- Sessions ---
    "Sessions auto-generate descriptive titles after the first exchange é¥?no manual naming needed.",
    "Session titles support lineage: \"my project\" é«?\"my project #2\" é«?\"my project #3\".",
    ...
    # --- Memory ---
    "Memory is a frozen snapshot é¥?changes appear in the system prompt only at next session start.",
    ...
    "Every installed skill automatically becomes a slash command é¥?type / to see them all.",
    "hermes skills install official/security/1password installs optional skills from the repo.",
    "Skills can restrict to specific OS platforms é¥?some only load on macOS or Linux.",
    ...
    "If a cron response starts with [SILENT], delivery is suppressed é¥?useful for monitoring-only jobs.",
    "Cron supports relative delays (30m), intervals (every 2h), cron expressions, and ISO timestamps.",
    "Cron jobs run in completely fresh agent sessions é¥?prompts must be self-contained.",
    ...
    "Streaming TTS plays sentences as they generate é¥?you don't wait for the full response.",
    ...
    "DingTalk uses Stream Mode é¥?no webhooks or public URL needed.",
    ...
    "The gateway supports inactivity-based timeouts é¥?active agents can run indefinitely.",
    ...
    "MCP subprocesses receive a filtered environment é¥?only safe system vars pass through.",
    ...
    # --- Context & Compression ---
    "Context auto-compresses when it reaches the threshold é¥?memories are flushed and history summarized.",
    "The status bar turns yellow, then orange, then red as context fills up.",
    "SOUL.md at ~/.hermes/SOUL.md is the agent's primary identity é¥?customize it to shape behavior.",
    ...
    "Camofox is an anti-detection browser é¥?Firefox fork with C++ fingerprint spoofing.",
    "browser_navigate returns a page snapshot automatically é¥?no need to call browser_snapshot after.",
    "browser_vision with annotate=true overlays numbered labels on interactive elements.",

    # --- MCP ---
    "MCP servers are configured in config.yaml é¥?both stdio and HTTP transports supported.",
    "Per-server tool filtering: tools.include whitelists and tools.exclude blacklists specific tools.",
    "MCP servers auto-generate toolsets at runtime é¥?hermes tools can toggle them per platform.",
    "MCP OAuth support: auth: oauth enables browser-based authorization with PKCE.",

    # --- Checkpoints & Rollback ---
    "Checkpoints have zero overhead when no files are modified é¥?enabled by default.",
    ...
    "Checkpoints use shadow repos in ~/.hermes/checkpoints/ é¥?your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hermes chat -Q enables quiet mode for programmatic use é¥?suppresses banner and spinner.",
    ...
    "The agent auto-generates session titles in a background thread é¥?zero latency impact.",
    ...
    "BOOT.md at ~/.hermes/BOOT.md runs automatically on every gateway start é¥?use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.hermes/scripts/ and run before the agent é¥?perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity é¥?rewrite it to make Hermes your own.",
    ...
    "network.force_ipv4: true in config.yaml fixes hangs on servers with broken IPv6 é¥?monkey-patches socket.",
    ...
    "Bare sudo commands are auto-rewritten to pipe SUDO_PASSWORD from .env é¥?no interactive prompt needed.",
    "execute_code has built-in helpers: json_parse() for tolerant parsing, shell_quote(), and retry() with backoff.",
    "execute_code's 7 sandbox tools (web_search, terminal, read/write/search/patch) use RPC é¥?never enter context.",
    ...
    "V4A patch format supports Add File, Delete File, and Move File directives é¥?not just Update.",
    "MCP servers can request LLM completions back via sampling é¥?the agent becomes a tool for the server.",
    ...
    "Delegation has a heartbeat thread é¥?child activity propagates to the parent, preventing gateway timeouts.",
    "When a provider returns HTTP 402 (payment required), the auxiliary client auto-falls back to the next one.",
    "agent.tool_use_enforcement steers models that describe actions instead of calling tools é¥?auto for GPT/Codex.",
    "agent.restart_drain_timeout (default 60s) lets running agents finish before a gateway restart takes effect.",
    "The gateway caches AIAgent instances per session é¥?destroying this cache breaks Anthropic prompt caching.",
    "Any website can expose skills via /.well-known/skills/index.json é¥?the skills hub discovers them automatically.",
    ...
    "Each profile gets its own subprocess HOME at HERMES_HOME/home/ é¥?isolated git, ssh, npm, gh configs.",
    ...
    "Ctrl+C has 5 priority tiers: cancel recording é«?cancel prompts é«?cancel picker é«?interrupt agent é«?exit.",
    "Every interrupt during an agent run is logged to ~/.hermes/interrupt_debug.log with timestamps.",
    "BROWSER_CDP_URL connects browser tools to any running Chrome é¥?accepts WebSocket, HTTP, or host:port.",
    ...
    "human_delay.mode in config simulates human typing speed é¥?configurable min_ms/max_ms range.",
    "Config version migrations run automatically on load é¥?new config keys appear without manual intervention.",
    ...
    "Audio level visualization shows é»ä½²æºé»å£æ½é»åªæé»?bars during voice recording based on microphone RMS levels.",
    "Profile names cannot collide with existing PATH binaries é¥?'hermes profile create ls' would be rejected.",
    "hermes profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    "The voice record key is configurable via voice.record_key in config.yaml é¥?not just Ctrl+B.",
    ".cursorrules and .cursor/rules/*.mdc files are auto-detected and loaded as project context.",
    "Context files support 10+ prompt injection patterns é¥?invisible Unicode, 'ignore instructions', exfil attempts.",
    ...
    "The auxiliary client treats 'main' as a provider alias é¥?resolves to your actual primary provider + model.",
    "Smart routing can auto-route simple queries to a cheaper model é¥?set smart_model_routing.enabled: true.",
    "hermes claw migrate --dry-run previews OpenClaw migration without writing anything.",
    "File paths pasted with quotes or escaped spaces are handled automatically é¥?no manual cleanup needed.",
    "Slash commands never trigger the large-paste collapse é¥?/command with big arguments works correctly.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn é¥?including vars from ~/.hermes/.env.",
