...
TIPS = [
    # --- Slash Commands ---
    "/btw <question> asks a quick side question without tools or history â€?great for clarifications.",
    ...
    "/title \"my project\" names your session â€?resume it later with /resume or hermes -c.",
    ...
    "Ctrl+Z suspends Hermes to the background â€?run fg in your shell to resume.",
    ...
    "hermes -w creates an isolated git worktree â€?perfect for parallel agent workflows.",
    ...
    "hermes dump outputs a compact setup summary â€?great for bug reports.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hermes tools programmatically â€?results stay out of context.",
    ...
    "Profile names become shell commands â€?'hermes profile create coder' creates the 'coder' command.",
    ...
    "SOUL.md at ~/.hermes/SOUL.md is the agent's primary identity â€?customize it to shape behavior.",
    ...
    "MCP servers auto-generate toolsets at runtime â€?hermes tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hermes/checkpoints/ â€?your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hermes chat -Q enables quiet mode for programmatic use â€?suppresses banner and spinner.",
    ...
    "BOOT.md at ~/.hermes/BOOT.md runs automatically on every gateway start â€?use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.hermes/scripts/ and run before the agent â€?perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity â€?rewrite it to make Hermes your own.",
    ...
    "Each profile gets its own subprocess HOME at HERMES_HOME/home/ â€?isolated git, ssh, npm, gh configs.",
    ...
    "BROWSER_CDP_URL connects browser tools to any running Chrome â€?accepts WebSocket, HTTP, or host:port.",
    ...
    "Profile names cannot collide with existing PATH binaries â€?'hermes profile create ls' would be rejected.",
    ...
    "Smart routing can auto-route simple queries to a cheaper model â€?set smart_model_routing.enabled: true.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn â€?including vars from ~/.hermes/.env.",
