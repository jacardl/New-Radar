...
Token type support (per GitHub docs):
  gho_          OAuth token           â? (default via copilot login)
  github_pat_   Fine-grained PAT      â? (needs Copilot Requests permission)
  ghu_          GitHub App token      â? (via environment variable)
  ghp_          Classic PAT           â? NOT SUPPORTED
...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  â?`copilot login` or `hermes model` to authenticate via OAuth\n"
...
def copilot_device_code_login(
    *,
    ...
    try:
        ...
        print(f"  â?Failed to start device authorization: {exc}")
        return None
    ...
    if not device_code or not user_code:
        print("  â?GitHub did not return a device code.")
        return None
    ...
    while time.time() < deadline:
        ...
        if error == "authorization_pending":
            ...
            print()
            print("  â?Device code expired. Please try again.")
            return None
        elif error == "access_denied":
            print()
            print("  â?Authorization was denied.")
            return None
        elif error:
            print()
            print(f"  â?Authorization failed: {error}")
            return None

    print()
    print("  â?Timed out waiting for authorization.")
    return None
