...
def cmd_whatsapp(args):
    ...
    # â”€â”€ Step 2: Enable WhatsApp â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if source in ("GITHUB_TOKEN", "GH_TOKEN"):
            print(f"  GitHub token: {api_key[:8]}... âœ?({source})")
    ...
    if live_models:
        ...
        if model_list:
            print("  âš?Could not auto-detect models from GitHub Copilot â€?showing defaults.")
...
def _model_flow_kimi(config, current_model=""):
    ...
    if not existing_key:
        ...
        print(f"  {pconfig.name} API key: {existing_key[:8]}... âœ?)
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if has_creds:
        # Show what we found
        if existing_key:
            print(f"  Anthropic credentials: {existing_key[:12]}... âœ?)
        elif cc_available:
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if r1.returncode != 0:
        print(f"  {'âœ? if fatal else 'âš?} Web UI npm install failed"
    ...
    if r2.returncode != 0:
        print(f"  {'âœ? if fatal else 'âš?} Web UI build failed"
              + ("" if fatal else " (hermes web will not be available)"))
        if fatal:
            print("  Run manually:  cd web && npm install && npm run build")
        return False
    print("  âœ?Web UI built")
    return True
...
def _update_via_zip(args):
    ...
    if removed:
        print(f"  âœ?Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
...
def _restore_stashed_changes(
    ...
    if stash_selector is None:
        print("âš?Local changes were restored, but Hermes couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("âš?Local changes were restored, but Hermes couldn't drop the saved stash entry.")
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  âœ?Added upstream: https://github.com/NousResearch/hermes-agent.git")
                has_upstream = True
            else:
    ...
    try:
        ...
        print("  âœ?Failed to pull from upstream. You may need to resolve conflicts manually.")
        return
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    try:
        ...
        print("  âš?Optional extras failed, reinstalling base dependencies and retrying extras individually...")
    ...
    if installed_extras:
        print(f"  âœ?Reinstalled optional extras individually: {', '.join(installed_extras)}")
    if failed_extras:
        print(f"  âš?Skipped optional extras that still failed: {', '.join(failed_extras)}")
    ...
    try:
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                print("  âš?Fast-forward not possible (history diverged), resetting to match remote...")
                ...
                for p in other_profiles:
                    try:
                        ...
                        if r:
                            ...
                            if updated: parts.append(f"â†‘{updated} updated")
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if check.stdout.strip() == "active":
                                ...
                                if restart.returncode == 0:
                                    ...
                                    print(f"  âš?Failed to restart {svc_name}: {restart.stderr.strip()}")
        ...
        try:
            ...
            if not clone_all:
                ...
                if result:
                    ...
                    print("âš?Skills could not be seeded. Run `{} update` to retry.".format(name))
