...
def main():
    ...
    if args.command == "add":
        ...
        if not issues:
            print(f"é?All {len(store.data['evidence'])} evidence entries passed SHA-256 integrity check.")
        else:
            print(f"é?{len(issues)} integrity issue(s) detected:")
            for i in issues:
