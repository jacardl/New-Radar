...
class BrowserUseProvider(CloudBrowserProvider):
    ...
    def close_session(self, session_id: str) -> bool:
        try:
            ...
            logger.warning("Cannot close Browser Use session %s â€?missing credentials", session_id)
            return False
    ...
    def emergency_cleanup(self, session_id: str) -> None:
        config = self._get_config_or_none()
        if config is None:
            logger.warning("Cannot emergency-cleanup Browser Use session %s â€?missing credentials", session_id)
            return
        try:
            requests.patch(
                f"{config['base_url']}/browsers/{session_id}",
