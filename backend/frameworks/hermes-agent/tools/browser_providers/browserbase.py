...
class BrowserbaseProvider(CloudBrowserProvider):
    ...
    def close_session(self, session_id: str) -> bool:
        try:
            ...
            logger.warning("Cannot close Browserbase session %s â€?missing credentials", session_id)
            return False
    ...
    def emergency_cleanup(self, session_id: str) -> None:
        config = self._get_config_or_none()
        if config is None:
            logger.warning("Cannot emergency-cleanup Browserbase session %s â€?missing credentials", session_id)
            return
        try:
            requests.post(
                f"{config['base_url']}/v1/sessions/{session_id}",
