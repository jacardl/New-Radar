...
class ProcessSession:
    ...
    # Watch patterns ‚Ä?trigger agent notification when output matches any pattern
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        # Notification queue ‚Ä?unified queue for all background process events.
    ...
    def _check_watch_patterns(self, session: ProcessSession, new_text: str) -> None:
        ...
        with session._lock:
            ...
            if session._watch_window_hits >= WATCH_MAX_PER_WINDOW:
                ...
                if session._watch_overload_since == 0.0:
                    ...
                    self.completion_queue.put({
                        ...
                        "message": (
                            f"Watch patterns disabled for process {session.id} ‚Ä?"
            ...
            # Under the rate limit ‚Ä?deliver notification
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        with the reader thread), the second call is a no-op ‚Ä?no duplicate
...
def _handle_process(args, **kw):
    ...
    # Coerce to string ‚Ä?some models send session_id as an integer
