"""
Checkpoint Manager â€?Transparent filesystem snapshots via shadow git repos.
...
Architecture:
    ~/.hermes/checkpoints/{sha256(abs_dir)[:16]}/   â€?shadow git repo
        HEAD, refs/, objects/                        â€?standard git internals
        HERMES_WORKDIR                               â€?original dir path
        info/exclude                                 â€?default excludes
...
# Valid git commit hash pattern: 4â€?0 hex chars (short or full SHA-1/SHA-256).
...
def _validate_file_path(file_path: str, working_dir: str) -> Optional[str]:
    ...
    # Reject absolute paths â€?restore targets must be relative to the workdir
...
class CheckpointManager:
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None) -> Dict:
        ...
        tracked files without moving HEAD â€?safe and reversible.
        ...
        # Restore â€?full directory or single file
    ...
    def get_working_dir_for_path(self, file_path: str) -> str:
        ...
        # No project root found â€?use the file's parent
    ...
    def _prune(self, shadow_repo: Path, working_dir: str) -> None:
        ...
        # For simplicity, we don't actually prune â€?git's pack mechanism
