...
def _reap_orphaned_browser_sessions():
    ...
    Called once on cleanup-thread startup â?not every 30 seconds â?to avoid
    ...
    for socket_dir in socket_dirs:
        ...
        if not os.path.isfile(pid_file):
            # No PID file â?just a stale dir, remove it
        ...
        # Daemon is alive and not tracked â?orphan. Kill it.
        try:
...
def _find_agent_browser() -> str:
    ...
    # Nothing found â?cache the failure so subsequent calls don't re-scan.
    _agent_browser_resolved = True
    raise FileNotFoundError(
