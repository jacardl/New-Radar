...
supports multiple backends ‚Ä?**Browser Use** (cloud, default for Nous
subscribers), **Browserbase** (cloud, direct credentials), and **local
Chromium** ‚Ä?with identical agent-facing behaviour.  The backend is
...
try:
    ...
    check_website_access = lambda url: None  # noqa: E731 ‚Ä?fail-open if policy module unavailable

try:
    from tools.url_safety import is_safe_url as _is_safe_url
except Exception:
    _is_safe_url = lambda url: False  # noqa: E731 ‚Ä?fail-closed: block all if safety module unavailable
...
def _get_vision_model() -> Optional[str]:
    """Model for browser_vision (screenshot analysis ‚Ä?multimodal)."""
    return os.getenv("AUXILIARY_VISION_MODEL", "").strip() or None


def _get_extraction_model() -> Optional[str]:
    """Model for page snapshot text summarization ‚Ä?same as web_extract."""
...
def _is_local_backend() -> bool:
    ...
    machine.  For local backends ‚Ä?Camofox, or the built-in headless
    Chromium without a cloud provider ‚Ä?the user already has full terminal
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-hermes_‚Ä¶`` the resulting socket path exceeds the
...
# async event loop ‚Ä?a SystemExit raised inside a key-binding callback
...
def _reap_orphaned_browser_sessions():
    ...
    Called once on cleanup-thread startup ‚Ä?not every 30 seconds ‚Ä?to avoid
    ...
    for socket_dir in socket_dirs:
        ...
        if not os.path.isfile(pid_file):
            # No PID file ‚Ä?just a stale dir, remove it
        ...
        try:
            ...
            # Alive but owned by someone else ‚Ä?leave it alone
            continue

        # Daemon is alive and not tracked ‚Ä?orphan. Kill it.
        try:
...
BROWSER_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "description": "Get a text-based snapshot of the current page's accessibility tree. Returns interactive elements with ref IDs (like @e1, @e2) for browser_click and browser_type. full=false (default): compact view with interactive elements. full=true: complete page content. Snapshots over 8000 chars are truncated or LLM-summarized. Requires browser_navigate first. Note: browser_navigate already returns a compact snapshot ‚Ä?use this to refresh after interactions that change the page, or with full=true for complete content.",
        "parameters": {
            "type": "object",
            "properties": {
                "full": {
    ...
    },
    {
        ...
        "description": "Get browser console output and JavaScript errors from the current page. Returns console.log/warn/error/info messages and uncaught JS exceptions. Use this to detect silent JavaScript errors, failed API calls, and application warnings. Requires browser_navigate to be called first. When 'expression' is provided, evaluates JavaScript in the page context and returns the result ‚Ä?use this for DOM inspection, reading page state, or extracting data programmatically.",
        "parameters": {
            "type": "object",
            "properties": {
                "clear": {
                ...
                "expression": {
                    "type": "string",
                    "description": "JavaScript expression to evaluate in the page context. Runs in the browser like DevTools console ‚Ä?full access to DOM, window, document. Return values are serialized to JSON. Example: 'document.title' or 'document.querySelectorAll(\"a\").length'"
                }
            },
            "required": []
        }
...
def _create_cdp_session(task_id: str, cdp_url: str) -> Dict[str, str]:
    ...
    logger.info("Created CDP browser session %s ‚Ü?%s for task %s",
...
def _find_agent_browser() -> str:
    ...
    # Nothing found ‚Ä?cache the failure so subsequent calls don't re-scan.
    _agent_browser_resolved = True
    raise FileNotFoundError(
...
def _run_browser_command(
    ...
    if session_info.get("cdp_url"):
        # Cloud mode ‚Ä?connect to remote Browserbase browser via CDP
        # IMPORTANT: Do NOT use --session with --cdp. In agent-browser >=0.13,
        # --session creates a local browser instance and silently ignores --cdp.
        backend_args = ["--cdp", session_info["cdp_url"]]
    else:
        # Local mode ‚Ä?launch a headless Chromium instance
    ...
    try:
        ...
        # Log stderr for diagnostics ‚Ä?use warning level on failure so it's visible
        ...
        # Empty output with rc=0 is a broken state ‚Ä?treat as failure rather
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    """
    # Secret exfiltration protection ‚Ä?block URLs that embed API keys or
    ...
    # SSRF protection ‚Ä?block private/internal addresses before navigating.
    ...
    # Website policy check ‚Ä?block before navigating
    ...
    # Camofox backend ‚Ä?delegate after safety checks pass
    if _is_camofox_mode():
    ...
    if result.get("success"):
        ...
        # Post-redirect SSRF check ‚Ä?if the browser followed a redirect to a
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        error_msg = str(e)
        # Graceful degradation ‚Ä?server may not support eval
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        # Keep the screenshot if it was captured successfully ‚Ä?the failure is
...
def cleanup_browser(task_id: Optional[str] = None) -> None:
    ...
    # Skip full close when managed persistence is enabled ‚Ä?the browser
...
def check_browser_requirements() -> bool:
    ...
    *and* the provider's required credentials must be present.
    ...
    """
    # Camofox backend ‚Ä?only needs the server URL, no agent-browser CLI
    if _is_camofox_mode():
        return True
...
if __name__ == "__main__":
    ...
    if check_browser_requirements():
        print("‚ú?All requirements met")
    else:
        print("‚ù?Missing requirements:")
        try:
            browser_cmd = _find_agent_browser()
            if _requires_real_termux_browser_install(browser_cmd):
    ...
    print("\nüìã Available Browser Tools:")
    for schema in BROWSER_TOOL_SCHEMAS:
        print(f"  üîπ {schema['name']}: {schema['description'][:60]}...")
