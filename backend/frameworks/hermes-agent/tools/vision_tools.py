...
# Resolution: config.yaml auxiliary.vision.download_timeout ‚Ü?env var ‚Ü?30s default.
...
# Hard limit for vision API payloads (20 MB) ‚Ä?matches the most restrictive
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    if estimated_b64 <= max_base64_bytes:
        # Small enough ‚Ä?just encode directly.
    ...
    try:
        ...
        logger.info("Pillow not installed ‚Ä?cannot auto-resize oversized image")
        if data_url is None:
            data_url = _image_to_base64_data_url(image_path, mime_type=mime_type)
    ...
    # For PNG, quality is irrelevant ‚Ä?only dimension reduction helps.
...
async def vision_analyze_tool(
    ...
    try:
        ...
        # Convert image to base64 ‚Ä?send at full resolution first.
        ...
        # Hard limit (20 MB) ‚Ä?no provider accepts payloads this large.
        ...
        # Extract the analysis ‚Ä?fall back to reasoning if content is empty
        ...
        # Detect vision capability errors ‚Ä?give the model a clear message
...
if __name__ == "__main__":
    ...
    """
    print("üëÅÔ∏?Vision Tools Module")
    print("=" * 40)
