...
def _build_orig_to_norm_map(original: str) -> List[int]:
    ...
    Because UNICODE_MAP replacements may expand characters (e.g. em-dash â†?'--',
    ellipsis â†?'...'), the normalised string can be longer than the original.
...
def _strategy_unicode_normalized(content: str, pattern: str) -> List[Tuple[int, int]]:
    ...
    ``_build_orig_to_norm_map`` â€?necessary because some UNICODE_MAP
    replacements expand a single character into multiple ASCII characters,
    making a naÃ¯ve position copy incorrect.
    """
    # Normalize both sides. Either the content or the pattern (or both) may
    # carry unicode variants â€?e.g. content has an em-dash that should match
...
def _strategy_block_anchor(content: str, pattern: str) -> List[Tuple[int, int]]:
    ...
    # Previous values (0.10 / 0.30) were dangerously loose â€?a 10% middle-section
