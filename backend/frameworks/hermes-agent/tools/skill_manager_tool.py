...
Directory layout for user skills:
    ...
    ‚îú‚îÄ‚îÄ my-skill/
    ‚î?  ‚îú‚îÄ‚îÄ SKILL.md
    ‚î?  ‚îú‚îÄ‚îÄ references/
    ‚î?  ‚îú‚îÄ‚îÄ templates/
    ‚î?  ‚îú‚îÄ‚îÄ scripts/
    ‚î?  ‚îî‚îÄ‚îÄ assets/
...
# Import security scanner ‚Ä?agent-created skills get the same scrutiny as
...
def _security_scan_skill(skill_dir: Path) -> Optional[str]:
    ...
    try:
        ...
        if allowed is None:
            # "ask" ‚Ä?allow but include the warning so the user sees the findings
            report = format_scan_report(result)
            logger.warning("Agent-created skill has security findings: %s", reason)
            # Don't block ‚Ä?return None to allow, but log the warning
            return None
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    # Security scan ‚Ä?roll back on block
...
def _patch_skill(
    ...
    # escape sequences, and block-anchor matching ‚Ä?saving the agent
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "memory ‚Ä?reusable approaches for recurring task types. "
        "New skills go to ~/.hermes/skills/; existing skills can be modified wherever they live.\n\n"
        "Actions: create (full SKILL.md + optional category), "
        "patch (old_string/new_string ‚Ä?preferred for fixes), "
        "edit (full SKILL.md rewrite ‚Ä?major overhauls only), "
