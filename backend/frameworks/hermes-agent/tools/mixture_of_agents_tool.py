...
async def _run_reference_model_safe(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not content:
                # Reasoning-only response ‚Ä?let the retry loop handle it
...
if __name__ == "__main__":
    ...
    if not api_available:
        print("‚ù?OPENROUTER_API_KEY environment variable not set")
        print("Please set your API key: export OPENROUTER_API_KEY='your-key-here'")
        print("Get API key at: https://openrouter.ai/")
        exit(1)
    else:
        print("‚ú?OpenRouter API key found")
    
    print("üõ†Ô∏? MoA tools ready for use!")
    ...
    print(f"  üå°Ô∏? Reference temperature: {config['reference_temperature']}")
    print(f"  üå°Ô∏? Aggregator temperature: {config['aggregator_temperature']}")
    print(f"  üõ°Ô∏? Failure tolerance: {config['failure_tolerance']}")
    print(f"  üìä Minimum successful models: {config['min_successful_references']}")
    
    # Show debug mode status
    if _debug.active:
        print(f"\nüêõ Debug mode ENABLED - Session ID: {_debug.session_id}")
...
MOA_SCHEMA = {
    ...
    "description": "Route a hard problem through multiple frontier LLMs collaboratively. Makes 5 API calls (4 reference models + 1 aggregator) with maximum reasoning effort ‚Ä?use sparingly for genuinely difficult problems. Best for: complex math, advanced algorithms, multi-step analytical reasoning, problems benefiting from diverse perspectives.",
    "parameters": {
        "type": "object",
        "properties": {
            "user_prompt": {
