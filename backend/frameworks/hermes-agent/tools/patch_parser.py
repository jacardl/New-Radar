...
def parse_v4a_patch(patch_content: str) -> Tuple[List[PatchOperation], Optional[str]]:
    ...
    if not operations:
        # Empty patch is not an error â€?callers get [] and can decide
...
def _validate_operations(
    ...
    """
    # Deferred import: breaks the patch_parser â†?fuzzy_match circular dependency
    ...
    for op in operations:
        if op.operation == OperationType.UPDATE:
            ...
            for hunk in op.hunks:
                ...
                if count == 0:
                    ...
                    errors.append(
                        ...
                        + (f" â€?{match_error}" if match_error else "")
                    )
                else:
                    # Advance simulation so subsequent hunks validate correctly.
                    # Reuse the result from the call above â€?no second fuzzy run.
            ...
            if not dst_result.error:
                errors.append(
                    f"{op.new_path}: destination already exists â€?move would overwrite"
                )
...
def apply_v4a_operations(operations: List[PatchOperation],
    ...
    if errors:
        return PatchResult(
            ...
            error="Apply phase failed (state may be inconsistent â€?run `git diff` to assess):\n"
...
def _apply_update(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    # Read current content â€?raw so no line-number prefixes or per-line truncation
    read_result = file_ops.read_file_raw(op.file_path)
    ...
    for hunk in op.hunks:
        ...
        if search_lines:
            ...
            if hunk.context_hint:
                ...
                if occurrences == 0:
                    # Hint not found â€?append at end as a safe fallback
                    new_content = new_content.rstrip('\n') + '\n' + insert_text + '\n'
                elif occurrences > 1:
                    return False, (
                        f"Addition-only hunk: context hint '{hunk.context_hint}' is ambiguous "
                        f"({occurrences} occurrences) â€?provide a more unique hint"
                    )
                else:
                    hint_pos = new_content.find(hunk.context_hint)
