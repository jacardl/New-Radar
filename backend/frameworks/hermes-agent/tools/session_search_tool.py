...
def _truncate_around_matches(
    ...
    if not match_positions:
        # Nothing at all â?take from the start
...
async def _summarize_session(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if content:
                return content
            # Reasoning-only / empty â?let the retry loop handle it
...
def session_search(
    ...
    # No LLM calls â?just DB queries for titles, previews, timestamps.
    ...
    try:
        ...
        # Resolve child sessions to their parent â?delegation stores detailed
        ...
        for result in raw_results:
            ...
            # Skip the current session lineage â?the agent already has that
        ...
        for (session_id, match_info, conversation_text, _), result in zip(tasks, results):
            ...
            if result:
                ...
                preview = (conversation_text[:500] + "\nâ¦[truncated]") if conversation_text else "No preview available."
                entry["summary"] = f"[Raw preview â?summarization unavailable]\n{preview}"
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "IMPORTANT: Use OR between keywords for best results â?FTS5 defaults to AND which misses "
    ...
    "parameters": {
        ...
        "properties": {
            "query": {
                "type": "string",
                "description": "Search query â?keywords, phrases, or boolean expressions to find in past sessions. Omit this parameter entirely to browse recent sessions instead (returns titles, previews, timestamps with no LLM cost).",
            },
            "role_filter": {
                "type": "string",
