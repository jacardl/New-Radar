...
async def web_extract_tool(
    ...
    try:
        ...
        # -- SSRF protection â?filter out private/internal URLs before any backend --
        safe_urls = []
        ...
        if not safe_urls:
            ...
            if backend == "parallel":
                ...
                for url in safe_urls:
                    ...
                    # Website policy check â?block before fetching
                    ...
                    try:
                        ...
                        try:
                            ...
                            results.append({
                                "url": url, "title": "", "content": "",
                                "error": "Scrape timed out after 60s â?page may be too large or unresponsive. Try browser_navigate instead.",
                            })
                            continue
...
async def web_crawl_tool(
    ...
    try:
        ...
        if backend == "tavily":
            ...
            # SSRF protection â?block private/internal addresses
            if not is_safe_url(url):
        ...
        # web_crawl requires Firecrawl or the Firecrawl tool-gateway â?Parallel has no crawl API
        ...
        # Website policy check â?block before crawling
        blocked = check_website_access(url)
        if blocked:
            logger.info("Blocked web_crawl for %s by rule %s", blocked["host"], blocked["rule"])
            return json.dumps({"results": [{"url": url, "title": "", "content": "", "error": blocked["message"],
...
if __name__ == "__main__":
    ...
    """
    print("ð Standalone Web Tools Module")
    print("=" * 40)
    ...
    if web_available:
        ...
        print("â?No web search backend configured")
        print(
    ...
    if not nous_available:
        print("â?No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Nous Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
        print("â ï¸  Without an auxiliary model, LLM content processing will be disabled")
    else:
        print(f"â?Auxiliary model available: {default_summarizer_model}")

    if not web_available:
        exit(1)

    print("ð ï¸? Web tools ready for use!")
    
    if nous_available:
        print(f"ð§  LLM content processing available with {default_summarizer_model}")
    ...
    if _debug.active:
        ...
        print("ð Debug mode disabled (set WEB_TOOLS_DEBUG=true to enable)")
    ...
    print("\nð Run 'python test_web_tools_llm.py' to test LLM processing capabilities")
