...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        while True:
            print()
            print(f"  â ï¸  DANGEROUS COMMAND: {description}")
            print(f"      {command}")
            print()
            if allow_permanent:
            ...
            if choice in ('o', 'once'):
                print("      â?Allowed once")
                return "once"
            elif choice in ('s', 'session'):
                print("      â?Allowed for this session")
                return "session"
            elif choice in ('a', 'always'):
                if not allow_permanent:
                    print("      â?Allowed for this session")
                    return "session"
                print("      â?Added to permanent allowlist")
                return "always"
            else:
                print("      â?Denied")
                return "deny"
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_gateway or os.getenv("HERMES_EXEC_ASK"):
        ...
        return {
            ...
            "description": description,
            "message": (
                f"â ï¸ This command is potentially dangerous ({description}). "
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_gateway or is_ask:
        ...
        return {
            ...
            "description": combined_desc,
            "message": (
                f"â ï¸ {combined_desc}. Asking the user for approval.\n\n**Command:**\n```\n{command}\n```"
            ),
        }
