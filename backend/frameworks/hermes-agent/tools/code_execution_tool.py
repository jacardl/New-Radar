...
# Resource limit defaults (overridable via config.yaml ‚Ü?code_execution.*)
...
def _ship_file_to_remote(env, remote_path: str, content: str) -> None:
    ...
    Uses ``echo ‚Ä?| base64 -d`` rather than stdin piping because some
...
def _execute_remote(
    ...
    if status == "timeout":
        ...
        result["output"] = (
            stdout_text + "\n[execution interrupted ‚Ä?user sent a new message]"
        )
    elif exit_code != 0:
        result["status"] = "error"
...
def execute_code(
    ...
    try:
        ...
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback ‚Ä?see lines above).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    the schema description should NOT mention web_search / web_extract ‚Ä?
    ...
    description = (
        ...
        "Also available (no import needed ‚Ä?built into hermes_tools):\n"
        "  json_parse(text: str) ‚Ä?json.loads with strict=False; use for terminal() output with control chars\n"
        "  shell_quote(s: str) ‚Ä?shlex.quote(); use when interpolating dynamic strings into shell commands\n"
        "  retry(fn, max_attempts=3, delay=2) ‚Ä?retry with exponential backoff for transient failures"
    )
