...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # Context exited cleanly â€?subprocess was terminated by the SDK.
        if new_pids:
            with _lock:
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    ...
    for server_name in server_names:
        ...
        if existing_ts and not str(existing_ts.get("description", "")).startswith("MCP server '"):
            logger.warning(
                "Skipping MCP toolset alias '%s' â€?a built-in toolset already uses that name",
                server_name,
            )
            continue
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``hermes tools`` interactive configuration â€?connects to each
...
def _kill_orphaned_mcp_children() -> None:
    ...
    Only kills PIDs tracked in ``_stdio_pids`` â€?never arbitrary children.
    """
