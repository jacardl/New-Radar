...
class MiniSWERunner:
    ...
    def __init__(
        ...
        print("ð¤ Mini-SWE Runner initialized")
    ...
    def _create_env(self):
        ...
        print(f"ð§ Creating {self.env_type} environment...")
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        """
        print(f"\n{'='*60}")
        print(f"ð Task: {task[:80]}{'...' if len(task) > 80 else ''}")
        print(f"{'='*60}")
...
Complete the user's task step by step."""
        ...
        try:
            while api_call_count < self.max_iterations:
                api_call_count += 1
                print(f"\nð API call #{api_call_count}/{self.max_iterations}")
                ...
                if assistant_message.content:
                    print(f"ð¤ Assistant: {assistant_message.content[:100]}...")
                
                # Check for tool calls
                if assistant_message.tool_calls:
                    print(f"ð§ Tool calls: {len(assistant_message.tool_calls)}")
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if "MINI_SWE_AGENT_FINAL_OUTPUT" in result["output"]:
                            print("   â?Task completion signal detected!")
                            completed = True
                        ...
                        print(f"   â?exit_code={result['exit_code']}, output={len(result['output'])} chars")
                    ...
                    completed = True
                    print("ð Agent finished (no more tool calls)")
                    break
    ...
    def run_batch(
        ...
        print(f"\nð¦ Running batch of {len(prompts)} tasks")
        print(f"ð Output: {output_file}")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            for i, prompt in enumerate(prompts, 1):
                print(f"\n{'='*60}")
                print(f"ð Task {i}/{len(prompts)}")
                print(f"{'='*60}")
                ...
                try:
                    ...
                    print(f"â?Task {i} completed (api_calls={result['api_calls']})")
        ...
        print(f"\nâ?Batch complete! {len(results)} trajectories saved to {output_file}")
        return results
...
def main(
    ...
    """
    print("ð Mini-SWE Runner with Hermes Trajectory Format")
    print("=" * 60)
    ...
    if task:
        ...
        print(f"\nð Trajectory saved to: {output_file}")
        print(f"â?Completed: {result['completed']}")
        print(f"ð API calls: {result['api_calls']}")
        print(f"ð¬ Turns: {len(result['conversations'])}")
        ...
        if not prompts:
            print(f"â?No prompts found in {prompts_file}")
            return
        
        runner.run_batch(prompts, output_file)
    
    else:
        print("â?Please provide either --task or --prompts_file")
