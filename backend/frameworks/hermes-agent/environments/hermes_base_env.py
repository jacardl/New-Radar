...
class HermesAgentBaseEnv(BaseEnv):
    ...
    def __init__(
        ...
        print(
            f"ðŸ–¥ï¸? Terminal: backend={config.terminal_backend}, "
        ...
        # for bidirectional tool call translation (raw text â†?OpenAI tool_calls).
        if hasattr(self.server, 'tool_parser'):
            self.server.tool_parser = config.tool_call_parser
            print(f"ðŸ”§ Tool parser: {config.tool_call_parser}")
