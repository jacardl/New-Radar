"""
WebResearchEnv â€?RL Environment for Multi-Step Web Research
============================================================

Trains models to do accurate, efficient, multi-source web research.

Reward signals:
  - Answer correctness  (LLM judge, 0.0â€?.0)
  - Source diversity    (used â‰? distinct domains)
  - Efficiency          (penalizes excessive tool calls)
  - Tool usage          (bonus for actually using web tools)

Dataset: FRAMES benchmark (Google, 2024) â€?multi-hop factual questions
...
Inspired by: GroceryMind â€?production Hermes agent doing live web research
...
SAMPLE_QUESTIONS = [
    ...
    },
    {
        ...
        "answer": "The Linux Foundation oversees Kubernetes. CNCF (Cloud Native Computing Foundation) is the specific body â€?it does not have a traditional CEO but has an executive director.",
        "difficulty": "medium",
        "hops": 2,
    },
    {
    ...
    },
    {
        ...
        "answer": "The European Central Bank sets rates for France/eurozone. The current rate should be verified â€?it has changed frequently in 2023-2025.",
        "difficulty": "hard",
        "hops": 2,
    },
    {
...
class WebResearchEnvConfig(HermesAgentEnvConfig):
    ...
    diversity_bonus: float = Field(
        default=0.1,
        description="Bonus reward for citing â‰? distinct domains.",
    )
    ...
    eval_split_ratio: float = Field(
        default=0.1,
        description="Fraction of dataset to hold out for evaluation (0.0â€?.0).",
    )
...
class WebResearchEnv(HermesAgentBaseEnv):
    ...
    Reward is multi-signal:
      60% â€?answer correctness (LLM judge)
      20% â€?tool usage (did the model actually search the web?)
      20% â€?efficiency (penalizes >5 tool calls)

    Bonus +0.1 for source diversity (â‰? distinct domains cited).
    """

    name = "web-research"
    env_config_cls = WebResearchEnvConfig

    # Default toolsets for this environment â€?web + file for saving notes
    ...
    # ------------------------------------------------------------------
    # 2. get_next_item â€?return the next question
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 3. format_prompt â€?build the user-facing prompt
    # ------------------------------------------------------------------
    ...
    def format_prompt(self, item: dict) -> str:
        ...
        return (
            ...
            f"You MUST search the web to find current, accurate information â€?"
    ...
    # ------------------------------------------------------------------
    # 4. compute_reward â€?multi-signal scoring
    # ------------------------------------------------------------------
    ...
    async def compute_reward(
        ...
        Multi-signal reward function:
          ...
          correctness_weight * correctness  â€?LLM judge comparing answer to ground truth
          tool_usage_weight  * tool_used    â€?binary: did the model use web tools?
          efficiency_weight  * efficiency   â€?penalizes wasteful tool usage
          + diversity_bonus                 â€?source diversity (â‰? distinct domains)
        """
        ...
        logger.debug(
            f"Reward breakdown â€?correctness={correctness:.2f}, "
            f"tool_used={tool_used:.1f}, efficiency={efficiency:.2f}, "
            f"diversity={diversity:.1f} â†?total={reward:.3f}"
        )

        return reward

    # ------------------------------------------------------------------
    # 5. evaluate â€?run on held-out eval split
    # ------------------------------------------------------------------

    async def evaluate(self, *args, **kwargs) -> None:
        """Run evaluation on the held-out split using the full agent loop with tools.

        Each eval item runs through the same agent loop as training â€?
        ...
        for i, item in enumerate(eval_items):
            ...
            try:
                ...
                logger.info(
                    f"  â†?correctness={correctness:.2f}, reward={reward:.3f}, "
        ...
        logger.info(
            f"Eval complete â€?correctness={eval_metrics['eval/mean_correctness']:.3f}, "
