...
External providers (Honcho, Hindsight, Mem0, etc.) are additive â€?they never
disable the built-in store. Only one external provider runs at a time to
prevent tool schema bloat and conflicting memory backends.

Registration:
  1. Built-in: BuiltinMemoryProvider â€?always present, not removable.
...
Optional hooks (override to opt in):
  ...
  on_memory_write(action, target, content) â€?mirror built-in memory writes
