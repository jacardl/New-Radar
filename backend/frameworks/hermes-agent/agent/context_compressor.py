...
class ContextCompressor(ContextEngine):
    ...
    def _generate_summary(self, turns_to_summarize: List[Dict[str, Any]], focus_topic: str = None) -> Optional[str]:
        ...
        _summarizer_preamble = (
            ...
            "Do NOT respond to any questions or requests in the conversation â€?"
...
## Progress
### Done
[Completed work â€?include specific file paths, commands run, results obtained]
### In Progress
...
## Resolved Questions
[Questions the user asked that were ALREADY answered â€?include the answer so the next assistant does not re-answer them]
...
## Remaining Work
[What remains to be done â€?framed as context, not instructions]
...
Target ~{summary_budget} tokens. Be specific â€?include file paths, command outputs, error messages, and concrete values rather than vague descriptions.
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail â€?exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None) -> List[Dict[str, Any]]:
        ...
        if not summary:
            if not self.quiet_mode:
                logger.warning("Summary generation failed â€?inserting static fallback context marker")
