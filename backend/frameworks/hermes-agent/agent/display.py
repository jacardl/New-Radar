...
# Diff colors ‚Ä?resolved lazily from the skin engine so they adapt
...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        ...
        # minus/plus use background colors ‚Ä?derive from ui_error/ui_ok
...
# Module-level helpers ‚Ä?each call resolves from the active skin lazily.
...
def get_tool_emoji(tool_name: str, default: str = "‚ö?) -> str:
    ...
    for raw_line in diff.splitlines():
        ...
        if raw_line.startswith("+++ "):
            ...
            if from_file or to_file:
                rendered.append(f"{_diff_file()}{from_file or 'a/?'} ‚Ü?{to_file or 'b/?'}{_ANSI_RESET}")
            continue
        if raw_line.startswith("@@"):
...
def _summarize_rendered_diff_sections(
    ...
    *,
    ...
    if omitted_files or omitted_lines:
        summary = f"‚Ä?omitted {omitted_lines} diff line(s)"
        if omitted_files:
...
class KawaiiSpinner:
    ...
    SPINNERS = {
        'dots': ['‚†?, '‚†?, '‚†?, '‚†?, '‚†?, '‚†?, '‚†?, '‚†?, '‚†?, '‚†?],
        'bounce': ['‚†?, '‚†?, '‚†?, '‚°Ä', '‚¢Ä', '‚†?, '‚†?, '‚†?],
        'grow': ['‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?, '‚ñ?],
        'arrows': ['‚Ü?, '‚Ü?, '‚Ü?, '‚Ü?, '‚Ü?, '‚Ü?, '‚Ü?, '‚Ü?],
        'star': ['‚ú?, '‚ú?, '‚ú?, '‚ú?, '‚ú?, '‚ú?, '‚ú?, '‚ú?],
        'moon': ['üåë', 'üåí', 'üåì', 'üåî', 'üåï', 'üåñ', 'üåó', 'üåò'],
        'pulse': ['‚ó?, '‚ó?, '‚ó?, '‚ó?, '‚ó?, '‚ó?],
        'brain': ['üß†', 'üí≠', 'üí°', '‚ú?, 'üí´', 'üåü', 'üí°', 'üí≠'],
        'sparkle': ['‚Å?, 'Àö', '*', '‚ú?, '‚ú?, '‚ú?, '*', 'Àö'],
    }

    KAWAII_WAITING = [
        "(ÔΩ°‚óï‚Äø‚óïÔΩ?", "(‚óï‚Äø‚óï‚ú?", "Ÿ©(‚óï‚Äø‚óïÔΩ?€∂", "(‚úø‚ó†‚Äø‚ó†)", "( Àò‚ñΩÀ?„Å?,
        "‚ô?¬¥Œµ` )", "(‚óï·¥ó‚óï‚úø)", "„É?Ôºæ‚àáÔº?", "(‚âß‚ó°‚â?", "(‚òÖœâ‚òÖ)",
    ]

    KAWAII_THINKING = [
        "(ÔΩ°‚Ä¢ÃÅÔ∏ø‚Ä¢ÃÄÔΩ?", "(‚óî_‚ó?", "(¬¨‚Äø¬?", "( ‚Ä¢_‚Ä?>‚åê‚ñ†-‚ñ?, "(‚åê‚ñ†_‚ñ?",
        "(¬¥ÔΩ•_ÔΩ•`)", "‚óâ_‚ó?, "(¬∞„É≠¬?", "( Àò‚å£À?‚ô?, "„É?>‚àÄ<‚ò?‚ò?,
        "Ÿ©(‡πë‚ùõ·¥ó‚ùõ‡π?€∂", "(‚äô_‚ä?", "(¬¨_¬¨)", "( Õ°¬∞ Õú ñ Õ°¬∞)", "‡≤†_‡≤?,
    ]
    ...
    def _write(self, text: str, end: str = '\n', flush: bool = False):
        ...
        it instead ‚Ä?allowing callers to silence the spinner with a no-op lambda.
        """
    ...
    def _is_patch_stdout_proxy(self) -> bool:
        ...
        the correct line ‚Ä?each spinner frame ends up on its own line.
    ...
    def _animate(self):
        ...
        # skip the animation entirely ‚Ä?it creates massive log bloat.
    ...
    def print_above(self, text: str):
        ...
        # codes when prompt_toolkit's patch_stdout is active ‚Ä?same approach
...
def get_cute_tool_message(
    ...
    def _wrap(line: str) -> str:
        ...
        if skin_prefix != "‚î?:
            line = line.replace("‚î?, skin_prefix, 1)
        if not is_failure:
            return line
        return f"{line}{failure_suffix}"

    if tool_name == "web_search":
        return _wrap(f"‚î?üîç search    {_trunc(args.get('query', ''), 42)}  {dur}")
    if tool_name == "web_extract":
        ...
        if urls:
            ...
            return _wrap(f"‚î?üìÑ fetch     {_trunc(domain, 35)}{extra}  {dur}")
        return _wrap(f"‚î?üìÑ fetch     pages  {dur}")
    if tool_name == "web_crawl":
        url = args.get("url", "")
        domain = url.replace("https://", "").replace("http://", "").split("/")[0]
        return _wrap(f"‚î?üï∏Ô∏? crawl     {_trunc(domain, 35)}  {dur}")
    if tool_name == "terminal":
        return _wrap(f"‚î?üíª $         {_trunc(args.get('command', ''), 42)}  {dur}")
    if tool_name == "process":
        action = args.get("action", "?")
        sid = args.get("session_id", "")[:12]
        labels = {"list": "ls processes", "poll": f"poll {sid}", "log": f"log {sid}",
                  "wait": f"wait {sid}", "kill": f"kill {sid}", "write": f"write {sid}", "submit": f"submit {sid}"}
        return _wrap(f"‚î?‚öôÔ∏è  proc      {labels.get(action, f'{action} {sid}')}  {dur}")
    if tool_name == "read_file":
        return _wrap(f"‚î?üìñ read      {_path(args.get('path', ''))}  {dur}")
    if tool_name == "write_file":
        return _wrap(f"‚î?‚úçÔ∏è  write     {_path(args.get('path', ''))}  {dur}")
    if tool_name == "patch":
        return _wrap(f"‚î?üîß patch     {_path(args.get('path', ''))}  {dur}")
    if tool_name == "search_files":
        pattern = _trunc(args.get("pattern", ""), 35)
        target = args.get("target", "content")
        verb = "find" if target == "files" else "grep"
        return _wrap(f"‚î?üîé {verb:9} {pattern}  {dur}")
    if tool_name == "browser_navigate":
        url = args.get("url", "")
        domain = url.replace("https://", "").replace("http://", "").split("/")[0]
        return _wrap(f"‚î?üåê navigate  {_trunc(domain, 35)}  {dur}")
    if tool_name == "browser_snapshot":
        mode = "full" if args.get("full") else "compact"
        return _wrap(f"‚î?üì∏ snapshot  {mode}  {dur}")
    if tool_name == "browser_click":
        return _wrap(f"‚î?üëÜ click     {args.get('ref', '?')}  {dur}")
    if tool_name == "browser_type":
        return _wrap(f"‚î?‚å®Ô∏è  type      \"{_trunc(args.get('text', ''), 30)}\"  {dur}")
    if tool_name == "browser_scroll":
        d = args.get("direction", "down")
        arrow = {"down": "‚Ü?, "up": "‚Ü?, "right": "‚Ü?, "left": "‚Ü?}.get(d, "‚Ü?)
        return _wrap(f"‚î?{arrow}  scroll    {d}  {dur}")
    if tool_name == "browser_back":
        return _wrap(f"‚î?‚óÄÔ∏? back      {dur}")
    if tool_name == "browser_press":
        return _wrap(f"‚î?‚å®Ô∏è  press     {args.get('key', '?')}  {dur}")
    if tool_name == "browser_get_images":
        return _wrap(f"‚î?üñºÔ∏? images    extracting  {dur}")
    if tool_name == "browser_vision":
        return _wrap(f"‚î?üëÅÔ∏? vision    analyzing page  {dur}")
    if tool_name == "todo":
        todos_arg = args.get("todos")
        merge = args.get("merge", False)
        if todos_arg is None:
            return _wrap(f"‚î?üìã plan      reading tasks  {dur}")
        elif merge:
            return _wrap(f"‚î?üìã plan      update {len(todos_arg)} task(s)  {dur}")
        else:
            return _wrap(f"‚î?üìã plan      {len(todos_arg)} task(s)  {dur}")
    if tool_name == "session_search":
        return _wrap(f"‚î?üîç recall    \"{_trunc(args.get('query', ''), 35)}\"  {dur}")
    if tool_name == "memory":
        action = args.get("action", "?")
        target = args.get("target", "")
        if action == "add":
            return _wrap(f"‚î?üß† memory    +{target}: \"{_trunc(args.get('content', ''), 30)}\"  {dur}")
        elif action == "replace":
            return _wrap(f"‚î?üß† memory    ~{target}: \"{_trunc(args.get('old_text', ''), 20)}\"  {dur}")
        elif action == "remove":
            return _wrap(f"‚î?üß† memory    -{target}: \"{_trunc(args.get('old_text', ''), 20)}\"  {dur}")
        return _wrap(f"‚î?üß† memory    {action}  {dur}")
    if tool_name == "skills_list":
        return _wrap(f"‚î?üìö skills    list {args.get('category', 'all')}  {dur}")
    if tool_name == "skill_view":
        return _wrap(f"‚î?üìö skill     {_trunc(args.get('name', ''), 30)}  {dur}")
    if tool_name == "image_generate":
        return _wrap(f"‚î?üé® create    {_trunc(args.get('prompt', ''), 35)}  {dur}")
    if tool_name == "text_to_speech":
        return _wrap(f"‚î?üîä speak     {_trunc(args.get('text', ''), 30)}  {dur}")
    if tool_name == "vision_analyze":
        return _wrap(f"‚î?üëÅÔ∏? vision    {_trunc(args.get('question', ''), 30)}  {dur}")
    if tool_name == "mixture_of_agents":
        return _wrap(f"‚î?üß† reason    {_trunc(args.get('user_prompt', ''), 30)}  {dur}")
    if tool_name == "send_message":
        return _wrap(f"‚î?üì® send      {args.get('target', '?')}: \"{_trunc(args.get('message', ''), 25)}\"  {dur}")
    if tool_name == "cronjob":
        ...
        if action == "create":
            ...
            return _wrap(f"‚î?‚è?cron      create {_trunc(label, 24)}  {dur}")
        if action == "list":
            return _wrap(f"‚î?‚è?cron      listing  {dur}")
        return _wrap(f"‚î?‚è?cron      {action} {args.get('job_id', '')}  {dur}")
    if tool_name.startswith("rl_"):
        rl = {
        ...
        return _wrap(f"‚î?üß™ rl        {rl.get(tool_name, tool_name.replace('rl_', ''))}  {dur}")
    if tool_name == "execute_code":
        code = args.get("code", "")
        first_line = code.strip().split("\n")[0] if code.strip() else ""
        return _wrap(f"‚î?üêç exec      {_trunc(first_line, 35)}  {dur}")
    if tool_name == "delegate_task":
        tasks = args.get("tasks")
        if tasks and isinstance(tasks, list):
            return _wrap(f"‚î?üîÄ delegate  {len(tasks)} parallel tasks  {dur}")
        return _wrap(f"‚î?üîÄ delegate  {_trunc(args.get('goal', ''), 35)}  {dur}")

    preview = build_tool_preview(tool_name, args) or ""
    return _wrap(f"‚î?‚ö?{tool_name[:9]:9} {_trunc(preview, 35)}  {dur}")
...
def format_context_pressure(
    ...
    Args:
        compaction_progress: How close to compaction (0.0‚Ä?.0, 1.0 = fires).
...
def format_context_pressure_gateway(
    ...
    No ANSI ‚Ä?just Unicode and plain text suitable for Telegram/Discord/etc.
    ...
    if compression_enabled:
        ...
        hint = "Auto-compaction is disabled ‚Ä?context may be truncated."
