"""Tests for cmd_update gateway auto-restart â€?systemd + launchd coverage.
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units hermes-gateway* â€?discover all gateway services
        ...
        # systemctl is-active â€?distinguish --user from system scope
        ...
        # systemctl restart â€?distinguish --user from system scope
...
# ---------------------------------------------------------------------------
# cmd_update â€?macOS launchd detection
# ---------------------------------------------------------------------------
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        # plist does NOT exist â€?no launchd service
        monkeypatch.setattr(
            gateway_cli, "get_launchd_plist_path", lambda: plist_path,
        )
...
# ---------------------------------------------------------------------------
# cmd_update â€?system-level systemd service detection
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Service PID exclusion â€?the core bug fix
# ---------------------------------------------------------------------------
...
class TestServicePidExclusion:
    ...
    def test_update_launchd_does_not_kill_service_pid(
        ...
        assert len(kill_calls) == 0, (
            f"Service PID {SERVICE_PID} was killed by the manual sweep â€?"
