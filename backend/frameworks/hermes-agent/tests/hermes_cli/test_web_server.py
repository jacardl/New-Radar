...
class TestReloadEnv:
    """Tests for reload_env() ‚Ä?re-reads .env into os.environ."""
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self):
        """Create a TestClient ‚Ä?import is deferred to avoid requiring fastapi."""
        try:
...
class TestBuildSchemaFromConfig:
    ...
    def test_no_single_field_categories(self):
        ...
        for cat, count in cats.items():
            assert count >= 2, f"Category '{cat}' has only {count} field(s) ‚Ä?should be merged"
...
class TestConfigRoundTrip:
    """Verify config survives GET ‚Ü?edit ‚Ü?PUT without data loss."""
    ...
    def test_schema_types_match_config_values(self):
        ...
        for key, entry in schema.items():
            ...
            if val is None:
                continue  # not set in user config ‚Ä?fine
            expected = entry["type"]
