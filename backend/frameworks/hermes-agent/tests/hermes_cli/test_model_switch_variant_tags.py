...
The fix: Step c now skips coloné«æ¯lash conversion when the model name already
...
# Shared mock context é¥?skip network calls, credential resolution, catalog lookups
...
class TestVariantTagPreservation:
    ...
    def test_legacy_colon_format_with_tag_converts_first_colon_only(self):
        """vendor:model:free (no slash) é«?vendor/model:free é¥?first colon becomes slash."""
