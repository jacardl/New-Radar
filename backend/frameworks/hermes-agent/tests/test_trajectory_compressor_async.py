...
instance attribute. When process_directory() calls asyncio.run() â€?which
creates and closes a fresh event loop â€?the client's internal httpx
...
class TestAsyncClientLazyCreation:
    """trajectory_compressor.py â€?_get_async_client()"""
...
class TestSourceLineVerification:
    ...
    def test_no_eager_async_openai_in_init(self):
        ...
        # should not exist â€?only self.async_client = None
        lines = src.split("\n")
