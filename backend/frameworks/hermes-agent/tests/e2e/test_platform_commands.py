...
Each test drives a message through the full async pipeline:
    adapter.handle_message(event)
        â†?BasePlatformAdapter._process_message_background()
        â†?GatewayRunner._handle_message() (command dispatch)
        â†?adapter.send() (captured for assertions)

No LLM involved â€?only gateway-level commands are tested.
...
class TestSendFailureResilience:
    ...
    async def test_send_failure_does_not_crash_pipeline(self, adapter, platform):
        ...
        event = make_event(platform, "/help")
        # Should not raise â€?pipeline handles send failures internally
        await adapter.handle_message(event)
        await asyncio.sleep(0.3)
