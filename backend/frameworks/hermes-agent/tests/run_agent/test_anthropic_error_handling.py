...
- 429 rate limit â†?retried with backoff
- 529 overloaded â†?retried with backoff
- 400 bad request â†?non-retryable, immediate fail
- 401 unauthorized â†?credential refresh + retry
- 500 server error â†?retried with backoff
- "prompt is too long" â†?context length error triggers compression
"""
...
def test_429_exhausts_all_retries_before_raising(monkeypatch):
    ...
    The agent no longer re-raises after exhausting retries â€?it returns a
...
def test_401_credential_refresh_recovers(monkeypatch):
    ...
    class _Auth401ThenSuccessAgent(run_agent.AIAgent):
        ...
        def run_conversation(self, user_message, conversation_history=None, task_id=None):
            ...
            self._interruptible_api_call = _fake_api_call
            # Also patch streaming path â€?run_conversation now prefers
...
def test_401_refresh_fails_is_non_retryable(monkeypatch):
    ...
    # 401 after failed refresh â†?non-retryable (falls through to is_client_error)
    assert result["api_calls"] == 1
