...
class TestAlignBoundaryBackward:
    ...
    def test_boundary_at_clean_position(self):
        """Boundary after a user message â€?no adjustment needed."""
        ...
        # Boundary at 7, messages[6] = user â€?no adjustment
    ...
    def test_boundary_after_assistant_with_tools(self):
        ...
        # Boundary at 4, messages[3] = assistant with tool_calls â†?pull back to 3
    ...
    def test_boundary_in_middle_of_tool_results(self):
        ...
        # Boundary at 8 â€?in middle of tool results. messages[7] = tool result.
    ...
    def test_boundary_at_last_tool_result(self):
        """Boundary right after last tool result â€?messages[idx-1] is tool."""
        comp = _make_compressor()
        messages = [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "hello"},
        ...
        # Boundary at 7 â€?messages[6] is last tool result.
        # Walk back: [6]=tool, [5]=tool, [4]=tool, [3]=assistant with tools â†?idx=3
        assert comp._align_boundary_backward(messages, 7) == 3

    def test_boundary_with_consecutive_tool_groups(self):
        """Two consecutive tool groups â€?only walk back to the nearest parent."""
        comp = _make_compressor()
        messages = [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "hello"},
        ...
        # Boundary at 7 â€?messages[6] = tool result for tc_2 group
        # Walk back: [6]=tool, [5]=assistant with tools â†?idx=5
...
class TestCompressionToolResultPreservation:
    ...
    def test_parallel_tool_results_not_lost(self):
        ...
        messages = [
            ...
            _tool_result("tc_G", "CRITICAL DATA in file G"),              # 11 â†?compress_end=15-4=11
        ...
        # Every tool result must have a parent â€?no orphans
        ...
        for msg in result:
            if msg.get("role") == "tool":
                assert msg["content"] != "[Result from earlier conversation â€?see context summary above]", \
                    f"Stub result found for {msg.get('tool_call_id')} â€?real result was lost"
