"""Tests for payload/context-length â†?compression retry logic in AIAgent.
...
class TestHTTP413Compression:
    ...
    def test_413_clears_conversation_history_on_persist(self, agent):
        ...
        is written to the new session â†?"Session found but has no messages" on resume.
        """
        err_413 = _make_413_error()
        ok_resp = _mock_response(content="OK", finish_reason="stop")
        agent.client.chat.completions.create.side_effect = [err_413, ok_resp]
    ...
    def test_400_context_length_triggers_compression(self, agent):
        ...
        immediately â€?now it correctly triggers compression+retry.
        """
        err_400 = Exception(
    ...
    def test_413_cannot_compress_further(self, agent):
        ...
        with (
            ...
            # Compression returns same number of messages â†?can't compress further
            mock_compress.return_value = (
                [{"role": "user", "content": "hello"}],
                "same prompt",
            )
...
class TestPreflightCompression:
    ...
    def test_preflight_compresses_oversized_history(self, agent):
        ...
        # (each message ~50 chars â‰?13 tokens, 40 messages â‰?520 tokens > 200 threshold)
        big_history = []
        for i in range(20):
    ...
    def test_no_preflight_when_under_threshold(self, agent):
        ...
        agent.compression_enabled = True
        # Large context â€?history easily fits
