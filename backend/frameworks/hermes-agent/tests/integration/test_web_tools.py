...
def print_section(text: str):
    ...
    print(f"\n{Colors.CYAN}{Colors.BOLD}é¦æ¶ {text}{Colors.ENDC}")
    print(f"{Colors.CYAN}{'-'*50}{Colors.ENDC}")


def print_success(text: str):
    """Print success message"""
    print(f"{Colors.GREEN}é?{text}{Colors.ENDC}")


def print_error(text: str):
    """Print error message"""
    print(f"{Colors.FAIL}é?{text}{Colors.ENDC}")


def print_warning(text: str):
    """Print warning message"""
    print(f"{Colors.WARNING}é¿çç¬  {text}{Colors.ENDC}")


def print_info(text: str, indent: int = 0):
    """Print info message"""
    indent_str = "  " * indent
    print(f"{indent_str}{Colors.BLUE}é©ç¸ç¬  {text}{Colors.ENDC}")
            ...
            try:
                ...
                for i, result in enumerate(web_results):
                    ...
                    if has_all_fields:
                        ...
                        if self.verbose:
                            print(f"    Result {i+1}: é?{result['title'][:50]}...")
                            print(f"      URL: {result['url'][:60]}...")
                    else:
                        missing = [f for f in required_fields if f not in result]
                        missing_fields.append(f"Result {i+1} missing: {missing}")
                        if self.verbose:
                            print(f"    Result {i+1}: é?Missing fields: {missing}")
            ...
            try:
                ...
                for i, result in enumerate(results):
                    ...
                    if error:
                        ...
                        if self.verbose:
                            print(f"    Page {i+1}: é?Error - {error}")
                    elif content:
                        ...
                        if self.verbose:
                            print(f"    Page {i+1}: é?{title[:50]}... - {content_len} characters")
                            print(f"      First 100 chars: {content[:100]}...")
                    else:
                        extraction_details.append(f"Page {i+1}: {title[:40]}... (EMPTY)")
                        if self.verbose:
                            print(f"    Page {i+1}: é¿?{title[:50]}... - Empty content")
