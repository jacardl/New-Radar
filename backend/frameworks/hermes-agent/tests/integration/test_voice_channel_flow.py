...
Does NOT require a Discord connection â€?tests the VoiceReceiver
...
class TestRealNaClDecrypt:
    """End-to-end: real NaCl encrypt â†?_on_packet decrypt â†?buffer."""

    def test_valid_encrypted_packet_buffered(self):
        """Real NaCl encrypted packet â†?decrypted â†?buffered."""
    ...
    def test_wrong_key_packet_dropped(self):
        """Packet encrypted with wrong key â†?NaCl fails â†?not buffered."""
    ...
    def test_bot_ssrc_ignored(self):
        """Packet from bot's own SSRC â†?ignored."""
        key = _make_secret_key()
    ...
    def test_multiple_packets_accumulate(self):
        """Multiple valid packets â†?buffer grows."""
        key = _make_secret_key()
        receiver = _make_voice_receiver(key)
    ...
    def test_different_ssrcs_separate_buffers(self):
        """Packets from different SSRCs â†?separate buffers."""
        key = _make_secret_key()
        receiver = _make_voice_receiver(key)
...
class TestRealNaClWithDAVE:
    ...
    def test_dave_unknown_ssrc_passthrough(self):
        """DAVE enabled but SSRC unknown â†?skip DAVE, buffer audio."""
        key = _make_secret_key()
    ...
    def test_dave_unencrypted_error_passthrough(self):
        """DAVE raises 'Unencrypted' â†?use NaCl-decrypted data as-is."""
        key = _make_secret_key()
        dave = MagicMock()
        ...
        # DAVE was called but failed â†?passthrough
        dave.decrypt.assert_called_once()
        assert 100 in receiver._buffers
        assert len(receiver._buffers[100]) > 0

    def test_dave_real_error_drops(self):
        """DAVE raises non-Unencrypted error â†?packet dropped."""
        key = _make_secret_key()
        dave = MagicMock()
...
class TestFullVoiceFlow:
    """End-to-end: encrypt â†?receive â†?buffer â†?silence detect â†?complete."""

    def test_single_utterance_flow(self):
        """Encrypt packets â†?buffer â†?silence â†?check_silence returns utterance."""
        key = _make_secret_key()
        receiver = _make_voice_receiver(key)
        receiver.map_ssrc(100, 42)
    ...
    def test_utterance_with_ssrc_automap(self):
        """No SPEAKING event â†?auto-map sole allowed user â†?utterance processed."""
        key = _make_secret_key()
        members = [
        ...
        # No map_ssrc call â€?simulating missing SPEAKING event
    ...
    def test_pause_blocks_during_playback(self):
        """Pause receiver â†?packets ignored â†?resume â†?packets accepted."""
        key = _make_secret_key()
        receiver = _make_voice_receiver(key)
    ...
    def test_corrupted_packet_ignored(self):
        """Corrupted/truncated packet â†?silently ignored."""
        key = _make_secret_key()
        receiver = _make_voice_receiver(key)
...
class TestAuthFiltering:
    ...
    def test_automap_rejects_unallowed_user(self):
        ...
        # No map_ssrc â€?SSRC unknown, auto-map should reject
...
class TestRejoinFlow:
    ...
    def test_rejoin_new_ssrc_works(self):
        """After rejoin, user may get new SSRC â€?still works."""
        key = _make_secret_key()
        receiver1 = _make_voice_receiver(key)
    ...
    def test_rejoin_without_speaking_event_automap(self):
        """Rejoin without SPEAKING event â€?auto-map sole allowed user."""
        key = _make_secret_key()
        members = [
            SimpleNamespace(id=9999, name="Bot"),
            SimpleNamespace(id=42, name="Alice"),
        ...
        # Rejoin â€?new key (Discord may assign new secret_key)
        new_key = _make_secret_key()
        receiver2 = _make_voice_receiver(
            new_key, allowed_user_ids={"42"}, members=members,
        )
        # No map_ssrc â€?simulating missing SPEAKING event
...
class TestEchoPreventionFlow:
    ...
    def test_audio_after_resume_processed(self):
        ...
        # Pause â†?send packets â†?resume â†?send more packets
        receiver.pause()
