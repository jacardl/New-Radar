...
def test_modal_requirements():
    ...
    print(f"  MODAL_TOKEN_ID env var: {'é?Set' if modal_token else 'é?Not set'}")
    print(f"  ~/.modal.toml file: {'é?Exists' if modal_toml.exists() else 'é?Not found'}")
    
    if config['env_type'] != 'modal':
        print(f"\né¿çç¬  TERMINAL_ENV is '{config['env_type']}', not 'modal'")
        print("   Set TERMINAL_ENV=modal in .env or export it to test Modal backend")
        return False
    
    requirements_met = check_terminal_requirements()
    print(f"\nRequirements check: {'é?Passed' if requirements_met else 'é?Failed'}")
...
def test_simple_command():
    ...
    print(f"\nTest: {'é?Passed' if success else 'é?Failed'}")
...
def test_environment_isolation():
    ...
    print(f"\nTest: {'é?Passed (environments isolated)' if isolated else 'é?Failed (environments NOT isolated)'}")
...
def main():
    ...
    print("é¦� Modal Terminal Tool Test Suite")
    print("=" * 60)
    ...
    if config['env_type'] != 'modal':
        print(f"\né¿çç¬  WARNING: TERMINAL_ENV is set to '{config['env_type']}', not 'modal'")
    ...
    if not results['requirements']:
        print("\né?Requirements not met. Cannot continue with other tests.")
        return
    ...
    for test_name, passed_test in results.items():
        status = "é?PASSED" if passed_test else "é?FAILED"
