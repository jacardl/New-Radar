...
class TestBuildChildProgressCallback:
    ...
    def test_gateway_batched_progress(self):
        ...
        # Send 4 tool calls â€?shouldn't flush yet (BATCH_SIZE = 5)
        for i in range(4):
    ...
    def test_parallel_callbacks_independent(self):
        ...
        # Send 3 calls to each â€?neither should flush (batch size = 5)
        for i in range(3):
            cb0(f"tool_{i}")
            cb1(f"other_{i}")
    ...
    def test_task_index_prefix_in_batch_mode(self):
        ...
        # task_index=0 in a batch of 3 â†?prefix "[1]"
        cb0 = _build_child_progress_callback(0, parent, task_count=3)
        cb0("web_search", "test")
        output = buf.getvalue()
        assert "[1]" in output

        # task_index=2 in a batch of 3 â†?prefix "[3]"
        buf.truncate(0)
        buf.seek(0)
