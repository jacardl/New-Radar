...
def project(tmp_path):
    ...
    # deep/nested/path/ â€?has .cursorrules
...
class TestPermissionErrorHandling:
    ...
    def test_check_tool_call_survives_inaccessible_path(self, project):
        ...
        with patch.object(Path, "is_dir", patched_is_dir):
            # Should not raise â€?gracefully skip the inaccessible directory
            result = tracker.check_tool_call(
                "read_file", {"path": str(project / "backend" / "src" / "main.py")}
            )
            # Result may be None (backend skipped) â€?the key point is no crash
