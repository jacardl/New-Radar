...
def _make_cli_stub():
    ...
    cli._command_spinner_frame = lambda: "‚ü?
    cli._tui_style_base = {
        "prompt": "#fff",
        "input-area": "#fff",
        "input-rule": "#aaa",
...
class TestCliSkinPromptIntegration:
    def test_default_prompt_fragments_use_default_symbol(self):
        ...
        assert cli._get_tui_prompt_fragments() == [("class:prompt", "‚ù?")]

    def test_ares_prompt_fragments_use_skin_symbol(self):
        cli = _make_cli_stub()

        set_active_skin("ares")
        assert cli._get_tui_prompt_fragments() == [("class:prompt", "‚ö?‚ù?")]
    ...
    def test_secret_prompt_fragments_preserve_secret_state(self):
        ...
        set_active_skin("ares")
        assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "üîë ‚ù?")]
    ...
    def test_narrow_terminals_compact_voice_prompt_fragments(self):
        ...
        with patch.object(HermesCLI, "_get_tui_terminal_width", return_value=50):
            assert cli._get_tui_prompt_fragments() == [("class:voice-prompt", "üé§ ")]
    ...
    def test_narrow_terminals_compact_voice_recording_prompt_fragments(self):
        ...
        assert frags[0][1].startswith("‚ó?)
        assert "‚ù? not in frags[0][1]

    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        cli = _make_cli_stub()
        cli._secret_state = {"response_queue": object()}

        with patch("hermes_cli.skin_engine.get_active_prompt_symbol", return_value="‚ö?"):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "üîë ‚ö?")]
