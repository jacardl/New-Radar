...
# ---------------------------------------------------------------------------
# _send_with_retry â€?success on first attempt
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _send_with_retry â€?network error with successful retry
# ---------------------------------------------------------------------------
...
class TestSendWithRetryNetworkRetry:
    ...
    async def test_timeout_not_retried_to_prevent_duplicates(self):
        ...
        the server â€?retrying a non-idempotent send risks duplicate delivery.
        ...
        # No retry, no fallback â€?timeout returns failure immediately
    ...
    async def test_connect_timeout_still_retried(self):
        """ConnectTimeout is safe to retry â€?the connection was never established."""
        adapter = _StubAdapter()
        adapter._send_results = [
...
# ---------------------------------------------------------------------------
# _send_with_retry â€?all retries exhausted â†?user notification
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _send_with_retry â€?non-network failure â†?plain-text fallback (no retry)
# ---------------------------------------------------------------------------
...
class TestSendWithRetryFallback:
    ...
    async def test_non_network_error_falls_back_immediately(self):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            result = await adapter._send_with_retry("chat1", "**bold**", max_retries=2, base_delay=0)
        # No sleep â€?no retry loop for non-network errors
        mock_sleep.assert_not_called()
        assert result.success
        assert len(adapter._send_calls) == 2
