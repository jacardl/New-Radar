...
class TestApplySessionModelOverride:
    ...
    def test_none_values_do_not_overwrite(self):
        ...
        assert rt["api_key"] == "ant-key"  # preserved â€?None didn't overwrite
    ...
    def test_empty_string_overwrites(self):
        """Empty string is not None â€?it should overwrite the config value."""
        runner = _make_runner()
        sk = build_session_key(_make_source())
    ...
    def test_different_session_key_not_affected(self):
        ...
        assert model == "anthropic/claude-sonnet-4"  # unchanged â€?wrong session key
