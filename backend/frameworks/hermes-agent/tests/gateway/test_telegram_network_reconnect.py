...
Specifically tests the fix for #3173 â€?when start_polling() fails after a
...
async def test_reconnect_self_schedules_on_start_polling_failure():
    ...
    schedule a new _handle_polling_network_error task â€?otherwise polling stays
    ...
    # Clean up â€?cancel the pending retry so it doesn't run after the test
    for t in pending:
        t.cancel()
        try:
            await t
...
async def test_reconnect_does_not_self_schedule_when_fatal_error_set():
    ...
    another retry task â€?the gateway is already shutting down this adapter.
    """
    adapter = _make_adapter()
    adapter._polling_network_error_count = 1
