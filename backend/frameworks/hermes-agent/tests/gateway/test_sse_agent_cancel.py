"""Tests for SSE client disconnect â†?agent task cancellation.
...
class TestSSEAgentCancelOnDisconnect:
    """gateway/platforms/api_server.py â€?_write_sse_chat_completion()"""
    ...
    def test_already_done_task_not_cancelled_on_disconnect(self):
        ...
        async def run():
            ...
            # Task was already done â€?should not be cancelled
            assert agent_task.done()
            assert not agent_task.cancelled()
    ...
    def test_agent_ref_none_still_cancels_task(self):
        ...
        on disconnect â€?just without the interrupt() call."""
        adapter = _make_adapter()
        ...
        async def run():
            ...
            with patch("gateway.platforms.api_server.web.StreamResponse",
                       return_value=mock_response):
                # No agent_ref passed â€?should still handle disconnect cleanly
                await adapter._write_sse_chat_completion(
