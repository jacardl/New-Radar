"""Tests for gateway.platforms.feishu é¥?Feishu scan-to-create registration."""
...
class TestQrRegister:
    ...
    # -- Contract: expected errors é«?None, unexpected errors é«?propagate --

    @patch("gateway.platforms.feishu._init_registration")
    def test_qr_register_returns_none_on_network_error(self, mock_init):
        """URLError (network down) is an expected failure é«?None."""
        from gateway.platforms.feishu import qr_register
        from urllib.error import URLError
    ...
    def test_qr_register_returns_none_on_json_error(self, mock_init):
        """Malformed server response is an expected failure é«?None."""
        from gateway.platforms.feishu import qr_register
    ...
    def test_qr_register_propagates_unexpected_errors(self, mock_init):
        """Bugs (e.g. AttributeError) must not be swallowed é¥?they propagate."""
        from gateway.platforms.feishu import qr_register
    ...
    def test_qr_register_returns_none_when_begin_missing_device_code(
        ...
        """Server returns begin response without device_code é«?RuntimeError é«?None."""
        from gateway.platforms.feishu import qr_register
    ...
    def test_qr_register_succeeds_even_when_probe_fails(
        ...
        """Registration succeeds but probe fails é«?result with bot_name=None."""
        from gateway.platforms.feishu import qr_register
