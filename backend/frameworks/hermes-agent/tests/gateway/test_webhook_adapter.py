...
class TestValidateSignature:
    ...
    def test_validate_no_signature_with_secret_rejects(self):
        """Secret configured but no recognised signature header â†?reject."""
        adapter = _make_adapter()
    ...
    def test_validate_no_secret_allows_all(self):
        ...
        assert not route_secret  # empty â†?validation is skipped in handler
...
class TestRenderPrompt:
    ...
    def test_render_prompt_no_template_dumps_json(self):
        """Empty template â†?JSON dump fallback with event/route context."""
        adapter = _make_adapter()
        payload = {"key": "value"}
...
class TestEventFilter:
    ...
    async def test_event_filter_empty_allows_all(self):
        """No events list â†?accept any event type."""
        routes = {
            "all": {
                "secret": _INSECURE_NO_AUTH,
                "prompt": "got it",
...
class TestHTTPHandling:
    ...
    async def test_connect_starts_server(self):
        ...
        # Use port 0 â€?the OS picks a free port, but aiohttp requires a real bind.
...
class TestInsecureNoAuth:
    ...
    async def test_insecure_no_auth_skips_validation(self):
        ...
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            # No signature header at all â€?should still be accepted
