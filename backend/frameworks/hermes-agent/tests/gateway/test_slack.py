...
# ---------------------------------------------------------------------------
# TestSendTyping ‚Ä?assistant.threads.setStatus
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TestFormatMessage ‚Ä?Markdown ‚Ü?mrkdwn conversion
# ---------------------------------------------------------------------------
...
class TestFormatMessage:
    ...
    def test_message_only_code_block(self, adapter):
        """Entire message is a fenced code block ‚Ä?no conversion."""
        code = "```python\nx = 1\n```"
        assert adapter.format_message(code) == code
    ...
    def test_nested_bold_in_link(self, adapter):
        """Bold inside link label ‚Ä?label is stashed before bold pass."""
...
class TestEditMessageStreamingPipeline:
    ...
    async def test_edit_message_formats_streaming_updates(self, adapter):
        ...
        # Second streaming update ‚Ä?bold + link
    ...
    async def test_edit_message_formats_code_and_bold(self, adapter):
        """Streaming update with code block and bold ‚Ä?code must be preserved."""
        adapter._app.client.chat_update = AsyncMock(return_value={"ok": True})
    ...
    async def test_edit_message_formats_blockquote_in_stream(self, adapter):
        """Streaming update with blockquote ‚Ä?'>' marker must survive."""
        adapter._app.client.chat_update = AsyncMock(return_value={"ok": True})
...
class TestReactions:
    ...
    async def test_reactions_in_message_flow(self, adapter):
        ...
        # Should have added üëÄ, then removed üëÄ, then added ‚ú?
...
# ---------------------------------------------------------------------------
# TestSlashCommands ‚Ä?expanded command set
# ---------------------------------------------------------------------------
...
class TestMessageSplitting:
    ...
    async def test_send_preserves_blockquote_formatting(self, adapter):
        """Blockquote '>' markers must survive format ‚Ü?chunk ‚Ü?send pipeline."""
...
class TestProgressMessageThread:
    ...
    async def test_dm_toplevel_progress_uses_message_ts_as_thread(self, adapter):
        ...
        event = {
            ...
            "text": "Hello bot",
            "ts": "1234567890.000001",
            # No thread_ts ‚Ä?this is a top-level DM
        }
        ...
        # The message_id should be the event's ts ‚Ä?this is what the gateway
    ...
    async def test_channel_mention_progress_uses_thread_ts(self, adapter):
        ...
        event = {
            ...
            # No thread_ts ‚Ä?top-level channel message
        }
