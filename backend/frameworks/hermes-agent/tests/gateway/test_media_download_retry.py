...
All async tests use asyncio.run() directly â€?pytest-asyncio is not installed
in this environment.
"""
...
class TestMattermostSendUrlAsFile:
    ...
    def test_success_on_first_attempt(self, _mock_safe):
        """200 on first attempt â†?file uploaded and post created."""
    ...
    def test_non_retryable_404_falls_back_immediately(self, _mock_safe):
        ...
        adapter.send.assert_called_once()
        # No sleep â€?fell back on first attempt
        mock_sleep.assert_not_called()
