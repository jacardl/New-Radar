...
async def test_restart_command_while_busy_requests_drain_without_interrupt(monkeypatch):
    # Ensure INVOCATION_ID is NOT set ‚Ä?systemd sets this in service mode,
    ...
    assert result == "‚è?Draining 1 active agent(s) before restart..."
...
async def test_draining_rejects_new_session_messages():
    ...
    assert result == "‚è?Gateway is restarting and is not accepting new work right now."
