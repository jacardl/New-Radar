...
class TestRunAsyncLoopLifecycle:
    ...
    def test_loop_not_closed_after_run_async(self):
        ...
        assert not loop.is_closed(), (
            "_run_async() closed the event loop â€?cached async clients will "
    ...
    def test_same_loop_reused_across_calls(self):
        ...
        assert loop1 is loop2, (
            "_run_async() created a new loop on the second call â€?cached "
...
class TestRunAsyncWorkerThread:
    ...
    def test_worker_thread_loop_not_closed(self):
        ...
        assert still_open, (
            "Worker thread's event loop was closed after _run_async â€?"
    ...
    def test_worker_thread_reuses_loop_across_calls(self):
        ...
        assert loop1 is loop2, (
            "Worker thread created different loops for consecutive calls â€?"
    ...
    def test_parallel_workers_get_separate_loops(self):
        ...
        assert len(loop_ids) == 3, (
            ...
            f"got {len(loop_ids)} â€?workers may be contending on a shared loop"
        )
    ...
    def test_worker_loop_separate_from_main_loop(self):
        ...
        assert worker_loop_id != id(main_loop), (
            "Worker thread used the main thread's loop â€?this would cause "
...
class TestVisionDispatchLoopSafety:
    ...
    verify the event loop stays alive afterwards â€?the exact scenario
    from issue #2104."""
    ...
    def test_vision_dispatch_keeps_loop_alive(self, tmp_path):
        ...
        loop = _get_tool_loop()
        assert not loop.is_closed(), (
            "Event loop closed after vision_analyze dispatch â€?cached async "
