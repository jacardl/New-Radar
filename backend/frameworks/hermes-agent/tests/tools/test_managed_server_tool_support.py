...
class TestManagedServerAPI:
    ...
    def test_managed_server_init_signature(self):
        ...
        # tool_call_parser â€?required for tool_call_support branch
        ...
        if not has_tool_parser:
            pytest.skip(
                "ManagedServer does not have tool_call_parser param â€?"
    ...
    def test_server_manager_managed_server_signature(self):
        ...
        if not has_tool_parser:
            pytest.skip(
                "ServerManager.managed_server() does not have tool_call_parser param â€?"
                "baseline atroposlib (pre tool_call_support branch)"
            )
    ...
    def test_managed_server_chat_template_kwargs(self):
        ...
        if not hasattr(ManagedServer, "CHAT_TEMPLATE_KWARGS"):
            pytest.skip(
                "ManagedServer does not have CHAT_TEMPLATE_KWARGS â€?"
                "baseline atroposlib (pre tool_call_support branch)"
            )
    ...
    def test_no_get_logprobs_method(self):
        ...
        # We just note the state â€?not a hard fail either way.
        has_get_logprobs = hasattr(ManagedServer, "get_logprobs")
        if has_get_logprobs:
            pytest.skip(
                "ManagedServer still has get_logprobs â€?baseline atroposlib"
            )
...
class TestParserCompatibility:
    ...
    def test_parser_no_tools_returns_none(self):
        """ManagedServer checks `if parsed_tool_calls:` â€?None should be falsy."""
        from environments.tool_call_parsers import get_parser
    ...
    def test_parser_content_is_string_or_none(self):
        """ManagedServer uses `parsed_content or ""` â€?must be str or None."""
        from environments.tool_call_parsers import get_parser
