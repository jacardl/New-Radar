...
class TestAdditionOnlyHunks:
    """Regression tests for #3081 â€?addition-only hunks were silently dropped."""
...
class TestReadFileRaw:
    """Bug 1 regression tests â€?files > 2000 lines and lines > 2000 chars."""
...
class TestValidationPhase:
    """Bug 2 regression tests â€?validation prevents partial apply."""
...
*** End Patch"""
        ...
        result = apply_v4a_operations(ops, FakeFileOps())
        assert result.success is True
        # unified_diff produces nothing for two empty inputs â€?fallback comment expected
...
class TestParseErrorSignalling:
    """Bug 3 regression tests â€?parse_v4a_patch must signal errors, not swallow them."""
