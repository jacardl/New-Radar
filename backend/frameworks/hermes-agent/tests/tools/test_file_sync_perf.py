...
class TestLocalPerf:
    """Local baseline â€?no file sync, no network. Sets the floor."""
...
@pytest.mark.ssh
class TestSSHPerf:
    """SSH with FileSyncManager â€?mtime skip should make sync ~0ms."""

    def test_echo_latency(self, ssh_env):
        """Sequential echo commands â€?measures per-command overhead including sync check."""
    ...
    def test_no_sync_within_interval(self, ssh_env):
        """Rapid sequential commands within 5s window â€?no sync at all."""
        # First command triggers sync
        ssh_env.execute("echo prime", timeout=10)

        # Immediately run 10 more â€?all within rate-limit window
