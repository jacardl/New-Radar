"""Tests for zombie process cleanup â€?verifies processes spawned by tools
...
class TestZombieReproduction:
    ...
    def test_orphaned_processes_survive_without_cleanup(self):
        ...
        them â€?this models the gap that causes zombie accumulation when
        ...
        try:
            ...
            for pid in pids:
                assert _pid_alive(pid), (
                    f"PID {pid} died after ref drop â€?"
...
class TestGatewayCleanupWiring:
    ...
    def test_evict_does_not_call_close(self):
        """_evict_cached_agent() should NOT call close() â€?it's also used
