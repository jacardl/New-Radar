...
该模块串联模板选择、布局设计、章节生成、IR装订与HTML渲染�?
...
class FileCountBaseline:
    ...
    该工具用于：
    - 在任务启动时记录 Insight/Media/Query 三个引擎导出�?Markdown 数量�?
    - 在后续轮询中快速判断是否有新报告落地；
    - �?Flask 层提供“输入是否准备完毕”的依据�?
    """
    ...
    def __init__(self):
        ...
        �?`logs/report_baseline.json` 不存在则会自动创建一份空快照�?
    ...
    def initialize_baseline(self, directories: Dict[str, str]) -> Dict[str, int]:
        ...
        遍历每个引擎目录并统�?`.md` 文件数量，将结果持久化为
        初始基准。后�?`check_new_files` 会据此对比增量�?
        """
        current_counts = {}
...
class ReportAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        Args:
            config: 配置对象，如果不提供则自动加�?
        
        步骤概览�?
            1. 解析配置并接入日�?LLM/渲染等核心组件；
    ...
    def _setup_logging(self):
        ...
        - 确保日志目录存在�?
        - 使用独立�?loguru sink 写入 Report Engine 专属 log 文件�?
          避免与其他子系统混淆�?
        - 【修复】配置实时日志写入，禁用缓冲，确保前端实时看到日�?
        - 【修复】防止重复添加handler
        """
        # 确保日志目录存在
        ...
        def _exclude_other_engines(record):
            """
            过滤掉其他引�?Insight/Media/Query/Forum)产生的日志，其余日志全部保留�?
        ...
        if not handler_exists:
            # 【修复】创建专用的logger，配置实时写�?
            # - enqueue=False: 禁用异步队列，立即写�?
            # - buffering=1: 行缓冲，每条日志立即刷新到文�?
    ...
    def _initialize_file_baseline(self):
        ...
        �?Insight/Media/Query 三个目录传入 `FileCountBaseline`�?
    ...
    def _initialize_llm(self) -> LLMClient:
        ...
        利用配置中的 API Key / 模型 / Base URL 构建统一�?
    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
        ...
        try:
            ...
            layout_design = self._run_stage_with_retry(
                ...
                # toc 字段已被 tocPlan 取代，这里按最新Schema挑�?校验
            ...
            emit('progress', {'progress': 20, 'message': '章节字数规划已生�?})
            # 记录每个章节的目标字�?强调点，后续传给章节LLM
            chapter_targets = {
            ...
            for section in sections:
                ...
                # 章节流式回调：把LLM返回的delta透传给SSE，便于前端实时渲�?
                ...
                while attempt <= chapter_max_attempts:
                    try:
                        ...
                        if will_fallback:
                            logger.warning(
                                "章节 {title} 达到最大尝试次数，保留字数最多（�?{score} 字）的版本作为兜底输�?,
                                title=section.title,
                        ...
                        # 捕获�?JSON 结构异常导致的运行时错误，包装为可重试异�?
                        # 包括�?
                        # - AttributeError: �?list.get() 调用失败
                        # - TypeError: 类型不匹�?
                        # - KeyError: 字典键缺�?
                        # - IndexError: 列表索引越界
                        # - ValueError: 值错误（�?LLM 返回空内容、缺少必要字段）
                        # - json.JSONDecodeError: JSON 解析失败（未被内部捕获的情况�?
                        error_type = type(structure_error).__name__
                        logger.warning(
                            "章节 {title} 生成过程中发�?{error_type}（第 {attempt}/{total} 次尝试），将尝试重新生成: {error}",
                            title=section.title,
                        ...
                        if attempt >= chapter_max_attempts:
                            # 达到最大重试次数，包装�?ChapterJsonParseError 抛出
                            raise ChapterJsonParseError(
                                f"{section.title} 章节�?{error_type} �?{chapter_max_attempts} 次尝试后仍无法生�? {structure_error}"
                        ...
                        if not self._should_retry_inappropriate_content_error(chapter_error):
                            raise
                        logger.warning(
                            "章节 {title} 触发内容安全限制（第 {attempt}/{total} 次尝试），准备重新生�? {error}",
                            title=section.title,
                            attempt=attempt,
                            total=chapter_max_attempts,
                            error=chapter_error,
                ...
                if chapter_payload is None:
                    raise ChapterJsonParseError(
                        f"{section.title} 章节JSON�?{chapter_max_attempts} 次尝试后仍无法解�?
                    )
            ...
            # --- 执行交叉验证与置信度评分 ---
            emit('progress', {'progress': 95, 'message': '正在执行交叉验证与事实核�?})
            ...
            logger.exception(f"报告生成过程中发生错�? {str(e)}")
    ...
    def _select_template(self, query: str, reports: List[Any], forum_logs: str, custom_template: str):
        ...
        优先使用用户指定的模板；否则将查询、三引擎报告与论坛日�?
        作为上下文交�?TemplateSelectionNode，由 LLM 返回最契合�?
        ...
        返回:
            dict: 包含 `template_name`、`template_content` �?`selection_reason` 的结构化结果，供后续节点消费�?
        """
        logger.info("选择报告模板...")
        
        # 如果用户提供了自定义模板，直接使�?
        if custom_template:
            logger.info("使用用户自定义模�?)
            return {
    ...
    def _extract_chapter_memory(self, chapter_payload: Dict[str, Any]) -> str:
        ...
        for block in blocks:
            ...
            if b_type == "paragraph":
                ...
                if len(text) > 100:
                    memory_parts.append(text[:100] + "...") # 取段落前100�?
                else:
    ...
    def _slice_template(self, template_markdown: str) -> List[TemplateSection]:
        ...
        委托 `parse_template_sections` 将Markdown标题/编号解析�?
        ...
        if sections:
            return sections
        logger.warning("模板未解析出章节，使用默认章节骨�?)
    ...
    def _build_generation_context(
        ...
        一次性整合为 `generation_context`，后续每章调�?LLM �?
        直接复用，确保所有章节共享一致的语调和视觉约束�?

        参数:
            query: 用户查询词�?
            reports: 归一化后�?query/media/insight 报告映射�?
            forum_logs: 三引擎讨论记录�?
            template_result: 模板节点返回的模板元信息�?
            layout_design: 文档布局节点产出的标�?目录/主题设计�?
    ...
    def _normalize_reports(self, reports: List[Any]) -> Dict[str, str]:
        ...
        约定顺序�?Query/Media/Insight，引擎提供的对象可能�?
        字典或自定义类型，因此统一�?`_stringify` 做容错�?
    ...
    def _should_retry_inappropriate_content_error(self, error: Exception) -> bool:
        """
        判断LLM异常是否由内容安�?不当内容导致�?

        当检测到供应商返回的错误包含特定关键词时，允许章节生�?
        重新尝试，以便绕过偶发的内容审查触发�?
    ...
    def _run_stage_with_retry(
        ...
        for attempt in range(1, self._STRUCTURAL_RETRY_ATTEMPTS + 1):
            try:
                ...
                last_error = exc
                logger.warning(
                    "{stage} 输出结构异常（第 {attempt}/{total} 次），将尝试修复或重�? {error}",
                    stage=stage_name,
                    attempt=attempt,
    ...
    def _ensure_mapping(
        ...
        if isinstance(value, list):
            ...
            if candidates:
                ...
                if expected_keys:
                    ...
                    best = candidates[0]
                logger.warning(
                    "{context} 返回列表，已自动提取包含最多预期键的元素继续执�?,
                    context=context,
                )
                return best
        ...
        if value is None:
            raise StageOutputFormatError(f"{context} 返回空结�?)

        raise StageOutputFormatError(
            f"{context} 返回类型 {type(value).__name__}，期望字�?
        )

    def _normalize_word_plan(self, word_plan: Dict[str, Any], stage_name: str) -> Dict[str, Any]:
        """
        清洗篇幅规划结果，确�?chapters/globalGuidelines/totalWords 类型安全�?
        """
        ...
        for idx, entry in enumerate(chapters_iterable):
            ...
            if isinstance(entry, list):
                ...
                if dict_candidate:
                    logger.warning(
                        "{stage} �?{idx} 个章节条目为列表，已提取首个对象用于后续流程",
                        stage=stage_name,
                        idx=idx + 1,
                    )
                    ...
                    continue
            logger.warning(
                "{stage} 跳过无法解析的章节条�?{idx}（类�? {type_name}�?,
                stage=stage_name,
                idx=idx + 1,
        ...
        if not isinstance(guidelines, list):
            if guidelines is None or guidelines == "":
                ...
                logger.warning(
                    "{stage} globalGuidelines 类型异常，已转换为列表封�?,
                    stage=stage_name,
                )
        ...
        if not isinstance(word_plan.get("totalWords"), (int, float)):
            logger.warning(
                "{stage} totalWords 类型异常，使用默认�?10000",
                stage=stage_name,
            )
    ...
    def _stringify(self, value: Any) -> str:
        ...
        - dict/list 统一序列化为格式�?JSON，便于提示词消费�?
        - 其他类型�?`str()`，None 则返回空串，避免 None 传播�?
...
"""
    ...
    def _save_report(self, html_content: str, document_ir: Dict[str, Any], report_id: str) -> Dict[str, Any]:
        ...
        # 在保存新文件之前，删除该 task_id 对应的所有旧报告文件（防�?query 变更导致残留�?
        try:
        ...
        logger.info(f"HTML报告已保�? {html_path}")
        logger.info(f"Document IR已保�? {ir_path}")
    ...
    def _save_document_ir(self, document_ir: Dict[str, Any], query_safe: str, timestamp: str) -> Path:
        ...
        `Document IR` �?HTML 解耦保存，便于调试渲染差异以及
        在不重新�?LLM 的情况下再次渲染或导出其他格式�?
    ...
    def check_input_files(self, insight_dir: str, media_dir: str, query_dir: str, forum_log_path: str, task_id: str = None) -> Dict[str, Any]:
        ...
        如果提供�?task_id，则检查各个目录下是否存在包含�?task_id 的文件（用于历史报告重新生成）�?
        ...
        if task_id:
            ...
            for engine, directory in directories.items():
                ...
                if matching_files:
                    ...
                    ready = False
                    missing_files.append(f"{engine} 引擎目录中未找到任务 {task_id} 的报�?)
        ...
        for engine, new_count in check_result['new_files_found'].items():
            ...
            if new_count > 0:
                result['files_found'].append(f"{engine}: {current_count}个文�?(新增{new_count}�?")
            else:
                result['missing_files'].append(f"{engine}: {current_count}个文�?(基准{baseline_count}个，无新�?")
        
        # 检查论坛日�?
        if forum_ready:
            result['files_found'].append(f"forum: {os.path.basename(forum_log_path)}")
        else:
            result['missing_files'].append("forum: 日志文件不存�?)
    ...
    def load_input_files(self, file_paths: Dict[str, str]) -> Dict[str, Any]:
        ...
        Returns:
            加载的内容字典，包含 `reports` 列表�?`forum_logs` 字符�?
        """
        content = {
            'reports': [],
            'forum_logs': ''
        ...
        for engine in engines:
            if engine in file_paths:
                try:
                    ...
                    logger.info(f"已加�?{engine} 报告: {len(report_content)} 字符")
                except Exception as e:
        ...
        if 'forum' in file_paths:
            try:
                ...
                logger.info(f"已加载论坛日�? {len(content['forum_logs'])} 字符")
            except Exception as e:
...
def create_agent(config_file: Optional[str] = None) -> ReportAgent:
    ...
    目前以环境变量驱�?`Settings`，保�?`config_file` 参数便于未来扩展�?
    """
