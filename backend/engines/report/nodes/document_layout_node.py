...
class DocumentLayoutNode(BaseNode):
    ...
    def run(
        ...
        # 将模板原文、切片结构与多源报告一并喂给LLM，便于其理解层级与素�?
        payload = {
            "query": query,
            "template": {
                "raw": template_markdown[:5000],  # 模板也稍微截�?
            ...
            "pruning_instruction": (
                "【强制动态大纲修剪】当前输入数据量极少，事实密度过低。你必须主动砍掉 PEST、SWOT、定量图表等深水区章节，将报告降级为一份简报。请�?tocPlan 中移除这些章节，绝对不要强�?LLM 去填模板编造内容�? 
    ...
    def _parse_response(self, raw: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(result.get("title"), str):
                logger.warning("文档设计缺少title字段或类型错误，使用默认�?)
                result.setdefault("title", "未命名报�?)

            # 处理tocPlan字段
            toc_plan = result.get("tocPlan", [])
            if not isinstance(toc_plan, list):
                logger.warning("文档设计缺少tocPlan字段或类型错误，使用空列�?)
                result["tocPlan"] = []
            else:
            ...
            if not isinstance(result.get("hero"), dict):
                logger.warning("文档设计缺少hero字段或类型错误，使用空对�?)
        ...
        for entry in toc_plan:
            ...
            if "description" in entry:
                ...
                if cleaned_desc != original_desc:
                    logger.warning(
                        f"清理目录�?'{entry.get('display', 'unknown')}' 的description字段中的JSON片段:\n"
                        f"  原文: {original_desc[:100]}...\n"
                        f"  清理�? {cleaned_desc[:100]}..."
                    )
