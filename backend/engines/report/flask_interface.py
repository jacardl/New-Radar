...
# ====== 流式推送与任务历史管理 ======
# 通过有界deque缓存最近的事件，方便SSE断线后快速补�?
MAX_TASK_HISTORY = 5
STREAM_HEARTBEAT_INTERVAL = 15  # 心跳间隔�?
...
def _register_stream(task_id: str) -> Queue:
    ...
    返回�?Queue 会存�?`stream_subscribers`，SSE 生成器将不断读取�?
...
def _prune_task_history_locked():
    ...
    仅保留最�?`MAX_TASK_HISTORY` 个任务，避免长时间运行占用过多内存�?

    说明:
        该函数假设调用方已获�?`task_lock`，否则存在竞态风险�?
    """
    ...
    if task_id.startswith('recovered_file_') and report_agent:
        ...
        if output_dir.exists():
            ...
            if target_html.exists():
                temp_task = ReportTask("历史任务 (从文件恢�?", task_id)
                ...
                if len(base_name_parts) >= 2:
                    ...
                    if state_path.exists():
                        ...
                        try:
                            ...
                            # 各引擎目录的报告前缀�?deep_search_report_，匹配查询词开�?
                            ...
                            logger.warning(f"读取历史任务状态文件失�?{state_path}: {e}")
...
def initialize_report_engine():
    ...
    单例�?ReportAgent，方�?API 启动后直接接收任务�?
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"依赖检测失�? {dep_err}")

        return True
    except Exception as e:
        logger.exception(f"Report Engine初始化失�? {str(e)}")
        return False
...
class ReportTask:
    ...
    def update_status(self, status: str, progress: int = None, error_message: str = ""):
        ...
        会自动刷�?`updated_at`、错误信息，并触�?`status` 类型�?SSE�?
    ...
    def history_since(self, last_event_id: Optional[int]) -> List[Dict[str, Any]]:
        ...
        返回:
            list[dict]: �?last_event_id 之后的事件列表�?
        """
        with self._event_lock:
    ...
    try:
        ...
        def stream_handler(event_type: str, payload: Dict[str, Any]):
            if task.is_cancelled:
                raise CancelGenerationException("用户已取消生�?)
            
            # 同步获取最新的增量HTML内容到任务对象中，使�?/result 接口能读�?
        ...
        task.publish_event('stage', {'message': '任务已启动，正在检查输入文�?, 'stage': 'prepare'})

        # 检查输入文�?
        check_result = check_engines_ready(task_id=task.task_id)
        if not check_result['ready']:
            task.update_status("error", 0, f"输入文件未准备就�? {check_result.get('missing_files', [])}")
            return

        task.publish_event('stage', {
            'message': '输入文件检查通过，准备载入内�?,
            'stage': 'io_ready',
        ...
        # �?seed_context 注入�?forum_logs 中（如果存在），作为最高优先级的补充信�?
        if task.seed_context:
            ...
            seed_supplement = (
                ...
                f"信息源标�?Title): {task.seed_filename}\n"
        ...
        task.update_status("cancelled", 0, "用户主动取消了生�?)
        task.publish_event('stage', {'message': '生成已取�?, 'stage': 'cancelled'})
...
def get_seed_content(seed_id: str):
    """
    提供在线预览用户上传�?Seed 内容的接口�?
    前端可以通过弹窗或新标签页调用该接口查看纯文本数据�?
    """
    try:
        # 安全校验，防止目录穿�?
        if ".." in seed_id or "/" in seed_id or "\\" in seed_id:
            return jsonify({'success': False, 'error': '无效�?seed_id'}), 400
        ...
        if not seed_path.exists():
            ...
            if not old_path.exists():
                return jsonify({'success': False, 'error': 'Seed 文件不存在或已过�?}), 404
        ...
        return jsonify({'success': False, 'error': '读取内容时发生错�?}), 500
...
def analyze_seed():
    ...
    try:
        ...
        if not query:
            return jsonify({'success': False, 'error': '未提供原始查�?}), 400

        files = request.files.getlist('files')
        if not files:
            return jsonify({'success': False, 'error': '未提供附�?}), 400
        ...
        primary_filename = file_names[0] if file_names else "上传的附�?

        # 截断过长文本，避免大模型Token爆炸（这里取�?5000字符作为上下文参考）
        ...
        # 触发 seed 入库，将其写入本�?daily_news 表并记录 crawler 日志
        try:
...
def cancel_report(task_id: str):
    ...
    try:
        ...
        if not task:
            return jsonify({'success': False, 'error': '任务不存�?}), 404

        if task.status == 'completed':
            return jsonify({'success': False, 'error': '报告已经生成完成，无法取�?}), 400

        # 设置取消标志位，后台线程会在下一�?stream 事件点抛出异常中�?
        ...
        return jsonify({'success': True, 'message': '已发送取消指�?})
    except Exception as e:
                ...
                try:
                    ...
                    if len(parts) >= 2:
                        ...
                        for engine_name, engine_dir in engine_dirs.items():
                            ...
                            # 子引擎的报告生成在项目根目录下的各个 engine_dir 中，而不是在 final_reports �?
...
def delete_history_task(task_id: str):
    ...
    参数:
        task_id: 要删除的任务ID，如果是恢复的任务，前缀通常�?'recovered_file_'
    """
    try:
        if not report_agent:
            return jsonify({'success': False, 'error': 'Report Engine未初始化'}), 500
...
def get_status():
    ...
    try:
        ...
        logger.exception(f"获取Report Engine状态失�? {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def generate_report():
    ...
    返回:
        Response: JSON，包�?task_id �?SSE stream url�?
    """
    global current_task
    ...
    try:
        ...
        if seed_id:
            ...
            try:
                if seed_path.exists():
                    ...
                    task.seed_filename = seed_data.get('filename', '上传的附�?)
                    task.seed_url = seed_data.get('fake_url', f"seed://{seed_id}/attachment")
                    logger.info(f"任务 {task_id} 已成功加载种子上下文 {seed_id} (JSON)，大�? {len(task.seed_context)} 字符")
                else:
                    # 兼容旧版�?
                    old_path = Path(settings.OUTPUT_DIR) / 'seeds' / f"{seed_id}.txt"
                    if old_path.exists():
                        task.seed_context = old_path.read_text(encoding='utf-8')
                        task.seed_filename = "上传的附�?
                        task.seed_url = f"seed://{seed_id}/attachment"
                        logger.info(f"任务 {task_id} 已成功加载种子上下文 {seed_id} (TXT)，大�? {len(task.seed_context)} 字符")
            except Exception as e:
                logger.error(f"读取种子上下�?{seed_id} 失败: {e}")
        ...
        logger.exception(f"开始生成报告失�? {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def stream_task(task_id: str):
    ...
    def client_disconnected() -> bool:
        ...
        eventlet �?Windows 上会在关闭连接时抛出 ConnectionAbortedError�?
        提前退出生成器可以缩减无意义的日志�?
        """
        try:
        ...
        try:
            ...
            while True:
                ...
                if client_disconnected():
                    logger.info(f"SSE客户端已断开，停止推�? {task_id}")
                    break
                event = None
                try:
                ...
                try:
                    ...
                    logger.info(f"SSE生成器关闭，停止任务 {task_id} 推�?)
                    break
...
def get_result(task_id: str):
    ...
    try:
        ...
        # 优先返回内存中的已渲�?HTML，即使任务未完成（实现“生成一章看一章”）
...
def download_report(task_id: str):
    ...
    try:
        ...
        # 如果内存中没有这个任务，尝试从硬盘中搜索匹配的历史文�?
        if not task:
            # �?final_reports �?document_ir 目录里找
            # task_id 在此处可能是�?ID 也可能是完整文件�?
...
def not_found(error):
    ...
    logger.exception(f"API端点不存�? {str(error)}")
    return jsonify({
        'success': False,
        'error': 'API端点不存�?
    }), 404
...
def internal_error(error):
    """500兜底处理：捕获未被主动捕获的异常"""
    logger.exception(f"服务器内部错�? {str(error)}")
    return jsonify({
        'success': False,
        'error': '服务器内部错�?
    }), 500
...
def clear_report_log():
    ...
    try:
        ...
        # 【修复】使用truncate而非重新打开，避免与logger的文件句柄冲�?
        # 追加模式打开，然后truncate，保持文件句柄有�?
        with open(log_file, 'r+', encoding='utf-8') as f:
            f.truncate(0)  # 清空文件内容但不关闭文件
            f.flush()      # 立即刷新

        logger.info(f"已清空日志文�? {log_file}")
    ...
    try:
        ...
        if file_size > max_size:
            ...
            with open(log_file, 'rb') as f:
                f.seek(-max_size, 2)  # 从文件末尾往�?0MB
                # 跳过可能不完整的第一�?
                f.readline()
                content = f.read().decode('utf-8', errors='replace')
            lines = content.splitlines()
            logger.warning(f"日志文件过大 ({file_size} bytes)，仅返回最�?{max_size} bytes")
        else:
            # 正常大小，完整读�?
...
def export_markdown(task_id: str):
    ...
    try:
        ...
        # 传入 ir_file_path，修复后的图表会自动保存�?IR 文件
    ...
    try:
        ...
        if not pango_available:
            return jsonify({
                ...
                'details': '请查看根目录 README.md “源码启动”的第二步（PDF 导出依赖）了解安装方�?,
                'help_url': 'https://github.com/666ghj/BettaFish#2-安装-pdf-导出所需系统依赖可�?,
