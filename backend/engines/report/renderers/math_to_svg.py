"""
LaTeX 数学公式�?SVG 渲染�?
使用 matplotlib �?LaTeX 公式渲染�?SVG 格式，用�?PDF 导出
"""
...
class MathToSVG:
    ...
    def convert_to_svg(self, latex: str, display_mode: bool = True) -> Optional[str]:
        ...
        Args:
            latex: LaTeX 公式字符串（不包�?$$ �?$ 符号�?
            display_mode: True 为显示模式（块级公式），False 为行内模�?

        Returns:
            SVG 字符串，如果转换失败则返�?None
        """
        try:
            ...
            if display_mode:
                ...
                text = fig.text(
                    ...
                    ha='center',
                    va='center',
                    usetex=False  # 使用 matplotlib 内置�?mathtext 而非完整 LaTeX
                )
            else:
                # 行内模式：左对齐，正常字�?
                text = fig.text(
