"""
关键词优化中间件
使用Qwen AI将Agent生成的搜索词优化为更适合舆情数据库查询的关键�?
"""
...
class KeywordOptimizer:
    ...
    def optimize_keywords(self, original_query: str, context: str = "") -> KeywordOptimizationResponse:
        ...
        Args:
            original_query: Agent生成的原始搜索查�?
            context: 额外的上下文信息（如段落标题、内容描述等�?
            
        Returns:
            KeywordOptimizationResponse: 优化后的关键词列�?
        """
        ...
        try:
            ...
            if response["success"]:
                ...
                try:
                    ...
                    logger.exception(f"⚠️ 解析响应失败，使用备用方�? {str(e)}")
                    # 备用方案：从原始查询中提取关键词
...
**核心要求与最优先指令�?*
1. **你生成的每一个关键词，都必须且只能围绕用户提供的【原始查询主体】展开！绝对禁止生成与原查询主体无关的通用废话（如"最近火了什�?�?热搜�?等）�?*
2. **关键词必须包含原始查询的核心实体**（例如原查询�?达巴水痕之地 bug"，生成的词必须包�?达巴"�?水痕之地"或游戏相关称呼，加上"bug"�?卡死"�?闪退"等变体）�?

**关键词扩展策略（基于专业 SEO/GEO Keyword Research Skill）：**
1. **意图分类 (Search Intent)**：针对原查询的意图进行组合�?
   - **Informational（信息类�?*：引导指南、答疑（�?达巴水痕之地 怎么�?�?什么是..."�?如何..."）�?
   - **Commercial（商业调查）**：评测、对比、最佳列表（�?达巴水痕之地 测评"�?水痕之地 值得买吗"�?水痕之地 vs 其他游戏"）�?
   - **Navigational/Transactional（导�?交易类）**：官方渠道、下载、购买（�?达巴水痕之地 下载"�?水痕之地 价格"）�?
2. **长尾变体 (Long-tail Variations)**：补充修饰词，但必须紧跟核心词�?
   - 包含特定受众、年份、或者修饰符（如"达巴水痕之地 新手攻略"�?水痕之地 真实评价"�?达巴水痕之地 破防"�?水痕之地 2026"）�?
3. **GEO 相关�?(AI-Citation Potential)**：生成更容易触发 AI 生成回答的问答结构词汇�?
   - 定义�?列表�?教程类查询（�?达巴水痕之地 为什�?�?水痕之地 优缺�?�?水痕之地 剧情解析"）�?

**基础约束原则**�?
1. **贴近网民语言**：使用普通网友真实使用的词汇，接地气�?
2. **避免专业术语**：坚决不使用"舆情管理"�?传播路径"等官方学术词汇�?
3. **严禁脑补专有名词**：绝不能自行捏造原查询未提供的游戏版本号、资料片名称、事件名或角色名（例如绝对不要凭空生成“潮蚀纪更新”、�?.0前瞻”等未知定语）。修饰词必须使用通用的意图词汇（如“攻略”、“测评”、“怎么玩”、“评价”、“bug”等）�?
4. **数量控制**：最少提�?0个，最多提�?0个高质量关键词�?
5. **格式严格**：每个关键词都必须是一个不可分割的独立词条，严禁在词条内部包含空格�?

**输出格式**�?
请以JSON格式返回结果，在 reasoning 中简要评估这些词的搜索意�?Intent)分布�?AI 可见�?GEO Potential)�?
{
    "keywords": ["实体+变体1", "实体+修饰2", "核心�?"],
    ...
    def _extract_keywords_from_text(self, text: str) -> List[str]:
        ...
        # 清理和验证关键词
        cleaned_keywords = []
        for keyword in keywords[:20]:  # 最�?0�?
