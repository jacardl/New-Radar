...
class DeepSearchAgent(BaseHermesAgent):
    ...
    def _cluster_and_sample_results(
        ...
        try:
            ...
            logger.info(
                f"  聚类完成: {len(results)} �?-> {n_clusters} 个主�?-> {len(sampled_results)} 条代表性结�?
            )
            return sampled_results

        except Exception as e:
            logger.warning(f"  聚类失败，返回前{max_results}�? {str(e)}")
    ...
    def _perform_sentiment_analysis(self, results: List) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"    �?情感分析过程中发生错�? {str(e)}")
            return None
        ...
        try:
            ...
            if isinstance(texts, str):
                ...
                if not result.analysis_performed:
                    ...
                    response["warning"] = (
                        result.error_message or "情感分析功能不可用，已直接返回原始文�?
                    )
                return response
            else:
    ...
    def research(self, query: str, save_report: bool = True) -> str:
        ...
        try:
            ...
            logger.exception(f"研究过程中发生错�? {str(e)}")
            raise e
        ...
        if hasattr(self.state, 'seed_id') and self.state.seed_id:
            try:
                ...
                if seed_path_json.exists():
                    ...
                    logger.info(f"读取到种子文�?JSON)内容，长�? {len(seed_context)}")
                    ...
                    logger.info(f"读取到种子文�?TXT)内容，长�? {len(seed_context)}")
            except Exception as e:
    ...
    def _process_paragraphs(self):
        ...
        for i in range(total_paragraphs):
            ...
            if i == total_paragraphs - 1:
                logger.info(f"所有段落处理完�?({progress:.1f}%)")
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        if search_tool in ["search_topic_by_date", "search_topic_on_platform"]:
            ...
            if start_date and end_date:
                ...
                if self._validate_date_format(
                    ...
                    logger.info(
                        f"      提供的日�? start_date={start_date}, end_date={end_date}"
                    )
                    search_tool = "search_topic_globally"
        ...
        # 处理限制参数，使用配置文件中的默认值而不是agent提供的参�?
        ...
        if search_results:
            ...
            for j, result in enumerate(search_results, 1):
                date_info = (
                    f" (发布�? {result.get('published_date', 'N/A')})"
        ...
        if paragraph_index == 0 and getattr(self, '_seed_data_dict', {}):
            ...
            seed_title = self._seed_data_dict.get('filename', '用户上传的附�?)
